 ...
 * Layer: Testing (MVC)
 * 
 * Provides common test utilities for UI components including:
 * - Mock service setup
 * - Grid data loading testing
 * - Lazy loading issue detection
 * - Navigation testing utilities
 * - Form validation testing helpers
 */
public abstract class CAbstractUITest<EntityClass extends CEntityDB> {
     ...
     */
    protected CAbstractUITest(Class<EntityClass> entityClass) {
    ...
    /**
     * Sets up test data entities. Should be overridden by subclasses to provide
     * specific test entities with all required relationships initialized.
     */
    ...
    protected void setupServiceMocks() {
        ...
        when(mockEntityService.list(any(Pageable.class)))
            .thenReturn(testEntities);

        // Mock individual entity retrieval
        if (testEntities != null && !testEntities.isEmpty()) {
            EntityClass firstEntity = testEntities.get(0);
            when(mockEntityService.get(firstEntity.getId()))
    ...
    /**
     * Tests grid data loading without exceptions.
     * Simulates the grid data provider callback and verifies no lazy loading exceptions occur.
     * 
     * @param grid the grid to test
     */
    protected void testGridDataLoading(Grid<EntityClass> grid) {
    ...
    /**
     * Tests grid column access for lazy loading issues.
     * Verifies that all configured columns can access entity properties without lazy loading exceptions.
     * 
     * @param grid the grid to test
     */
    protected void testGridColumnAccess(Grid<EntityClass> grid) {
     ...
     * @param grid the grid to test
     */
    protected void testGridSelection(Grid<EntityClass> grid) {
        ...
        try {
            ...
            EntityClass selected = grid.asSingleSelect().getValue();
            assertEquals(testEntity, selected, "Selected entity should match");
            
            // Test deselection
            grid.deselectAll();
            assertNull(grid.asSingleSelect().getValue(), "No entity should be selected after deselect");
    ...
    /**
     * Creates a test entity with the specified ID and name.
     * Should be overridden by subclasses to create properly initialized entities.
     * 
    ...
    /**
     * Verifies that an entity has all required relationships properly initialized
     ...
     */
    protected Page<EntityClass> createPage(List<EntityClass> entities, Pageable pageable) {
        return new PageImpl<>(entities, pageable, entities.size());
    }

    /**
     * Utility method to run assertions without failing the entire test.
     ...
     */
    protected boolean softAssert(Runnable assertion, String description) {
        try {
            assertion.run();
