 ...
 * <p>
 * This demo page shows a typical use case: a grid of activities on the left, and when an activity is selected, its details are displayed on the
 * right using CComponentItemDetails.
 * </p>
...
public class CComponentItemDetailsDemo extends VerticalLayout {
	...
	private CComponentItemDetails create_itemDetails() throws Exception {
		final CComponentItemDetails details = new CComponentItemDetails(sessionService, pageEntityService, detailSectionService);
		details.setWidthFull();
		details.setHeight("600px");

		// Add value change listener to log changes
		details.addValueChangeListener(event -> {
			final CProjectItem<?> newValue = event.getValue();
