...
/**
 * Test class for CDecisionStatusService lazy loading functionality. Tests the enhanced lazy loading fixes for
 * decision status entities.
 */
class CDecisionStatusServiceLazyLoadingTest {
    ...
    void testOverriddenGetMethodUsesEagerLoading() {
        ...
        final CDecisionStatus decisionStatus = new CDecisionStatus("Test Status", project);
        when(decisionStatusRepository.findByIdWithEagerLoading(statusId)).thenReturn(Optional.of(decisionStatus));
        
        // When & Then - Should not throw LazyInitializationException
        assertDoesNotThrow(() -> {
            final Optional<CDecisionStatus> result = decisionStatusService.get(statusId);
    ...
    void testInitializeLazyFieldsHandlesNullEntity() {
        ...
        assertDoesNotThrow(() -> {
            ...
            decisionStatusService.get(null);
        });
    }
    ...
    void testGetWithNullIdReturnsEmpty() {
        // When
        final Optional<CDecisionStatus> result = decisionStatusService.get(null);
