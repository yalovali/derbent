...
public class CActivityStatusViewPlaywrightTest extends CApplicationGeneric_UITest {
	...
	void testActivityStatusFormValidationAndSave() {
		...
		// Verify successful save by checking we're still on same view and no error
		// messages
		final String currentUrl = page.url();
		assertTrue(
			currentUrl.contains("activity-status")
				|| currentUrl.contains("activity-statuses"),
	...
	void testActivityStatusGridSelectionLazyLoading() {
		...
		if (rowCount > 0) {
			...
			if (gridRows.isVisible()) {
				...
				// Verify form is populated without lazy loading exceptions The form
				// should display project information if it has project relationships
				LOGGER.debug(
