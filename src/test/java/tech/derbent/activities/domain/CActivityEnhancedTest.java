...
/**
 * Unit tests for enhanced CActivity class functionality.
 * Tests the new features added for comprehensive project management.
 */
...
class CActivityEnhancedTest {
    ...
    void shouldCreateActivityWithAssignment() {
        CActivity assignedActivity = new CActivity("Assigned Task", project, user1);
    ...
    void shouldHandleTimeTracking() {
        ...
        BigDecimal timeVariance = activity.calculateTimeVariance();
    ...
    void shouldHandleCostTracking() {
        ...
        BigDecimal costVariance = activity.calculateCostVariance();
    ...
    void shouldPreventNegativeValues() {
        ...
        // Note: Actual validation happens through @DecimalMin annotations and service layer
    }
    ...
    void shouldHandleDateManagement() {
        ...
        LocalDate dueDate = LocalDate.of(2025, 7, 15);
        LocalDate completionDate = LocalDate.of(2025, 7, 14);
    ...
    void shouldUpdateLastModifiedDate() {
        LocalDateTime originalModified = activity.getLastModifiedDate();
    ...
    void shouldHandleProgressValidation() {
        ...
        // Note: Invalid values (< 0 or > 100) should be handled by service layer validation
        // The domain object accepts the values but @Min/@Max annotations provide validation hints
    }
    ...
    void shouldHandleAdditionalInformation() {
        String description = "Detailed activity description";
        String acceptanceCriteria = "Must pass all tests and code review";
        String notes = "Important notes about implementation";
    ...
    void shouldCalculateVariancesWithNullValues() {
        ...
        // With only estimated hours set, actual is zero (from getter), so variance = 0 - estimated
        activity.setEstimatedHours(new BigDecimal("10.00"));
        assertEquals(new BigDecimal("-10.00"), activity.calculateTimeVariance()); // 0 - 10 = -10

        // With both values set
        activity.setActualHours(new BigDecimal("8.00"));
        assertEquals(new BigDecimal("-2.00"), activity.calculateTimeVariance()); // 8 - 10 = -2
    }
}