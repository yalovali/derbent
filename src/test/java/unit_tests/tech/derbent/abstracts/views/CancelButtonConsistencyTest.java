...
/** Test to ensure all views that extend CAbstractMDPage have cancel button functionality. This test validates the requirement from the coding
 * guidelines that all views should have cancel buttons to reject changes. */
class CancelButtonConsistencyTest extends CTestBase {
	...
	void testCAbstractMDPageHasCancelButton() {
		assertDoesNotThrow(() -> {
			...
			final Method createCancelButton = CAbstractNamedEntityPage.class.getDeclaredMethod("createCancelButton", String.class);
			...
			// includes cancel
			final Method createDetailsTabButtonLayout = CAbstractNamedEntityPage.class.getDeclaredMethod("createDetailsTabButtonLayout");
	...
	void testCDialogBaseClassExists() {
		assertDoesNotThrow(() -> {
			...
			assertTrue(java.lang.reflect.Modifier.isAbstract(setupButtons.getModifiers()), "setupButtons should be abstract to force implementation");
