...
/**
 * Test to verify that the EntityFormBuilder forField fix resolves the CUserView grid
 * click issue. This ensures that BigDecimal and Integer fields with type converters work
 * correctly without causing "All bindings created with forField must be completed"
 * errors.
 */
public class CEntityFormBuilderForFieldFixVerificationTest {
	...
	/**
	 * Test that simulates the CUserView grid click scenario: 1. Create a form (like view
	 * initialization) 2. Populate form with entity data (like grid row selection) This
	 * should work without forField binding completion errors.
	 */
	@Test
	void testGridClickScenarioWorksWithNumericFields() {
		assertDoesNotThrow(() -> {
			...
			CVerticalLayout formLayout =
				CEntityFormBuilder.buildForm(TestEntityWithNumbers.class, binder);
