...
/**
 * Test to verify that the CEntityFormBuilder fixes for incomplete forField bindings work
 * correctly. This specifically tests the fix for the binding error: "All bindings created
...
public class CEntityFormBuilderBindingTest {
	...
	void testFormBuildingWithNumberFields() {
		assertDoesNotThrow(() -> {
			...
			final CEnhancedBinder<TestEntity> binder =
				new CEnhancedBinder<>(TestEntity.class);
	...
	void testSpecificNumberFieldBindings() {
		assertDoesNotThrow(() -> {
			...
			final Field bigDecimalField =
				TestEntity.class.getDeclaredField("bigDecimalField");
			...
			assertNotNull(formLayout,
				"Form with number fields should be created successfully");
