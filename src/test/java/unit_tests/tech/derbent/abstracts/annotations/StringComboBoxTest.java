	...
	void testStringFieldWithDataProvider() {
		...
		try {
			...
			formLayout.getChildren().forEach(child -> {
				System.out
					.println("Child component: " + child.getClass().getSimpleName());
				child.getChildren().forEach(grandchild -> {
					System.out.println(
						"  - Grandchild: " + grandchild.getClass().getSimpleName());
					// Look for ComboBox components
					grandchild.getChildren().forEach(greatGrandchild -> {
						System.out.println("    - Great-grandchild: "
							+ greatGrandchild.getClass().getSimpleName());

						if (greatGrandchild.getClass().getSimpleName()
							.contains("ComboBox")) {
		...
		assertNull(caughtException,
			"No exception should be thrown when creating String ComboBox");
	}
}