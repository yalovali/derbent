...
public abstract class CGenericViewTest<T> extends CBaseUITest {
	...
	protected String getViewRoute() {
		...
		Check.notNull(routeAnnotation, "View class " + getViewClass().getSimpleName()
			+ " must have @Route annotation");
	...
	public void testCRUDOperations() {
		...
		LOGGER.debug("🧪 Testing CRUD operations for {} with comprehensive validation",
			entityName);
		...
		try {
			...
			wait_1000();
			// Verify form opened
			final var formFields =
				page.locator("vaadin-text-field, vaadin-text-area, vaadin-combo-box");
			...
			if (formFields.count() > 0) {
				...
				final var validationErrors =
					page.locator(".v-errormessage, vaadin-error-message");
				LOGGER.debug(
					"Validation test: found {} validation messages for empty form",
				...
				LOGGER.debug(
					"No form fields found - create operation may not be available for {} view",
					entityName);
			}
			...
			if (gridCells.count() > 0) {
				LOGGER.debug(
					"Successfully verified Read operation - grid displays {} data",
					entityName);
			}
			LOGGER.debug("✅ CRUD operations test completed for {}", entityName);
		} catch (final Exception e) {
			LOGGER.error("❌ CRUD operations test failed for {}: {}", entityName,
				e.getMessage());
			takeScreenshot("crud-operations-failed-" + entityName.toLowerCase(), true);
			throw new AssertionError(
				"CRUD operations test failed for " + entityName + ": " + e.getMessage(),
				e);
		}
	}
	...
	public void testGridInteractions() {
		...
		try {
			...
			if (gridRowCount > 0) {
				...
				LOGGER.debug("Successfully clicked first row in {} grid without errors",
					entityName);
				...
				LOGGER.debug("No rows in {} grid - this is acceptable for empty views",
					entityName);
			}
			LOGGER.debug("✅ {} grid interactions test completed successfully",
				entityName);
		} catch (final Exception e) {
			LOGGER.error("❌ Grid interactions test failed for {}: {}", entityName,
				e.getMessage());
			takeScreenshot("grid-interactions-failed-" + entityName.toLowerCase(), true);
			throw new AssertionError(
				"Grid interactions test failed for " + entityName + ": " + e.getMessage(),
				e);
		}
	}
	...
	public void testNewItemCreation() {
		...
		try {
			...
			final var formElements =
				page.locator("vaadin-form-layout, vaadin-text-field, vaadin-text-area");
			final int formElementCount = formElements.count();
			Check.condition(formElementCount > 0,
				"Form elements should appear after clicking New in " + entityName
					+ " view");
			...
			Check.condition(finalRowCount == initialRowCount,
				"Row count should be unchanged after canceling new " + entityName
					+ " creation");
			LOGGER.debug("✅ New {} creation test completed successfully", entityName);
		} catch (final Exception e) {
			LOGGER.error("❌ New item creation test failed for {}: {}", entityName,
				e.getMessage());
			takeScreenshot("new-item-test-failed-" + entityName.toLowerCase(), true);
			throw new AssertionError(
				"New item creation test failed for " + entityName + ": " + e.getMessage(),
				e);
		}
	}
	...
	public void testViewAccessibility() {
		...
		try {
			...
			final var focusableElements = page.locator(
			...
			LOGGER.debug("{} view has {} elements with ARIA attributes", entityName,
				ariaElements.count());
			...
			for (int i = 0; i < Math.min(buttons.count(), 3); i++) {
				...
				Check.condition(
					...
					"Buttons in " + entityName
						+ " view should have accessible text or aria-label");
			}
			testAccessibilityBasics(entityName);
			LOGGER.debug("✅ {} accessibility test completed successfully", entityName);
		} catch (final Exception e) {
			LOGGER.error("❌ Accessibility test failed for {}: {}", entityName,
				e.getMessage());
			takeScreenshot("accessibility-test-failed-" + entityName.toLowerCase(), true);
			throw new AssertionError(
				"Accessibility test failed for " + entityName + ": " + e.getMessage(), e);
		}
	}
	...
	public void testViewComboBoxes() {
		final String entityName = getEntityDisplayName();
		LOGGER.debug("🧪 Testing {} ComboBox components with enhanced verification",
			entityName);
		...
		try {
			...
			if (!CTestUtils.isFormVisible(page)) {
				LOGGER.debug(
					"No New button available in {} view - skipping ComboBox test",
					entityName);
				return;
			}
			wait_1000();
			...
			LOGGER.debug("Found {} ComboBox components in {} form", comboBoxCount,
				entityName);
			...
			for (int i = 0; i < Math.min(comboBoxCount, 5); i++) {
				...
				try {
					...
					final var options = page.locator(
						"vaadin-combo-box-item, vaadin-combo-box-dropdown-wrapper vaadin-item");
					final int optionCount = options.count();
					LOGGER.debug("ComboBox '{}' has {} options available", comboBoxLabel,
						optionCount);
					// Verify ComboBox functionality
					Check.condition(optionCount >= 0, "ComboBox '" + comboBoxLabel
						+ "' should have accessible options without errors");

					if (optionCount > 0) {
						options.first().click();
						wait_500();
						LOGGER.debug(
							"Successfully selected first option in ComboBox '{}'",
							comboBoxLabel);
					}
					...
					LOGGER.warn("Error testing ComboBox {} in {}: {}", i, entityName,
						cbException.getMessage());
			...
			throw new AssertionError(
				"ComboBox test failed for " + entityName + ": " + e.getMessage(), e);
		}
	}
	...
	public void testViewLoading() {
		...
		try {
			...
			Check.condition((gridCount > 0) || (buttonCount > 0) || (formCount > 0),
				entityName
					+ " view should contain at least one interactive element (grid, button, or form)");
			...
			Check.condition(
				currentUrl.contains(getViewRoute())
					|| currentUrl.contains(entityName.toLowerCase()),
				"URL should reflect the " + entityName + " view route");
			// Test responsive layout basics
			final var mainLayout = page.locator(
				"vaadin-app-layout, vaadin-vertical-layout, vaadin-horizontal-layout");
			...
			LOGGER.error("❌ View loading test failed for {}: {}", entityName,
				e.getMessage());
			takeScreenshot("view-loading-failed-" + entityName.toLowerCase(), true);
			throw new AssertionError(
				"View loading test failed for " + entityName + ": " + e.getMessage(), e);
		}
	}
	...
	public void testViewNavigation() {
		final String entityName = getEntityDisplayName();
		LOGGER.debug("🧪 Testing navigation to {} view with enhanced validation",
			entityName);
		...
		try {
			...
			Check.condition(
				currentUrl.contains(expectedRoute)
					|| currentUrl.contains(entityName.toLowerCase()),
				"Navigation should lead to correct URL containing '" + expectedRoute
					+ "' or '" + entityName.toLowerCase() + "'");
			// Verify page is responsive and loaded
			final var pageContent = page.locator("body");
			Check.condition(pageContent.isVisible(),
				"Page content should be visible after navigation to " + entityName
					+ " view");
			...
			Check.condition(
				finalUrl.contains(expectedRoute)
					|| finalUrl.contains(entityName.toLowerCase()),
			...
			throw new AssertionError(
				"Navigation test failed for " + entityName + " view: " + e.getMessage(),
				e);
		}
	}
}