...
/**
 * CTestUtils - Common utility functions for UI testing Provides reusable functions with
 * annotation-based approach and reduced logging. Uses class metadata instead of magic
...
public final class CTestUtils {
	 ...
	 * @param viewName the view name for logging
	...
	public static boolean checkAccessibility(final Page page, final String viewName) {
		...
		try {
			...
			LOGGER.debug(
				"Accessibility check for {}: {} headings, {} aria-labeled elements, {} buttons",
				viewName, headingCount, ariaCount, buttonCount);
			return (headingCount > 0) && (buttonCount > 0);
		} catch (final Exception e) {
			LOGGER.error("Accessibility check failed for {}: {}", viewName,
				e.getMessage());
			return false;
		}
	}
	 ...
	 * @param page     the Playwright page
	 * @param rowIndex the row index to click (0-based)
	 * @return true if grid row was found and clicked
	 */
	public static boolean clickGrid(final Page page, final int rowIndex) {
		...
		try {
			...
			if (gridCells.count() > rowIndex) {
				...
				LOGGER.debug("Grid row at index {} not found (total rows: {})", rowIndex,
					gridCells.count());
				return false;
			}
		} catch (final Exception e) {
	 ...
	 * @param viewClass the view class with @Route annotation
	...
	public static String getRouteFromClass(final Class<?> viewClass) {
		...
		if (routeAnnotation == null) {
			throw new IllegalArgumentException(
				"Class " + viewClass.getSimpleName() + " has no @Route annotation");
		}
	...
	public static boolean isFormVisible(final Page page) {
		final int formElements = page
			.locator("vaadin-form-layout, vaadin-text-field, vaadin-text-area").count();
	 ...
	 * @param page the Playwright page
	 * @param rowIndex the row index to select (0-based)
	 ...
	 * @param page the Playwright page
	 * @param viewName the view name for context
	...
	public static int checkAccessibilityScore(final Page page, final String viewName) {
		try {
			...
			if (semanticElements.count() > 0) score += 20;
	 ...
	 * @param page the Playwright page
	 * @param comboBoxIndex the ComboBox index to test
	 ...
	 * @param page the Playwright page
	 * @param expectedRoute the expected route path
	 * @param viewName the view name for context
	...
	public static boolean validateViewUrl(final Page page, final String expectedRoute, final String viewName) {
		try {
			...
			final boolean urlValid = currentUrl.contains(expectedRoute) || 
				currentUrl.contains(viewName.toLowerCase());
