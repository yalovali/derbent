package unit_tests.tech.derbent.base.ui.view;
...
/** Integration test for dashboard data availability. This test verifies that the database is properly initialized with sample data and that the
 * dashboard services can retrieve the data correctly. */
@SpringBootTest
...
class CDashboardViewIntegrationTest extends CTestBase {
	...
	void testDashboardDataInitialization() {
		...
		final boolean hasDigitalTransformation = projects.stream().anyMatch(p -> "Digital Transformation Initiative".equals(p.getName()));
		assertTrue(hasDigitalTransformation, "Should have 'Digital Transformation Initiative' from CSampleDataInitializer");
		final boolean hasProductDevelopment = projects.stream().anyMatch(p -> "Product Development Phase 2".equals(p.getName()));
		assertTrue(hasProductDevelopment, "Should have 'Product Development Phase 2' from CSampleDataInitializer");
		final boolean hasInfrastructureModernization = projects.stream().anyMatch(p -> "Infrastructure Modernization".equals(p.getName()));
		assertTrue(hasInfrastructureModernization, "Should have 'Infrastructure Modernization' from CSampleDataInitializer");
		final boolean hasCustomerExperience = projects.stream().anyMatch(p -> "Customer Experience Enhancement".equals(p.getName()));
