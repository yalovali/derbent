...
/**
 * Integration test for dashboard data availability. This test verifies that the database
 * is properly initialized with sample data and that the dashboard services can retrieve
 * the data correctly.
 */
@SpringBootTest
...
class CDashboardViewIntegrationTest extends CTestBase {
	...
	void testDashboardDataInitialization() {
		...
		for (final CProject project : projects) {
			System.out
				.println("  - " + project.getName() + " (ID: " + project.getId() + ")");
		...
		assertTrue(hasProductDevelopment,
			"Should have 'Product Development Phase 2' from CSampleDataInitializer");
		...
		assertTrue(hasCustomerExperience,
			"Should have 'Customer Experience Enhancement' from CSampleDataInitializer");
	}
}