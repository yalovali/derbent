...
/** Test class for CActivityStatusService lazy loading functionality. Tests the enhanced lazy loading fixes for activity status entities. */
class CActivityStatusServiceLazyLoadingTest extends CTestBase {
	...
	void testOverriddenGetMethodUsesEagerLoading() {
		...
		when(activityStatusRepository.findById(statusId)).thenReturn(Optional.of(activityStatus));
