...
/** Manual verification tool to demonstrate the profile picture functionality. This class shows the end-to-end functionality without requiring a
 * database connection. */
public class CProfilePictureManualVerification {
	...
	private void displayUserInGrid(final CUser user) {
		...
		LOGGER.info("Grid Row: [{}] {} {} | {} | {} | {}", profilePictureDisplay.substring(0, Math.min(20, profilePictureDisplay.length())) + "...",
				user.getName(), user.getLastname(), user.getLogin(), user.getEmail(), user.isEnabled() ? "Enabled" : "Disabled");
	}
	...
	public void manualVerificationOfProfilePictureFunctionality() throws IOException {
		...
		LOGGER.info("Created test images: Red ({} bytes), Blue ({} bytes), Green ({} bytes)", redImage.length, blueImage.length, greenImage.length);
		...
		LOGGER.info("Resized images to {}x{} pixels", CImageUtils.PROFILE_PICTURE_WIDTH, CImageUtils.PROFILE_PICTURE_HEIGHT);
		...
		LOGGER.info("✅ Image resizing to {}x{} works correctly", CImageUtils.PROFILE_PICTURE_WIDTH, CImageUtils.PROFILE_PICTURE_HEIGHT);
