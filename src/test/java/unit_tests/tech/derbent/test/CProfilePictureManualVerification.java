...
/**
 * Manual verification tool to demonstrate the profile picture functionality. This class
 * shows the end-to-end functionality without requiring a database connection.
 */
public class CProfilePictureManualVerification {

	private static final Logger LOGGER =
		LoggerFactory.getLogger(CProfilePictureManualVerification.class);

	/**
	 * Creates a simple test image as byte array with specified color.
	 */
	private byte[] createTestImage(final int width, final int height, final Color color)
		throws IOException {
	...
	private void displayUserInGrid(final CUser user) {
		...
		LOGGER.info("Grid Row: [{}] {} {} | {} | {} | {}",
			profilePictureDisplay.substring(0,
				Math.min(20, profilePictureDisplay.length())) + "...",
	...
	public void manualVerificationOfProfilePictureFunctionality() throws IOException {
		...
		final CUser user1 =
			new CUser("admin", "password", "System", "admin@derbent.tech");
		final CUser user2 =
			new CUser("sarah.johnson", "password", "Sarah", "sarah.johnson@derbent.tech");
		final CUser user3 =
			new CUser("michael.chen", "password", "Michael", "michael.chen@derbent.tech");
		...
		LOGGER.info(
			"Created test images: Red ({} bytes), Blue ({} bytes), Green ({} bytes)",
			redImage.length, blueImage.length, greenImage.length);
		...
		LOGGER.info("✅ Image resizing to {}x{} works correctly",
			CImageUtils.PROFILE_PICTURE_WIDTH, CImageUtils.PROFILE_PICTURE_HEIGHT);
