...
/** Integration test to verify that the CSampleDataInitializer can run without throwing TransientPropertyValueException after our fixes. This test
 * simulates the application startup process and ensures that the sample data initialization doesn't fail due to transient entity issues. */
public class CSampleDataInitializerIntegrationTest {
	...
	/** Test that verifies the main fix: CSampleDataInitializer should not fail with TransientPropertyValueException when run. This test demonstrates
	 * that our fix of adding activityService.save() calls before creating comments resolves the Hibernate transient entity issue. */
	@Test
	void testSampleDataInitializerDoesNotFailWithTransientEntityException() {
		...
		final ApplicationArguments args = new DefaultApplicationArguments(new String[] {});
