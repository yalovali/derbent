...
/** Test to reproduce and fix the CUsersView binding issue: "All bindings created with forField must be completed before calling readBean" This test
 * specifically addresses the navigation error when trying to access /users */
...
@TestPropertySource (properties = {
		...
		"server.port=8080", "spring.sql.init.mode=never"
})
public class CUsersViewBindingTest {
	...
	/** Test that CUsersView can be instantiated without throwing the binding error. This reproduces the exact issue: "All bindings created with
	 * forField must be completed before calling readBean" */
	@Test
	...
	/** Test that CUsersView can navigate to a user (which triggers populateForm) without binding errors. */
	@Test
