...
/** Integration test for CUsersView to verify that navigation works without forField binding errors. This test simulates the exact scenario described
 * in the problem statement: "There was an exception while trying to navigate to 'users'" */
...
public class CUsersViewNavigationTest {
	...
	/** Test that form clearing and repopulation works correctly. This simulates the navigation scenario that could trigger binding errors. */
	@Test
	void testFormClearAndRepopulate() {
		assertDoesNotThrow(() -> {
			final CUsersView view = new CUsersView(userService, projectService, userTypeService, companyService, sessionService,
					userProjectSettingsService, screenService);
			// Create test users
			...
			assertNotNull(view, "CUsersView should handle form clearing and repopulation successfully");
		}, "Form clearing and repopulation should work without binding errors");
	}

	/** Test navigation to the users view without any user ID (list view). This should not throw: "All bindings created with forField must be
	...
	void testNavigateToUsersListView() {
		assertDoesNotThrow(() -> {
			...
			assertNotNull(view, "CUsersView should handle navigation to list view successfully");
