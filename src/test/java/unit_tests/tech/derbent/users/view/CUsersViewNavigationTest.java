...
/**
 * Integration test for CUsersView to verify that navigation works without forField binding errors.
 * This test simulates the exact scenario described in the problem statement:
 * "There was an exception while trying to navigate to 'users'"
 */
@SpringBootTest(classes = tech.derbent.Application.class, webEnvironment = WebEnvironment.RANDOM_PORT)
...
public class CUsersViewNavigationTest {
    ...
    /**
     * Test navigation to the users view without any user ID (list view).
    ...
    void testNavigateToUsersListView() {
        assertDoesNotThrow(() -> {
            ...
            // Test that the view can handle null population (simulating navigation to list view)
    ...
    /**
     * Test that form clearing and repopulation works correctly.
     * This simulates the navigation scenario that could trigger binding errors.
     */
    @Test
    void testFormClearAndRepopulate() {
        assertDoesNotThrow(() -> {
            ...
            // Simulate navigation between users (this is where binding errors could occur)
