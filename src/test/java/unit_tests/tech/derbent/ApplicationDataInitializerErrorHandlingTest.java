...
/**
 * Test to verify that the Application's dataInitializer handles the PostgreSQL "relation
 * cuser does not exist" error gracefully.
 */
public class ApplicationDataInitializerErrorHandlingTest {
	...
	public void shouldHandleCuserTableNotExistError() throws Exception {
		...
		when(mockJdbcTemplate.queryForObject(eq("SELECT COUNT(*) FROM cuser"),
			eq(Integer.class))).thenThrow(
				new DataAccessException("ERROR: relation \"cuser\" does not exist") {
