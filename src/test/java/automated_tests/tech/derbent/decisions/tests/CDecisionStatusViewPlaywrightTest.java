package automated_tests.tech.derbent.decisions.tests;
...
/** CDecisionStatusViewPlaywrightTest - Tests for decision status view focusing on lazy loading fixes and navigation behavior after save
 * operations. */
...
public class CDecisionStatusViewPlaywrightTest extends CApplicationGeneric_UITest {
	...
	void testDecisionStatusColorAndSortOrder() {
		...
		assertTrue(colorPickers.count() > 0, "Color picker should be available in the decision status form");
		...
		assertTrue(numberFields.count() > 0, "Sort order field should be available in the decision status form");
