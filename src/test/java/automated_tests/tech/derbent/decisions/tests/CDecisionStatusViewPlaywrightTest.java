...
public class CDecisionStatusViewPlaywrightTest extends CApplicationGeneric_UITest {
	...
	private static final Logger LOGGER =
		LoggerFactory.getLogger(CDecisionStatusViewPlaywrightTest.class);
	...
	void testDecisionStatusApprovalRequirement() {
		...
		final String statusName =
			"Approval Required Status " + System.currentTimeMillis();
	...
	void testDecisionStatusGridSelectionLazyLoading() {
		...
		if (rowCount > 0) {
			...
			if (gridRows.isVisible()) {
				...
				LOGGER.debug(
					"Form populated after grid selection - checking for lazy loading issues");
			}
		}
		else {
	...
	void testDecisionStatusLazyLoadingAndNavigation() {
		...
		if (textAreas.count() > 0) {
			textAreas.first()
				.fill("Test description for decision status lazy loading test");
		}
		...
		assertTrue(
			currentUrl.contains("decision-status")
				|| currentUrl.contains("decision-statuses"),
