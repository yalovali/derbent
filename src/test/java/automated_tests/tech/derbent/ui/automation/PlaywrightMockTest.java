...
public class PlaywrightMockTest extends CBaseUITest {
	...
	private void createMockScreenshot(final String viewName) throws Exception {
		...
		final String filename =
			"mock-" + viewName + "-" + System.currentTimeMillis() + ".png";
		final File outputFile = new File("target/screenshots/" + filename);
		ImageIO.write(image, "png", outputFile);
	...
	private void createWorkflowScreenshot() throws Exception {
		...
		g2d.setFont(new Font("Arial", Font.PLAIN, 14));
		final String[] steps = {
			"1. Navigate to Users view", "2. Navigate to Projects view",
			"3. Navigate to Activities view", "4. Navigate to Meetings view",
			"5. Navigate to Decisions view", "6. Create new project",
			"7. Fill project form", "8. Save project" };
		...
		g2d.dispose();
		final String filename =
			"workflow-complete-" + System.currentTimeMillis() + ".png";
		final File outputFile = new File("target/screenshots/" + filename);
		ImageIO.write(image, "png", outputFile);
	...
	private void createWorkflowStepScreenshot(final String stepName) throws Exception {
		...
		g2d.setColor(Color.WHITE);
		g2d.setFont(new Font("Arial", Font.BOLD, 18));
		g2d.drawString("Workflow Step: " + stepName.replace("-", " ").toUpperCase(), 20,
			45);
		// Step content
		g2d.setColor(Color.BLACK);
		g2d.setFont(new Font("Arial", Font.PLAIN, 14));
	...
	void testPlaywrightScreenshotFunctionality() {
		...
		try {
			...
			final File[] screenshots =
				screenshotsDir.listFiles((dir, name) -> name.endsWith(".png"));

			if (screenshots != null && screenshots.length > 0) {
				LOGGER.info("📸 Generated {} mock screenshots in target/screenshots/",
					screenshots.length);
