...
/**
 * Comprehensive Playwright test suite for dynamically created entity pages.
 * Tests navigation to pages by entity type, CRUD operations, and error handling.
 * This test never ignores exceptions and fails fast on any errors.
 */
...
public class CDynamicEntityPagesPlaywrightTest extends CBaseUITest {
    ...
    // Entity types that commonly have project relations
    private static final String[] PROJECT_RELATION_ENTITIES = {
        "CUser", "CActivity", "CMeeting", "COrder"
    };
    ...
    void testUserPageProjectRelations() {
        ...
        try {
            ...
            Check.isTrue(navigateToDynamicPageByEntityType("CUser"), 
                "Must be able to navigate to CUser page");
    ...
    private void testUserProjectRelationsDialog() {
        try {
            ...
            Locator projectButtons = page.locator("vaadin-button").filter(
                new Locator.FilterOptions().setHasText("Project"));
    ...
    private void createTestUser() {
        try {
            ...
            Locator emailField = page.locator("vaadin-text-field").filter(
    ...
    private boolean hasGridSupport() {
        try {
            return page.locator("vaadin-grid").count() > 0 && 
                   page.locator("vaadin-button:has-text('New')").count() > 0;
        } catch (Exception e) {
    ...
    protected void loginToApplicationEnhanced() {
        try {
            ...
            page.waitForSelector("vaadin-side-nav, vaadin-app-layout", 
