...
/** UserColorAndEntryViewsPlaywrightTest - Comprehensive Playwright tests for user color functionality and entry views status/type validation. This
 * test class specifically validates: 1. User color display and functionality in color-aware components 2. Status entry views display colors correctly
 * 3. Type entry views display colors correctly 4. Color-aware ComboBox components work properly 5. Entry forms show status and type fields
 * properly */
...
public class UserColorAndEntryViewsPlaywrightTest extends CBaseUITest {
			...
			if (comboBoxCount > 0) {
				...
				try {
					...
					if (dropdownOpen) {
						...
						final boolean hasColoredOptions = (page.locator("vaadin-combo-box-item[style*='background']").count() > 0)
	...
	void testColorContrastAndAccessibility() {
		...
		for (final Class<?> view : statusViews) {
			...
			final boolean hasTabableElements = page.locator("[tabindex], button, input, select, textarea, [href]").count() > 0;
	...
	void testCompleteUserColorWorkflow() {
		...
		final boolean hasUserFields = (page.locator("vaadin-combo-box[label*='User'], vaadin-combo-box[label*='user']").count() > 0)
		...
		final boolean hasStatusTypeFields = page.locator("vaadin-combo-box[label*='Status'], vaadin-combo-box[label*='Type']").count() > 0;
	...
	void testEntryViewsShowStatusAndTypeFields() {
		...
		for (final Class<?> entryView : entryViews) {
			...
			final boolean hasStatusField = (page.locator("vaadin-combo-box[label*='Status'], vaadin-combo-box[label*='status']").count() > 0)
					|| (page.locator("vaadin-select[label*='Status'], vaadin-select[label*='status']").count() > 0);
			// Check for type fields (ComboBox or similar)
			final boolean hasTypeField = (page.locator("vaadin-combo-box[label*='Type'], vaadin-combo-box[label*='type']").count() > 0)
			...
			final boolean hasColorAwareComponents =
					(page.locator("[class*='color-aware']").count() > 0) || (page.locator("[style*='background-color']").count() > 0);
	...
	void testStatusViewsDisplayColorsCorrectly() {
		...
		for (final Class<?> statusView : statusViews) {
			...
			if (rowCount > 0) {
				...
				final boolean hasColorField = page.locator("vaadin-text-field[label*='Color'], vaadin-text-field[label*='color']").count() > 0;
				if (hasColorField) {
				...
				final boolean hasStyledStatus = page.locator("[style*='background'], [class*='status-'], [class*='color-']").count() > 0;
				if (hasStyledStatus) {
	...
	void testTypeViewsDisplayColorsCorrectly() {
		...
		for (final Class<?> typeView : typeViews) {
			...
			if (rowCount > 0) {
				...
				final boolean hasActiveField = page.locator("vaadin-checkbox[label*='Active'], vaadin-checkbox[label*='active']").count() > 0;
				if (hasActiveField) {
	...
	void testUserColorDisplayInColorAwareComponents() {
		...
		final boolean hasUserSelectionComponents =
				(page.locator("vaadin-combo-box").count() > 0) || (page.locator("[class*='color-aware']").count() > 0);
