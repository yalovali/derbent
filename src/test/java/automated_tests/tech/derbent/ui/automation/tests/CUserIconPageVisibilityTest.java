...
public class CUserIconPageVisibilityTest extends CBaseUITest {
	...
	void testUserIconPageVisibilityAndIconDisplay() {
		...
		try {
			...
			final String testSupportUrl = "http://localhost:" + port + "/cpagetestauxillary";
			...
			final Locator userIconButton = page.locator("[data-title='User Icon Test']");
			final int buttonCount = userIconButton.count();
			if (buttonCount > 0) {
				...
				final Locator allButtons = page.locator("[id^='test-aux-btn-']");
				final int totalButtons = allButtons.count();
				LOGGER.error("Found {} total buttons on Test Support Page:", totalButtons);
				for (int i = 0; i < totalButtons; i++) {
					final String title = allButtons.nth(i).getAttribute("data-title");
					final String route = allButtons.nth(i).getAttribute("data-route");
			...
			final String directUrl = "http://localhost:" + port + "/user-icon-test";
			...
			final Locator pageTitle = page.locator("h2:has-text('SVG Icon Test Page')");
			...
			// Check for SVG icons
			final Locator svgIcons = page.locator("svg");
			final int svgCount = svgIcons.count();
			LOGGER.info("Found {} SVG elements on the page", svgCount);
			// Check for Vaadin icons (icon elements)
			final Locator iconElements = page.locator("vaadin-icon");
			final int iconCount = iconElements.count();
			LOGGER.info("Found {} vaadin-icon elements on the page", iconCount);
			// Check for user labels (CLabelEntity)
			final Locator userLabels = page.locator("div:has-text('Login:')");
			final int labelCount = userLabels.count();
