...
/**
 * Fast hierarchical menu navigation test - logs in and browses all menu items at all levels.
...
public class CMenuNavigationTest extends CBaseUITest {
	...
	private void exploreMenuLevel(int depth) {
		...
		Locator menuItems = page.locator(MENU_ITEM_SELECTOR);
		...
		for (int i = 0; i < itemCount; i++) {
			try {
				...
				Locator items = page.locator(MENU_ITEM_SELECTOR);
				if (i >= items.count()) {
				...
				boolean hasSubMenu = checkIfHasSubMenu(item);
				...
				if (hasSubMenu) {
					...
					String safeName = makeSafeName(label);
	 ...
	 */
	private boolean checkIfHasSubMenu(Locator item) {
		try {
			// Check if item has arrow icon or navigation arrow
			Locator arrow = item.locator("vaadin-icon[icon*='angle-right'], vaadin-icon[icon*='arrow-right']");
	...
	private void goBackToParentMenu(String indent) {
		try {
			Locator backButton = page.locator(BACK_BUTTON_SELECTOR);
