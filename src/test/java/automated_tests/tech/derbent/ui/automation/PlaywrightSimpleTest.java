...
/**
 * Simple Playwright test to demonstrate screenshot functionality
 * This test uses a minimal setup to avoid browser download issues
 */
...
public class PlaywrightSimpleTest {
		...
		try {
			...
			try (Playwright playwright = Playwright.create()) {
				BrowserType.LaunchOptions options = new BrowserType.LaunchOptions()
					.setHeadless(true)
					.setExecutablePath(java.nio.file.Paths.get("/usr/bin/chromium-browser"));
				...
				try (Browser browser = playwright.chromium().launch(options)) {
					...
					// Take a screenshot
					String screenshotPath = "target/screenshots/test-screenshot-" + System.currentTimeMillis() + ".png";
			...
			LOGGER.warn("Screenshot test skipped due to browser setup issue: {}", e.getMessage());
	...
	void testScreenshotDirectoryCreation() {
		...
		try {
			...
			java.nio.file.Path testFile = screenshotDir.resolve("test-file.txt");
