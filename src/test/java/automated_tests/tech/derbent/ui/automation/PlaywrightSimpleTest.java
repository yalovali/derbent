...
/**
 * Simple Playwright test to demonstrate screenshot functionality This test uses a minimal
...
public class PlaywrightSimpleTest extends CBaseUITest {
	...
	private static final Logger LOGGER =
		LoggerFactory.getLogger(PlaywrightSimpleTest.class);
	...
	void testBasicScreenshotFunctionality() {
		...
		try {
			...
			if (Paths.get("/usr/bin/chromium-browser").toFile().exists()) {
				options.setExecutablePath(
					java.nio.file.Paths.get("/usr/bin/chromium-browser"));
				LOGGER.info("Using system Chromium browser for Playwright tests");
			}
			else if (Paths.get("/usr/bin/google-chrome").toFile().exists()) {
				options
					.setExecutablePath(java.nio.file.Paths.get("/usr/bin/google-chrome"));
				LOGGER.info("Using system google chrome browser for Playwright tests");
			}
			else {
				LOGGER.warn(
					"No compatible browser found in system path, using default Chromium");
			}
			...
			page.navigate(
				"data:text/html,<html><body><h1>Playwright Test</h1><p>Screenshot test successful!</p></body></html>");
			// Take a screenshot
			final String screenshotPath = "target/screenshots/test-screenshot-"
	...
	void testScreenshotDirectoryCreation() {
		...
		try {
			...
			final java.nio.file.Path screenshotDir =
