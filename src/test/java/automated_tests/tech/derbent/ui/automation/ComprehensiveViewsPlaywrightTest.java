...
/**
 * ComprehensiveViewsPlaywrightTest - Comprehensive Playwright UI automation test suite
 * for ALL views including main views, status views, type views, admin views, and more.
 * This test class implements the requirement to "run playwright tests for all views and
 * status and type views all" by testing every accessible view in the application.
 */
@SpringBootTest (
...
public class ComprehensiveViewsPlaywrightTest extends CBaseUITest {
	...
	private static final Logger LOGGER =
		LoggerFactory.getLogger(ComprehensiveViewsPlaywrightTest.class);
	...
	void testAccessibilityForAllViews() {
		...
		for (final Class<?> view : allViewClasses) {
			...
			try {
				...
				takeScreenshot("accessibility-all-" + view.getSimpleName().toLowerCase(),
					false);
				testAccessibilityBasics(view.getSimpleName());
			} catch (final Exception e) {
				LOGGER.warn("⚠️ Accessibility test failed for view {}: {}",
					view.getSimpleName(), e.getMessage());
	...
	void testAllStatusAndTypeViews() {
		...
		for (final Class<?> view : statusAndTypeViewClasses) {
			...
			navigateToViewByClass(view);
			takeScreenshot("status-type-view-" + view.getSimpleName().toLowerCase(),
				false);
			testBasicViewFunctionality(view.getSimpleName());
		}
	...
	private void testBasicViewFunctionality(final String viewName) {
		...
		try {
			...
			final boolean hasContent = page.locator("body").isVisible()
				&& !page.locator("body").textContent().trim().isEmpty();
			assertTrue(hasContent, "View " + viewName + " should have visible content");
			// Check for common UI elements
			final boolean hasVaadinElements = page
				.locator("vaadin-grid, vaadin-button, vaadin-text-field, "
					+ "vaadin-form-layout, vaadin-vertical-layout, vaadin-horizontal-layout")
				.count() > 0;
			...
			LOGGER.warn("Basic functionality test failed for view {}: {}", viewName,
				e.getMessage());
			throw e;
		}
	}
	...
	void testComprehensiveAllViews() {
		LOGGER.info("🧪 Testing ALL views comprehensively ({} total views)...",
		...
		for (final Class<?> view : allViewClasses) {
			...
			try {
				LOGGER.info("Testing comprehensive view ({}/{}): {}", successCount + 1,
					totalViews, view.getSimpleName());
				navigateToViewByClass(view);
				takeScreenshot("comprehensive-all-" + view.getSimpleName().toLowerCase(),
					false);
				testBasicViewFunctionality(view.getSimpleName());
				successCount++;
				LOGGER.debug("✅ Successfully tested view: {}", view.getSimpleName());
			} catch (final Exception e) {
				LOGGER.warn("⚠️ Failed to test view {}: {}", view.getSimpleName(),
					e.getMessage());
				// Continue with other views even if one fails
			}
		}
		LOGGER.info(
			"✅ Comprehensive all views test completed: {}/{} views tested successfully",
			successCount, totalViews);
		// Ensure at least 80% of views were tested successfully
		assertTrue(successCount >= (totalViews * 0.8), String.format(
			"Expected at least 80%% of views to be tested successfully, but only %d/%d (%.1f%%) passed",
			successCount, totalViews, ((successCount * 100.0) / totalViews)));
	}
	...
	private void testNewButtonFunctionality(final String viewName) {
		...
		try {
			...
			final var newButtons = page.locator("vaadin-button:has-text('New'), "
				+ "vaadin-button[data-test-id='new-button'], "
	...
	void testStatusAndTypeViewsCRUDFunctionality() {
		...
		for (final Class<?> view : statusAndTypeViewClasses) {
			...
			try {
				...
				LOGGER.warn("⚠️ CRUD test failed for view {}: {}", view.getSimpleName(),
					e.getMessage());
				// Continue with other views
			}
		}
