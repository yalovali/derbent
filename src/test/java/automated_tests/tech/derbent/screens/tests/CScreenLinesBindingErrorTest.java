 ...
 * Description" in CScreenLinesEditDialog. Error: com.vaadin.flow.data.binder.BindingException: An exception has been thrown inside binding logic for
 * the field element [label='Entity Field Name'] */
		...
		if (getGridRowCount() > 0) {
			...
			if (addButtonFound) {
				...
				if (dialogOpened) {
					...
					boolean entityLineTypeExists = page.getByLabel("Entity Line Type").count() > 0 || page.getByText("Entity Line Type").count() > 0;
