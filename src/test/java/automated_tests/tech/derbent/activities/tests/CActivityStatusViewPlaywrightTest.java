...
/** CActivityStatusViewPlaywrightTest - Tests for activity status view focusing on lazy loading fixes and navigation behavior after save
 * operations. */
...
public class CActivityStatusViewPlaywrightTest extends CApplicationGeneric_UITest {
	...
	void testActivityStatusGridSelectionLazyLoading() {
		...
		try {
			...
			if (rowCount > 0) {
				...
				Check.isTrue(formFields.count() >= 0, "Form fields should be accessible after grid selection without lazy loading errors");
			} else {
	...
	void testActivityStatusLazyLoadingAndNavigation() {
		...
		try {
			...
			final var formFields = page.locator("vaadin-text-field, vaadin-text-area");
			Check.isTrue(formFields.count() > 0, "Form fields should be available when creating new activity status");
			...
			Check.isTrue(finalRowCount > initialRowCount, "Grid should have more rows after saving new activity status");
