...
public class CBaseUITest {
	...
	protected Class<?>[] statusAndTypeViewClasses = {
			CActivityStatusView.class, CActivityTypeView.class, CDecisionStatusView.class, CDecisionTypeView.class, CMeetingStatusView.class,
			CMeetingTypeView.class, CUserTypeView.class, CRiskStatusView.class, CCommentPriorityView.class
	};
	...
	protected Class<?>[] kanbanViewClasses = {
			CActivityKanbanBoardView.class, CMeetingKanbanBoardView.class, CGenericActivityKanbanBoardView.class
	};
	// Example and demo views
	protected Class<?>[] exampleViewClasses = {
			CExampleHierarchicalMenuView.class, CExampleSettingsView.class, CSearchDemoView.class, CSearchShowcaseView.class
	};
	// Detail views
	...
	protected void assertBrowserAvailable() {
		Check.isTrue(isBrowserAvailable(), "Browser is not available for testing - ensure Playwright setup completed successfully");
	}
	...
	protected void assertGridHasRows(final int expectedMinRows) {
		...
		Check.isTrue(actualRows >= expectedMinRows, "Grid should have at least " + expectedMinRows + " rows but has " + actualRows);
	}
	...
	protected void gridClickCell(final int cellIndex) {
		...
		try {
			final String selector = "vaadin-grid-cell-content";
			final var gridCells = page.locator(selector);
			Check.isTrue(cellIndex < (gridCells.count() - gridFirstCellIndex), "invalid grid cell index: " + cellIndex);
	...
	private boolean isBrowserAvailable() { return (page != null) && (playwright != null); }
	...
	protected void navigateToViewByClass(final Class<?> viewClass) {
		...
		Check.isTrue(viewLocator.count() > 0, "View element with selector '" + viewSelector + "' should exist after navigation");
	}
	...
	/** Attempts to perform logout using the specific menu item IDs from MainLayout with proper assertions for test validation */
	protected void performLogout() {
		...
		Check.isTrue(waitForElementById("logout-menu-item", 2000), "Logout menu item should be available after opening user menu");
		// Then click on the logout menu item
		clickById("logout-menu-item");
		wait_1000();
		Check.isTrue(waitForElementById("custom-username-input", 5000), "Should be redirected to login page after logout");
	...
	void setUp() {
		...
		try {
			...
			final BrowserType.LaunchOptions launchOptions = new BrowserType.LaunchOptions().setHeadless(false).setSlowMo(50);
	...
	protected void testCRUDOperationsInView(final String viewName, final String newButtonId, final String saveButtonId, final String deleteButtonId) {
		...
		if (elementExistsById(deleteButtonId)) {
			...
			final var confirmButtons =
					page.locator("vaadin-button:has-text('Yes'), vaadin-button:has-text('Confirm'), vaadin-button:has-text('Delete')");
	...
	protected void testSearchFunctionality(final Class<?> viewClass, final String searchTerm) {
		LOGGER.info("🧪 Testing search functionality in {} view with term: '{}'", viewClass.getSimpleName(), searchTerm);
		try {
			// Navigate to the view
			navigateToViewByClass(viewClass);
			...
			if (searchFields.count() > 0) {
				...
				Check.isTrue(filteredRowCount <= initialRowCount,
						"Search should filter results (filtered: " + filteredRowCount + ", initial: " + initialRowCount + ")");
				...
				LOGGER.warn("No search field found in {} view - entity may not implement CSearchable", viewClass.getSimpleName());
	...
	protected void wait_loginscreen() {
		...
		final ElementHandle handler = page.waitForSelector("#custom-username-input", new Page.WaitForSelectorOptions().setTimeout(10000));
