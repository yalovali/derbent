...
public class CBaseUITest {
    ...
    protected Class<?>[] mainViewClasses = { CProjectsView.class, CActivitiesView.class, CMeetingsView.class,
            CDecisionsView.class, CUsersView.class, COrdersView.class, CRiskView.class, CCompanyView.class,
            CScreenView.class };

    // Status and Type configuration views
    protected Class<?>[] statusAndTypeViewClasses = { CActivityStatusView.class, CActivityTypeView.class,
            CDecisionStatusView.class, CDecisionTypeView.class, CMeetingStatusView.class, CMeetingTypeView.class,
            CUserTypeView.class, CRiskStatusView.class, CCommentPriorityView.class };

    // Administrative and system configuration views
    protected Class<?>[] adminViewClasses = { CCompanySettingsView.class, CSystemSettingsView.class,
            CDashboardView.class };

    // Kanban board views
    protected Class<?>[] kanbanViewClasses = { CActivityKanbanBoardView.class, CMeetingKanbanBoardView.class,
            CGenericActivityKanbanBoardView.class };

    // Example and demo views
    protected Class<?>[] exampleViewClasses = { CExampleHierarchicalMenuView.class, CExampleSettingsView.class,
            CSearchDemoView.class, CSearchShowcaseView.class };

    // Detail views
    protected Class<?>[] detailViewClasses = { CProjectDetailsView.class };

    // All views combined for comprehensive testing
    protected Class<?>[] allViewClasses = { CProjectsView.class, CActivitiesView.class, CMeetingsView.class,
            CDecisionsView.class, CUsersView.class, COrdersView.class, CRiskView.class, CCompanyView.class,
            CScreenView.class, CActivityStatusView.class, CActivityTypeView.class, CDecisionStatusView.class,
            CDecisionTypeView.class, CMeetingStatusView.class, CMeetingTypeView.class, CUserTypeView.class,
            CRiskStatusView.class, CCommentPriorityView.class, CCompanySettingsView.class, CSystemSettingsView.class,
            CDashboardView.class, CActivityKanbanBoardView.class, CMeetingKanbanBoardView.class,
            CGenericActivityKanbanBoardView.class, CExampleHierarchicalMenuView.class, CExampleSettingsView.class,
    ...
    protected void assertBrowserAvailable() {
        Check.isTrue(isBrowserAvailable(),
    ...
    protected void assertElementExistsById(final String id) {
        Check.isTrue(elementExistsById(id), "Element with ID '" + id + "' should exist but was not found");
    }
    ...
    protected void assertElementNotExistsById(final String id) {
        Check.isTrue(!elementExistsById(id), "Element with ID '" + id + "' should not exist but was found");
    }
    ...
    protected void assertGridHasRows(final int expectedMinRows) {
        ...
        Check.isTrue(actualRows >= expectedMinRows,
                "Grid should have at least " + expectedMinRows + " rows but has " + actualRows);
    }
    ...
    protected void assertTextById(final String id, final String expectedText) {
        ...
        Check.notNull(actualText, "Element with ID '" + id + "' should have text content");
        Check.equals(expectedText, actualText, "Text content mismatch for element with ID '" + id + "'");
    }
    ...
    protected void clickById(final String id, final int timeoutMs) {
        ...
        Check.isTrue(locator.count() > 0, "Element with ID '" + id + "' should exist before clicking");
    ...
    protected void clickNew() {
        ...
        Check.isTrue(newButtons.count() > 0, "New/Add button should exist before clicking");
    ...
    protected void clickSaveButton() {
        ...
        Check.isTrue(saveButtons.count() > 0, "Save button should exist before clicking");
    ...
    protected void fillById(final String id, final String value) {
        ...
        Check.isTrue(locator.count() > 0, "Element with ID '" + id + "' should exist before filling");
        // Fill the field
    ...
    protected int getGridFirstCell() {
        ...
        Check.isTrue(grids.count() > 0, "No grids found in view - expected at least one grid");
        ...
        Check.isTrue(gridCellCount >= 1, "No grid cells found - expected at least one cell");
        ...
        Check.isTrue(firstDataCell >= 0, "No data cells found after empty cells");
    ...
    protected int getGridRowCount() {
        ...
        Check.isTrue(columns > 0, "No columns detected in grid");
        ...
        Check.isTrue(rowCount >= 0, "Invalid row count calculated: " + rowCount);
        return rowCount;
    }
    ...
    protected String getTextById(final String id) {
        ...
        Check.isTrue(item.count() > 0, "Element with ID '" + id + "' should exist before getting text");
    ...
    protected void gridClickCell(final int cellIndex) {
        ...
        try {
            final String selector = "vaadin-grid-cell-content";
            final var gridCells = page.locator(selector);
            Check.isTrue(cellIndex < (gridCells.count() - gridFirstCellIndex),
    ...
    protected void navigateToViewByClass(final Class<?> viewClass) {
        ...
        Check.isTrue(routeAnnotation != null, "Class " + viewClass.getName() + " must have a @Route annotation");
        final String route = routeAnnotation.value().split("/")[0];
        Check.isTrue(!route.isEmpty(), "Route value must not be empty for class: " + viewClass.getName());
        ...
        Check.isTrue(viewLocator.count() > 0,
                "View element with selector '" + viewSelector + "' should exist after navigation");
    }

    /**
     * Performs login with specified credentials Updated for CCustomLoginView (new login screen)
     */
    protected void performLogin(final String username, final String password) {
        ...
        Check.isTrue(!(username.trim().isEmpty()), "Username cannot be empty");
        Check.isTrue(!(password.trim().isEmpty()), "Password cannot be empty");
    ...
    /**
     * Attempts to perform logout using the specific menu item IDs from MainLayout with proper assertions for test
     * validation
     */
    ...
    void setUp() {
        ...
        try {
            ...
            if (Paths.get("/usr/bin/google-chrome").toFile().exists()) {
                ...
                LOGGER.info("Using system Google Chrome browser at: /usr/bin/google-chrome");
        ...
        try {
            ...
            if (cellCount > 0) {
                ...
                for (int i = 0; i < Math.min(cellCount, 20); i++) { // Check first 20
                    ...
                    if ((cellText != null) && !cellText.trim().isEmpty() && !cellText.matches("\\d+") && // Not just numbers (IDs)
            ...
            LOGGER.error("Entity relation grid test failed for {}: {}", viewClass.getSimpleName(), e.getMessage());
    ...
    protected void testSearchFunctionality(final Class<?> viewClass, final String searchTerm) {
        LOGGER.info("🧪 Testing search functionality in {} view with term: '{}'", viewClass.getSimpleName(),
                searchTerm);
        ...
        try {
            ...
            final var searchFields = page
                    .locator("vaadin-text-field[placeholder*='earch'], .search-toolbar vaadin-text-field");
            ...
            if (searchFields.count() > 0) {
                ...
                Check.isTrue(filteredRowCount <= initialRowCount, "Search should filter results (filtered: "
                        + filteredRowCount + ", initial: " + initialRowCount + ")");
                ...
                Check.isTrue(clearedRowCount >= filteredRowCount, "Clearing search should restore results");
            ...
            LOGGER.error("Search functionality test failed for {}: {}", viewClass.getSimpleName(), e.getMessage());
    ...
    protected void wait_afterlogin() {
        ...
        if (isBrowserAvailable()) {
            wait_500();
            page.waitForSelector("#main-layout", new Page.WaitForSelectorOptions().setTimeout(10000));
            LOGGER.debug("Application layout found after login");
            return;
        }
        throw new RuntimeException("Login failed - application layout not found after login");
    }
