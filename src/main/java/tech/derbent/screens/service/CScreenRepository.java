...
public interface CScreenRepository extends CEntityOfProjectRepository<CScreen> {
    ...
    @Query(
        ...
        "LEFT JOIN FETCH s.relatedActivity " +
        "LEFT JOIN FETCH s.relatedMeeting " +
        "LEFT JOIN FETCH s.relatedRisk " +
    ...
    List<CScreen> findByProjectAndEntityType(@Param("project") CProject project, @Param("entityType") String entityType);

    @Query(
        "SELECT s FROM CScreen s " +
        "WHERE s.project = :project AND s.isActive = true"
    )
