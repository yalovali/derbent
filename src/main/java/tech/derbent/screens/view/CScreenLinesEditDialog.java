...
public class CScreenLinesEditDialog extends CDBEditDialog<CScreenLines> {
    ...
    private void createFormFields() {
        ...
        try {
            ...
            getDialogLayout().add(formClassType.build(CScreenLines.class, binder, List.of("fieldClass")));
            // add tab here
            getDialogLayout().add(tabsOfDialog);
            // BUILD ENTITY TAB
            tabEntitySpan.add(formEntity.build(CScreenLines.class, binder,
                    List.of("entityProperty", "lineOrder", "fieldCaption", "fieldDescription", "isRequired",
                            "isReadonly", "isHidden", "defaultValue", "relatedEntityType", "dataProviderBean",
                            "maxLength", "isActive")));
            // BUILD SECTION TAB
            tabSectionSpan.add(formSection.build(CScreenLines.class, binder, List.of("sectionName", "isActive")));
    ...
    private void updateEntityPropertyBasedOnClass() {
        ...
        if (fieldClass.equals(CEntityFieldService.SECTION)) {
            ...
            fieldProperties = entityFieldService.getEntitySimpleFields(screen.getEntityType(), null);
        } else {
            ...
            final EntityFieldInfo info = entityFieldService.getEntityFieldInfo(screen.getEntityType().toString(),
                    fieldClass);
            Check.notNull(info, "Field info must not be null for field class: " + fieldClass);
