...
public class CScreenLinesEditDialog extends CDBEditDialog<CScreenLines> {
    ...
    public CScreenLinesEditDialog(final CScreenLines screenLine, final Consumer<CScreenLines> onSave,
            final boolean isNew, final CEntityFieldService entityFieldService, final CScreen screen,
    ...
    private void createEntityFieldNameComboBox(final VerticalLayout layout) {
        ...
        entityFieldNameComboBox.setHelperText("Select the field from the chosen entity type");
    ...
    private void createEntityLineTypeComboBox(final VerticalLayout layout) {
        ...
        entityLineTypeComboBox.setHelperText("Select the entity type: the class itself or a related entity");
        ...
        if ((screen != null) && (screen.getEntityType() != null)) {
            final List<String> entityLineTypes = viewsService.getAvailableEntityLineTypes(screen.getEntityType());
        ...
        // Bind to entity
        binder.forField(entityLineTypeComboBox).bind(CScreenLines::getEntityLineType, CScreenLines::setEntityLineType);
    ...
    private void updateEntityFieldNameOptions(final String entityLineType) {
        ...
        final List<String> fieldNames = fieldInfos.stream().filter(info -> !info.isHidden()) // Only show non-hidden
                // fields
    ...
    private void updateFieldTypeBasedOnSelection() {
        ...
        final List<CEntityFieldService.EntityFieldInfo> fieldInfos = entityFieldService
                .getEntityFields(entityClassName);
        fieldInfos.stream().filter(info -> info.getFieldName().equals(fieldName)).findFirst().ifPresent(fieldInfo -> {
