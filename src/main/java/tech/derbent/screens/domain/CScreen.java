...
/**
 * CScreen - Domain entity representing screen views for entities. Layer: Domain (MVC) Inherits from CEntityOfProject to
 * provide project association. This entity allows creating custom view definitions for various project entities.
 */
@Entity
@Table(name = "cscreen")
...
public class CScreen extends CEntityOfProject<CScreen> {
    ...
    @MetaData(displayName = "Entity Type", required = true, readOnly = false, description = "Type of entity this screen is designed for", hidden = false, order = 2, maxLength = 100, dataProviderBean = "CViewsService", dataProviderMethod = "getAvailableBaseTypes")
    private String entityType;

    @Column(name = "screen_title", nullable = true, length = 255)
    @Size(max = 255, message = "Screen title cannot exceed 255 characters")
    @MetaData(displayName = "Screen Title", required = false, readOnly = false, description = "Title to display for this screen view", hidden = false, order = 3, maxLength = 255)
    private String screenTitle;

    @Column(name = "header_text", nullable = true, length = 500)
    @Size(max = 500, message = "Header text cannot exceed 500 characters")
    @MetaData(displayName = "Header Text", required = false, readOnly = false, description = "Header text to display at the top of the screen", hidden = false, order = 4, maxLength = 500)
    private String headerText;

    // Relations to project entities - only one can be set at a time
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "related_activity_id", nullable = true)
    @MetaData(displayName = "Related Activity", required = false, readOnly = false, description = "Activity this screen is related to", hidden = false, order = 10, dataProviderBean = "CActivityService")
    private CActivity relatedActivity;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "related_meeting_id", nullable = true)
    @MetaData(displayName = "Related Meeting", required = false, readOnly = false, description = "Meeting this screen is related to", hidden = false, order = 11, dataProviderBean = "CMeetingService")
    private CMeeting relatedMeeting;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "related_risk_id", nullable = true)
    @MetaData(displayName = "Related Risk", required = false, readOnly = false, description = "Risk this screen is related to", hidden = false, order = 12, dataProviderBean = "CRiskService")
    ...
    @MetaData(displayName = "Active", required = false, readOnly = false, description = "Whether this screen definition is active", hidden = false, order = 20, defaultValue = "true")
    ...
    public String toString() {
        return String.format("CScreen{id=%d, name='%s', entityType='%s', screenTitle='%s'}", getId(), getName(),
                entityType, screenTitle);
    }
}