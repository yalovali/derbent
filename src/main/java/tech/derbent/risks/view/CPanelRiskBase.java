...
/**
 * CPanelRiskBase - Abstract base class for all CRisk-related accordion panels.
 * Layer: View (MVC)
 * Provides common functionality for risk entity panels following the same pattern as CPanelActivityBase.
 */
public abstract class CPanelRiskBase extends CAccordionDescription<CRisk> {
	...
	protected void createPanelContent() {
		...
		getBaseLayout().add(CEntityFormBuilder.buildForm(CRisk.class, getBinder(),
			getEntityFields()));
	}
}