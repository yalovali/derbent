 ...
 * Layer: Service (MVC)
 * Provides business logic for activity priority management including CRUD operations,
 * validation, and priority level management.
 */
@Service
@Transactional
public class CActivityPriorityService {

    private static final Logger logger = LoggerFactory.getLogger(CActivityPriorityService.class);

    private final CActivityPriorityRepository activityPriorityRepository;

    @Autowired
    public CActivityPriorityService(final CActivityPriorityRepository activityPriorityRepository) {
        logger.debug("CActivityPriorityService(activityPriorityRepository={}) - Initializing service", 
            activityPriorityRepository);
        if (activityPriorityRepository == null) {
            logger.error("CActivityPriorityService constructor - Repository parameter is null");
            throw new IllegalArgumentException("Activity priority repository cannot be null");
        }
    ...
    public List<CActivityPriority> findAll() {
        ...
        final List<CActivityPriority> priorities = activityPriorityRepository.findAllOrderedByPriorityLevel();
    ...
    public Optional<CActivityPriority> findById(final Long id) {
        ...
        final Optional<CActivityPriority> priority = activityPriorityRepository.findById(id);
    ...
    public Optional<CActivityPriority> findByName(final String name) {
        ...
        final Optional<CActivityPriority> priority = activityPriorityRepository.findByNameIgnoreCase(name.trim());
        logger.debug("findByName(name={}) - Found priority: {}", name, priority.isPresent());
        return priority;
    }
    ...
    public Optional<CActivityPriority> findByPriorityLevel(final Integer priorityLevel) {
        logger.debug("findByPriorityLevel(priorityLevel={}) - Finding activity priority by level", priorityLevel);
        if (priorityLevel == null) {
            logger.warn("findByPriorityLevel(priorityLevel=null) - Priority level parameter is null");
            return Optional.empty();
        }
        final Optional<CActivityPriority> priority = activityPriorityRepository.findByPriorityLevel(priorityLevel);
        logger.debug("findByPriorityLevel(priorityLevel={}) - Found priority: {}", priorityLevel, priority.isPresent());
        return priority;
    }
    ...
    public List<CActivityPriority> findAllHighPriorities() {
        ...
        final List<CActivityPriority> priorities = activityPriorityRepository.findAllHighPriorities();
        logger.debug("findAllHighPriorities() - Found {} high priorities", priorities.size());
        return priorities;
    }
    ...
    public List<CActivityPriority> findAllLowPriorities() {
        ...
        final List<CActivityPriority> priorities = activityPriorityRepository.findAllLowPriorities();
        logger.debug("findAllLowPriorities() - Found {} low priorities", priorities.size());
        return priorities;
    }
    ...
    public Optional<CActivityPriority> findDefaultPriority() {
        ...
        final Optional<CActivityPriority> priority = activityPriorityRepository.findDefaultPriority();
        logger.debug("findDefaultPriority() - Found default priority: {}", priority.isPresent());
        return priority;
    }
    ...
    public CActivityPriority save(final CActivityPriority activityPriority) {
        ...
        if (activityPriority == null) {
            logger.error("save(activityPriority=null) - Activity priority parameter is null");
            throw new IllegalArgumentException("Activity priority cannot be null");
        }

        if (activityPriority.getName() == null || activityPriority.getName().trim().isEmpty()) {
            logger.error("save() - Activity priority name is null or empty for priority id={}", 
                activityPriority.getId());
            throw new IllegalArgumentException("Activity priority name cannot be null or empty");
        }

        if (activityPriority.getPriorityLevel() == null || activityPriority.getPriorityLevel() < 1 
        ...
        final Optional<CActivityPriority> existingByName = activityPriorityRepository.findByNameIgnoreCase(trimmedName);
        if (existingByName.isPresent() && !existingByName.get().getId().equals(activityPriority.getId())) {
            logger.error("save() - Activity priority name '{}' already exists", trimmedName);
            throw new IllegalArgumentException("Activity priority name '" + trimmedName + "' already exists");
        }
        ...
        if (existingByLevel.isPresent() && !existingByLevel.get().getId().equals(activityPriority.getId())) {
            logger.error("save() - Priority level {} already exists", activityPriority.getPriorityLevel());
            throw new IllegalArgumentException("Priority level " + activityPriority.getPriorityLevel() + " already exists");
        }

        try {
            final CActivityPriority savedPriority = activityPriorityRepository.save(activityPriority);
            logger.debug("save() - Successfully saved activity priority with id={}", savedPriority.getId());
            return savedPriority;
        } catch (final Exception e) {
            logger.error("save() - Error saving activity priority: {}", e.getMessage(), e);
    ...
    public void deleteById(final Long id) {
        ...
        final Optional<CActivityPriority> existing = activityPriorityRepository.findById(id);
        ...
        try {
            ...
            logger.debug("deleteById(id={}) - Successfully deleted activity priority", id);
        } catch (final Exception e) {
            logger.error("deleteById(id={}) - Error deleting activity priority: {}", id, e.getMessage(), e);
    ...
    public boolean existsByName(final String name) {
        logger.debug("existsByName(name={}) - Checking if activity priority name exists", name);
        if (name == null || name.trim().isEmpty()) {
            logger.warn("existsByName(name={}) - Name parameter is null or empty", name);
            return false;
        }
        final boolean exists = activityPriorityRepository.existsByNameIgnoreCase(name.trim());
    ...
    public boolean existsByPriorityLevel(final Integer priorityLevel) {
        logger.debug("existsByPriorityLevel(priorityLevel={}) - Checking if priority level exists", priorityLevel);
        if (priorityLevel == null) {
            logger.warn("existsByPriorityLevel(priorityLevel=null) - Priority level parameter is null");
            return false;
        }
        final boolean exists = activityPriorityRepository.existsByPriorityLevel(priorityLevel);
        logger.debug("existsByPriorityLevel(priorityLevel={}) - Level exists: {}", priorityLevel, exists);
        return exists;
    }

    /**
     * Create default activity priorities if they don't exist.
     * This method should be called during application startup.
     */
    public void createDefaultPrioritiesIfNotExist() {
        logger.debug("createDefaultPrioritiesIfNotExist() - Creating default activity priorities");
        
        final String[][] defaultPriorities = {
            {"CRITICAL", "1", "Critical priority - immediate attention required", "#FF0000", "false"},
        ...
        for (final String[] priorityData : defaultPriorities) {
            ...
            if (!existsByName(name) && !existsByPriorityLevel(level)) {
                logger.debug("createDefaultPrioritiesIfNotExist() - Creating default priority: {}", name);
                final CActivityPriority priority = new CActivityPriority(name, level, priorityData[2], 
                    priorityData[3], Boolean.parseBoolean(priorityData[4]));
                save(priority);
            } else {
                logger.debug("createDefaultPrioritiesIfNotExist() - Priority '{}' or level {} already exists", 
                    name, level);
            }
        }
        logger.debug("createDefaultPrioritiesIfNotExist() - Completed creating default activity priorities");
    }
}