 ...
 * Layer: Service (MVC)
 * Provides business logic for activity status management including CRUD operations,
 * validation, and workflow management.
 */
@Service
@Transactional
public class CActivityStatusService {
    ...
    @Autowired
    public CActivityStatusService(final CActivityStatusRepository activityStatusRepository) {
        logger.debug("CActivityStatusService(activityStatusRepository={}) - Initializing service", 
    ...
    public CActivityStatus save(final CActivityStatus activityStatus) {
        ...
        if (activityStatus.getName() == null || activityStatus.getName().trim().isEmpty()) {
            logger.error("save() - Activity status name is null or empty for status id={}", 
        ...
        final Optional<CActivityStatus> existing = activityStatusRepository.findByNameIgnoreCase(trimmedName);
        if (existing.isPresent() && !existing.get().getId().equals(activityStatus.getId())) {
        ...
        try {
            final CActivityStatus savedStatus = activityStatusRepository.save(activityStatus);
            logger.debug("save() - Successfully saved activity status with id={}", savedStatus.getId());
            return savedStatus;
    ...
    /**
     * Create default activity statuses if they don't exist.
    ...
    public void createDefaultStatusesIfNotExist() {
        ...
        for (final String[] statusData : defaultStatuses) {
            ...
            if (!existsByName(name)) {
                logger.debug("createDefaultStatusesIfNotExist() - Creating default status: {}", name);
                ...
                logger.debug("createDefaultStatusesIfNotExist() - Status '{}' already exists", name);
            }
        }
