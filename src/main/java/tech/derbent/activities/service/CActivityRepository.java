...
public interface CActivityRepository extends CEntityOfProjectRepository<CActivity> {
    ...
    /**
     * Finds an activity by ID with eagerly loaded CActivityType and CActivityStatus to prevent
     ...
     */
    @Query("SELECT a FROM CActivity a LEFT JOIN FETCH a.activityType LEFT JOIN FETCH a.status WHERE a.id = :id")
    Optional<CActivity> findByIdWithActivityTypeAndStatus(@Param("id") Long id);

    /**
     * Finds an activity by ID with eagerly loaded CActivityType, CActivityStatus, and CProject to prevent
     * LazyInitializationException. This extends the base method with activity-specific relationships.
     * 
     * @param id
     *            the activity ID
    ...
    /**
     * Finds all activities by project with eagerly loaded CActivityType, CActivityStatus, and CProject to prevent
     * LazyInitializationException. This method returns all activities without pagination with activity-specific
     * relationships.
     * 
     * @param project
     *            the project
    ...
    /**
     * Finds activities by project with eagerly loaded CActivityType, CActivityStatus, and CProject to prevent
     * LazyInitializationException.
     * 
     * @param project
     *            the project
    ...
    @Query("SELECT a FROM CActivity a " + "LEFT JOIN FETCH a.project " + "LEFT JOIN FETCH a.assignedTo "
            + "LEFT JOIN FETCH a.createdBy " + "LEFT JOIN FETCH a.activityType " + "LEFT JOIN FETCH a.status "
            + "LEFT JOIN FETCH a.parentActivity " + "WHERE a.project = :project")
    Page<CActivity> findByProjectWithAllRelationships(@Param("project") CProject project, Pageable pageable);
}
