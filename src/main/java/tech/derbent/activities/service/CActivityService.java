...
public class CActivityService extends CEntityOfProjectService<CActivity> implements CKanbanService<CActivity, CActivityStatus> {
    ...
    /**
     * Gets all activities for a project grouped by activity status. Activities without a status are grouped under a "No
     * Status" key.
     * 
     * @param project
     *            the project to get activities for
    ...
    public Map<CActivityStatus, List<CActivity>> getActivitiesGroupedByStatus(final CProject project) {
        ...
        final List<CActivity> activities = activityRepository.findByProjectWithAllRelationships(project);
    ...
    /**
     * Gets all activities for a project grouped by activity type. Activities without a type are grouped under a "No
     * Type" key.
     * 
     * @param project
     *            the project to get activities for
    ...
    public Map<CActivityType, List<CActivity>> getActivitiesGroupedByType(final CProject project) {
        ...
        return activities.stream()
                .collect(Collectors.groupingBy(activity -> activity.getActivityType() != null
                        ? activity.getActivityType()
                        : createNoTypeInstance(project), Collectors.toList()));
    }
    ...
    /**
     * Gets an activity by ID with eagerly loaded CActivityType and CActivityStatus relationships. This method should be
     * used in UI contexts to prevent LazyInitializationException.
     * 
     * @param id
     *            the activity ID
     ...
     * lazy-loaded CActivityType and CActivityStatus relationships.
     * 
     * @param entity
    ...
    /**
     * Auxiliary method to set activity type and basic properties for an activity. Following coding guidelines to use
     * service layer methods instead of direct field setting.
     * 
     * @param activity
     ...
     */
    @Transactional
    public CActivity setActivityType(final CActivity activity, final CActivityType activityType,
            final String description) {
        LOGGER.info("setActivityType called for activity: {} with type: {}",
                activity != null ? activity.getName() : "null", activityType != null ? activityType.getName() : "null");
     ...
     */
    @Transactional
    public CActivity setAdditionalInfo(final CActivity activity, final String acceptanceCriteria, final String notes,
    ...
    public CActivity setAssignedUsers(final CActivity activity, final CUser assignedTo, final CUser createdBy) {
        LOGGER.info("setAssignedUsers called for activity: {} with assignedTo: {}, createdBy: {}",
                activity != null ? activity.getName() : "null", assignedTo != null ? assignedTo.getName() : "null",
     ...
     */
    @Transactional
    public CActivity setDateInfo(final CActivity activity, final LocalDate startDate, final LocalDate dueDate,
    ...
    /**
     * Auxiliary method to set comprehensive activity information in one call. This demonstrates how auxiliary methods
     * can be combined for complex setup scenarios.
     * 
     * @param activity
     *            the activity to configure
     * @param activityType
    ...
    public CActivity setStatusAndPriority(final CActivity activity, final CActivityStatus status,
        ...
        LOGGER.info("setStatusAndPriority called for activity: {} with status: {}, priority: {}, progress: {}%",
                activity != null ? activity.getName() : "null", status != null ? status.getName() : "null",
