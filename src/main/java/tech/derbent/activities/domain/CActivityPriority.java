...
/**
 * CActivityPriority - Domain entity representing activity priority levels. 
 * Layer: Domain (MVC) 
 * Inherits from CTypeEntity to provide type functionality for activity priorities.
 * Supports priority-based task management with levels like LOW, MEDIUM, HIGH, CRITICAL, URGENT.
 */
@Entity
...
public class CActivityPriority extends CTypeEntity {
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "", 
        description = "Name of the activity priority (e.g., LOW, MEDIUM, HIGH, CRITICAL)", 
        hidden = false,
        order = 1, 
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "", 
        description = "Description of the activity priority level", 
        hidden = false,
        order = 2, 
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "3", 
        description = "Numeric priority level (1=Highest, 5=Lowest)", 
        hidden = false,
        order = 3
    )
    ...
    @MetaData(
        displayName = "Is Default Priority", 
        required = true, 
        readOnly = false,
        defaultValue = "false", 
     ...
     * @param priorityLevel the numeric priority level (1=Highest, 5=Lowest) - must not be null
     */
    public CActivityPriority(final String name, final Integer priorityLevel) {
        super();
        logger.debug("CActivityPriority(name={}, priorityLevel={}) - Creating activity priority", name, priorityLevel);
        if (name == null) {
            logger.warn("CActivityPriority constructor - Name parameter is null");
        }
        if (priorityLevel == null) {
            logger.warn("CActivityPriority constructor - Priority level parameter is null");
        }
        this.name = name;
     ...
     */
    public CActivityPriority(final String name, final Integer priorityLevel, final String description) {
        super();
        logger.debug("CActivityPriority(name={}, priorityLevel={}, description={}) - Creating activity priority with description", 
     ...
     */
    public CActivityPriority(final String name, final Integer priorityLevel, final String description, 
                           final String color, final boolean isDefault) {
        super();
        logger.debug("CActivityPriority(name={}, priorityLevel={}, description={}, color={}, isDefault={}) - Creating full activity priority", 
    ...
    public String getColor() {
        return color != null ? color : "#FFA500";
    }
    ...
    public void setColor(final String color) {
        logger.debug("setColor(color={}) - Setting color for priority id={}", color, getId());
        this.color = color != null ? color : "#FFA500";
    }
    ...
    public void setDefault(final boolean isDefault) {
        logger.debug("setDefault(isDefault={}) - Setting default flag for priority id={}", isDefault, getId());
        this.isDefault = isDefault;
    }

    /**
     * Sets the description for this priority.
     * @param description the description text - can be null
     */
    public void setDescription(final String description) {
        logger.debug("setDescription(description={}) - Setting description for priority id={}", description, getId());
        this.description = description;
    }

    /**
     * Sets the name for this priority.
     * @param name the priority name - should not be null
     */
    public void setName(final String name) {
        logger.debug("setName(name={}) - Setting name for priority id={}", name, getId());
        if (name == null) {
            logger.warn("setName(name=null) - Attempting to set null name for priority id={}", getId());
        }
        this.name = name;
    }
     ...
     * @param priorityLevel the priority level (1=Highest, 5=Lowest) - if null, defaults to 3
     */
    public void setPriorityLevel(final Integer priorityLevel) {
        logger.debug("setPriorityLevel(priorityLevel={}) - Setting priority level for priority id={}", 
            priorityLevel, getId());
        if (priorityLevel == null) {
            logger.warn("setPriorityLevel(priorityLevel=null) - Attempting to set null priority level for priority id={}", 
                getId());
        }
        this.priorityLevel = priorityLevel != null ? priorityLevel : 3;
    }
