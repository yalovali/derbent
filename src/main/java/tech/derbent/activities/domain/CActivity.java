...
/**
 * CActivity - Enhanced domain entity representing project activities with comprehensive management features.
 * Layer: Domain (MVC)
 * 
 * Supports:
 * - Resource management (user assignments, time tracking)
 * - Task tracking (status, priority, progress)
 * - Project management (milestones, dependencies, deliverables)
 * - Budget planning (cost estimation, actual vs planned tracking)
 * 
 * Inspired by Jira and ProjeQtOr functionality for professional project management.
 */
@Entity
...
public class CActivity extends CEntityOfProject {
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "0.00",
        description = "Estimated time in hours to complete this activity",
        hidden = false,
        order = 20
    )
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "0.00",
        description = "Actual time spent on this activity in hours",
        hidden = false,
        order = 21
    )
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "0.00",
        description = "Estimated remaining time in hours",
        hidden = false,
        order = 22
    )
    ...
    @MetaData(
        ...
        readOnly = false,
        defaultValue = "0.00",
        description = "Estimated cost to complete this activity",
        hidden = false,
        order = 50
    )
    ...
    @MetaData(
        ...
        required = false,
        readOnly = false,
        description = "Parent activity for hierarchical task breakdown",
        hidden = false,
        order = 60,
    ...
    public CActivity(final String name, final CProject project) {
        super(name, project);
        logger.debug("CActivity(name={}, project={}) - Creating activity with name and project", name, 
    ...
    public CActivity(final String name, final CProject project, final CUser assignedTo) {
        super(name, project);
        logger.debug("CActivity(name={}, project={}, assignedTo={}) - Creating activity with assignment", 
    ...
    public BigDecimal calculateCostVariance() {
        logger.debug("calculateCostVariance() - Calculating cost variance for activity id={}", getId());
        if (actualCost == null || estimatedCost == null) {
            logger.debug("calculateCostVariance() - Missing cost data, actual={}, estimated={}", 
    ...
    public BigDecimal calculateTimeVariance() {
        logger.debug("calculateTimeVariance() - Calculating time variance for activity id={}", getId());
        if (actualHours == null || estimatedHours == null) {
            logger.debug("calculateTimeVariance() - Missing time data, actual={}, estimated={}", 
    ...
    public void updateLastModified() {
        logger.debug("updateLastModified() - Updating last modified date for activity id={}", getId());
        this.lastModifiedDate = LocalDateTime.now();
    }
    ...
    public CActivityPriority getPriority() {
        return priority;
    }
    ...
    public CActivityStatus getStatus() {
        return status;
    }

    public void setAcceptanceCriteria(final String acceptanceCriteria) {
        logger.debug("setAcceptanceCriteria(acceptanceCriteria={}) - Setting acceptance criteria for activity id={}", 
    ...
    public void setActualCost(final BigDecimal actualCost) {
        logger.debug("setActualCost(actualCost={}) - Setting actual cost for activity id={}", actualCost, getId());
        if (actualCost != null && actualCost.compareTo(BigDecimal.ZERO) < 0) {
            logger.warn("setActualCost - Attempting to set negative actual cost: {} for activity id={}", 
                actualCost, getId());
        }
    ...
    public void setActualHours(final BigDecimal actualHours) {
        logger.debug("setActualHours(actualHours={}) - Setting actual hours for activity id={}", actualHours, getId());
        if (actualHours != null && actualHours.compareTo(BigDecimal.ZERO) < 0) {
            logger.warn("setActualHours - Attempting to set negative actual hours: {} for activity id={}", 
                actualHours, getId());
        }
    ...
    public void setActivityType(final CActivityType activityType) {
        logger.debug("setActivityType(activityType={}) - Setting activity type for activity id={}", 
    ...
    public void setAssignedTo(final CUser assignedTo) {
        logger.debug("setAssignedTo(assignedTo={}) - Setting assigned user for activity id={}", 
    ...
    public void setCompletionDate(final LocalDate completionDate) {
        logger.debug("setCompletionDate(completionDate={}) - Setting completion date for activity id={}", 
            completionDate, getId());
        this.completionDate = completionDate;
        if (completionDate != null && progressPercentage != null && progressPercentage < 100) {
            logger.debug("setCompletionDate - Auto-setting progress to 100% for completed activity id={}", getId());
    ...
    public void setCreatedBy(final CUser createdBy) {
        logger.debug("setCreatedBy(createdBy={}) - Setting created by user for activity id={}", 
            createdBy != null ? createdBy.getUsername() : "null", getId());
        this.createdBy = createdBy;
    }

    public void setCreatedDate(final LocalDateTime createdDate) {
        logger.debug("setCreatedDate(createdDate={}) - Setting created date for activity id={}", 
            createdDate, getId());
        this.createdDate = createdDate;
    }

    public void setDescription(final String description) {
        logger.debug("setDescription(description={}) - Setting description for activity id={}", 
            description, getId());
    ...
    public void setDueDate(final LocalDate dueDate) {
        logger.debug("setDueDate(dueDate={}) - Setting due date for activity id={}", dueDate, getId());
        this.dueDate = dueDate;
        updateLastModified();
    }

    public void setEstimatedCost(final BigDecimal estimatedCost) {
        logger.debug("setEstimatedCost(estimatedCost={}) - Setting estimated cost for activity id={}", 
            estimatedCost, getId());
        if (estimatedCost != null && estimatedCost.compareTo(BigDecimal.ZERO) < 0) {
            logger.warn("setEstimatedCost - Attempting to set negative estimated cost: {} for activity id={}", 
                estimatedCost, getId());
        }
    ...
    public void setEstimatedHours(final BigDecimal estimatedHours) {
        logger.debug("setEstimatedHours(estimatedHours={}) - Setting estimated hours for activity id={}", 
    ...
    public void setHourlyRate(final BigDecimal hourlyRate) {
        logger.debug("setHourlyRate(hourlyRate={}) - Setting hourly rate for activity id={}", hourlyRate, getId());
        if (hourlyRate != null && hourlyRate.compareTo(BigDecimal.ZERO) < 0) {
            logger.warn("setHourlyRate - Attempting to set negative hourly rate: {} for activity id={}", 
                hourlyRate, getId());
        }
    ...
    public void setLastModifiedDate(final LocalDateTime lastModifiedDate) {
        logger.debug("setLastModifiedDate(lastModifiedDate={}) - Setting last modified date for activity id={}", 
            lastModifiedDate, getId());
        this.lastModifiedDate = lastModifiedDate;
    }

    public void setNotes(final String notes) {
        logger.debug("setNotes(notes={}) - Setting notes for activity id={}", notes, getId());
        this.notes = notes;
        updateLastModified();
    }

    public void setParentActivity(final CActivity parentActivity) {
        logger.debug("setParentActivity(parentActivity={}) - Setting parent activity for activity id={}", 
    ...
    public void setProgressPercentage(final Integer progressPercentage) {
        logger.debug("setProgressPercentage(progressPercentage={}) - Setting progress percentage for activity id={}", 
            progressPercentage, getId());
        if (progressPercentage != null && (progressPercentage < 0 || progressPercentage > 100)) {
            logger.warn("setProgressPercentage - Invalid progress percentage: {} for activity id={}", 
                progressPercentage, getId());
            return;
        }
        ...
        if (progressPercentage != null && progressPercentage >= 100 && completionDate == null) {
            logger.debug("setProgressPercentage - Auto-setting completion date for 100% progress activity id={}", 
                getId());
    ...
    public void setRemainingHours(final BigDecimal remainingHours) {
        logger.debug("setRemainingHours(remainingHours={}) - Setting remaining hours for activity id={}", 
            remainingHours, getId());
        if (remainingHours != null && remainingHours.compareTo(BigDecimal.ZERO) < 0) {
            logger.warn("setRemainingHours - Attempting to set negative remaining hours: {} for activity id={}", 
                remainingHours, getId());
        }
    ...
    public void setStartDate(final LocalDate startDate) {
        logger.debug("setStartDate(startDate={}) - Setting start date for activity id={}", startDate, getId());
    ...
    public void setStatus(final CActivityStatus status) {
        ...
        if (status != null && status.isFinal() && completionDate == null) {
            logger.debug("setStatus - Auto-setting completion date for final status activity id={}", getId());
            this.completionDate = LocalDate.now();
