...
/**
 * CActivity - Enhanced domain entity representing project activities with comprehensive management features. Layer:
 * Domain (MVC) Supports: - Resource management (user assignments, time tracking) - Task tracking (status, priority,
 * progress) - Project management (milestones, dependencies, deliverables) - Budget planning (cost estimation, actual vs
 * planned tracking) Inspired by Jira and ProjeQtOr functionality for professional project management.
 */
@Entity
...
public class CActivity extends CEntityOfProject<CActivity> {
    ...
    @MetaData(displayName = "Activity Type", required = false, readOnly = false, description = "Type category of the activity", hidden = false, order = 2, dataProviderBean = "CActivityTypeService")
    ...
    @MetaData(displayName = "Estimated Hours", required = false, readOnly = false, defaultValue = "0.00", description = "Estimated time in hours to complete this activity", hidden = false, order = 20)
    ...
    @MetaData(displayName = "Actual Hours", required = false, readOnly = false, defaultValue = "0.00", description = "Actual time spent on this activity in hours", hidden = false, order = 21)
    ...
    @MetaData(displayName = "Remaining Hours", required = false, readOnly = false, defaultValue = "0.00", description = "Estimated remaining time in hours", hidden = false, order = 22)
    ...
    @MetaData(displayName = "Priority", required = false, readOnly = false, description = "Priority level of the activity", hidden = false, order = 31, dataProviderBean = "CActivityPriorityService")
    ...
    @MetaData(displayName = "Progress %", required = false, readOnly = false, defaultValue = "0", description = "Completion percentage (0-100)", hidden = false, order = 32)
    private Integer progressPercentage = 0;

    // Date Management
    @Column(name = "start_date", nullable = true)
    @MetaData(displayName = "Start Date", required = false, readOnly = false, description = "Planned or actual start date of the activity", hidden = false, order = 40)
    private LocalDate startDate;

    @Column(name = "due_date", nullable = true)
    @MetaData(displayName = "Due Date", required = false, readOnly = false, description = "Expected completion date", hidden = false, order = 41)
    private LocalDate dueDate;

    @Column(name = "completion_date", nullable = true)
    @MetaData(displayName = "Completion Date", required = false, readOnly = true, description = "Actual completion date", hidden = false, order = 42)
    ...
    @MetaData(displayName = "Estimated Cost", required = false, readOnly = false, defaultValue = "0.00", description = "Estimated cost to complete this activity", hidden = false, order = 50)
    ...
    @MetaData(displayName = "Actual Cost", required = false, readOnly = false, defaultValue = "0.00", description = "Actual cost spent on this activity", hidden = false, order = 51)
    ...
    @MetaData(displayName = "Hourly Rate", required = false, readOnly = false, defaultValue = "0.00", description = "Hourly rate for cost calculation", hidden = false, order = 52)
    private BigDecimal hourlyRate;

    // Hierarchical Structure Support
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "parent_activity_id", nullable = true)
    @MetaData(displayName = "Parent Activity", required = false, readOnly = false, description = "Parent activity for hierarchical task breakdown", hidden = false, order = 60, dataProviderBean = "CActivityService")
    private CActivity parentActivity;

    // Additional Information
    @Column(name = "acceptance_criteria", nullable = true, length = 2000)
    @Size(max = 2000)
    @MetaData(displayName = "Acceptance Criteria", required = false, readOnly = false, defaultValue = "", description = "Criteria that must be met for the activity to be considered complete", hidden = false, order = 70, maxLength = 2000)
     ...
     * @param project
     *            the project this activity belongs to - must not be null
     */
     ...
     *            the project this activity belongs to - must not be null
     * @param assignedTo
     *            the user assigned to this activity - can be null
     */
    ...
    public BigDecimal calculateCostVariance() {
        ...
        if ((actualCost == null) || (estimatedCost == null)) {
            LOGGER.debug("calculateCostVariance() - Missing cost data, actual={}, estimated={}", actualCost,
                    estimatedCost);
    ...
    public void setActualCost(final BigDecimal actualCost) {
        ...
        if ((actualCost != null) && (actualCost.compareTo(BigDecimal.ZERO) < 0)) {
            LOGGER.warn("setActualCost - Attempting to set negative actual cost: {} for activity id={}", actualCost,
                    getId());
        }
    ...
    public void setActualHours(final BigDecimal actualHours) {
        ...
        if ((actualHours != null) && (actualHours.compareTo(BigDecimal.ZERO) < 0)) {
            LOGGER.warn("setActualHours - Attempting to set negative actual hours: {} for activity id={}", actualHours,
                    getId());
        }
    ...
    public void setHourlyRate(final BigDecimal hourlyRate) {
        ...
        if ((hourlyRate != null) && (hourlyRate.compareTo(BigDecimal.ZERO) < 0)) {
            LOGGER.warn("setHourlyRate - Attempting to set negative hourly rate: {} for activity id={}", hourlyRate,
                    getId());
        }
