...
/**
 * CActivityStatusView - View for managing activity statuses.
 * Layer: View (MVC)
 * 
 * Provides CRUD operations for activity statuses using the abstract master-detail pattern.
 * Allows users to create, read, update, and delete activity status definitions.
 */
@Route("activity-statuses/:activity_status_id?/:action?(edit)")
@PageTitle("Activity Statuses")
@Menu(order = 2, icon = "vaadin:flag", title = "Activity.Activity Statuses")
@PermitAll
public class CActivityStatusView extends CAbstractMDPage<CActivityStatus> {
    ...
    /**
     * Creates the details layout for editing activity status entities.
     * Uses CEntityFormBuilder to automatically generate form fields based on MetaData annotations.
     */
    @Override
    ...
    /**
     * Creates the grid for displaying activity status entities.
     * Sets up columns for name and description with appropriate headers and sorting.
     */
    @Override
    protected void createGridForEntity() {
        ...
        try {
            ...
            throw new RuntimeException("Failed to create grid for activity status view", e);
        }
    }
    ...
    protected void setupToolbar() {
        ...
        // Currently using default toolbar from parent class
    }
}