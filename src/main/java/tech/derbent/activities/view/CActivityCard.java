 ...
 * Layer: View (MVC)
 * 
 * Displays activity information including name, description/summary, and status.
 * Used within CActivityKanbanColumn to represent individual activities.
 */
public class CActivityCard extends Div {
    ...
    private String getActivityDescription() {
        // For now, we'll use a placeholder or derive from activity name
        // This can be enhanced when activity entity has description field
    ...
    private void updateStatusElement() {
        if (activity.getActivityStatus() != null && activity.getActivityStatus().getName() != null) {
            statusElement.setText(activity.getActivityStatus().getName());
    ...
    /**
     * Refreshes the card display with updated activity data.
