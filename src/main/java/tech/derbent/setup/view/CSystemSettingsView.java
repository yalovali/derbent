...
/**
 * CSystemSettingsView - View for managing system-wide configuration settings. Layer: View (MVC) Provides a
 * comprehensive interface for system administrators to configure application-wide settings including security, file
 * management, email configuration, database settings, backup preferences, and UI themes. Unlike company settings, this
 * is a singleton configuration that applies to the entire system.
 */
...
public class CSystemSettingsView extends CAbstractPage {
    ...
    private final com.vaadin.flow.data.binder.BeanValidationBinder<CSystemSettings> binder;
    ...
    /**
     * Constructor for CSystemSettingsView. Annotated with @Autowired to let Spring inject dependencies.
     * 
     * @param systemSettingsService
     *            the CSystemSettingsService instance
     */
    @Autowired
    ...
    /**
     * Cancels any unsaved changes and reverts the form to the last saved state. This method provides users with a way
     * to reject changes they made without saving.
     */
    ...
    /**
     * Post-construction initialization method called after dependency injection is complete. This ensures that
     * systemSettingsService is properly injected before we try to use it.
     */
    @PostConstruct
    ...
    private void testConfiguration() {
        ...
        try {
            ...
            if ((currentSettings.getSmtpServer() != null) && !currentSettings.getSmtpServer().trim().isEmpty()) {
                testResults.append("✓ SMTP server: ").append(currentSettings.getSmtpServer()).append(":")
