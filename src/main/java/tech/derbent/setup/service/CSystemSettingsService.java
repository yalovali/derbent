...
/**
 * CSystemSettingsService - Business logic layer for CSystemSettings entities. Layer: Service (MVC) Provides
 * comprehensive business logic for managing system-wide configuration settings. Since system settings are singleton by
 * design, this service ensures only one settings record exists and provides specialized methods for system
...
public class CSystemSettingsService extends CAbstractService<CSystemSettings> {
     ...
     * 
     * @param repository
     *            the CSystemSettingsRepository instance
     * @param clock
     *            the Clock instance for time-related operations
     */
    public CSystemSettingsService(final CSystemSettingsRepository repository, final Clock clock) {
    ...
    public CSystemSettings createDefaultSystemSettings() {
        ...
        try {
            ...
            final CSystemSettings savedSettings = systemSettingsRepository.saveAndFlush(newSettings);
            LOGGER.info("Default system settings created successfully with ID: {}", savedSettings.getId());
    ...
    public String[] getAllowedFileExtensions() {
        ...
        try {
            final Optional<String> result = systemSettingsRepository.getAllowedFileExtensions();
            ...
            LOGGER.debug("Allowed file extensions: {}", java.util.Arrays.toString(extensionArray));
            return extensionArray;
        } catch (final Exception e) {
            LOGGER.error("Error retrieving allowed file extensions", e);
            return new String[] { ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".png", ".jpg", ".jpeg", ".txt", ".zip" };
        }
    }

    @Override
    protected Class<CSystemSettings> getEntityClass() {
    ...
    public Optional<String> getMaintenanceMessage() {
        ...
        try {
            final Optional<String> result = systemSettingsRepository.getMaintenanceMessage();
    ...
    public double getMaxFileUploadSizeMb() {
        ...
        try {
            final Optional<java.math.BigDecimal> result = systemSettingsRepository.getMaxFileUploadSizeMb();
    ...
    public int getMaxLoginAttempts() {
        ...
        try {
            final Optional<Integer> result = systemSettingsRepository.getMaxLoginAttempts();
    ...
    public CSystemSettings getOrCreateSystemSettings() {
        ...
        final Optional<CSystemSettings> existingSettings = systemSettingsRepository.findSystemSettings();
    ...
    public int getSessionTimeoutMinutes() {
        ...
        try {
            final Optional<Integer> result = systemSettingsRepository.getSessionTimeoutMinutes();
    ...
    public Optional<CSystemSettings> getSystemSettings() {
        ...
        try {
            final Optional<CSystemSettings> result = systemSettingsRepository.findSystemSettings();
    ...
    public boolean isMaintenanceModeEnabled() {
        ...
        try {
            final Optional<Boolean> result = systemSettingsRepository.isMaintenanceModeEnabled();
    ...
    public boolean isStrongPasswordsRequired() {
        ...
        try {
            final Optional<Boolean> result = systemSettingsRepository.isStrongPasswordsRequired();
     ...
     * @param message
     *            the maintenance message to display (optional)
     * @return the updated CSystemSettings
     */
    @Transactional
    public CSystemSettings setMaintenanceMode(final boolean enabled, final String message) {
        LOGGER.debug("setMaintenanceMode called with enabled: {}, message: {}", enabled, message);
     ...
     *             if settings is null or invalid
     * @throws EntityNotFoundException
     *             if settings don't exist in database
     */
    @Transactional
    public CSystemSettings updateSystemSettings(final CSystemSettings settings) {
        LOGGER.debug("updateSystemSettings called with settings ID: {}", settings != null ? settings.getId() : "null");
        ...
        try {
            ...
            final CSystemSettings updatedSettings = systemSettingsRepository.saveAndFlush(settings);
            LOGGER.info("System settings updated successfully with ID: {}", updatedSettings.getId());
            return updatedSettings;
    ...
    private void validateSystemSettingsBusinessRules(final CSystemSettings settings) {
        ...
        if ((settings.getMaxLoginAttempts() != null)
                && ((settings.getMaxLoginAttempts() < 1) || (settings.getMaxLoginAttempts() > 10))) {
            throw new IllegalArgumentException("Max login attempts must be between 1 and 10");
        }

        // Validate lockout duration
        if ((settings.getAccountLockoutDurationMinutes() != null) && ((settings.getAccountLockoutDurationMinutes() < 1)
        ...
        if ((settings.getDatabaseConnectionPoolSize() != null) && ((settings.getDatabaseConnectionPoolSize() < 1)
        ...
        if ((settings.getCacheTtlMinutes() != null)
                && ((settings.getCacheTtlMinutes() < 1) || (settings.getCacheTtlMinutes() > 1440))) {
        ...
        if ((settings.getBackupRetentionDays() != null)
                && ((settings.getBackupRetentionDays() < 1) || (settings.getBackupRetentionDays() > 365))) {
        ...
        if ((settings.getPasswordExpiryDays() != null)
                && ((settings.getPasswordExpiryDays() < 1) || (settings.getPasswordExpiryDays() > 365))) {
