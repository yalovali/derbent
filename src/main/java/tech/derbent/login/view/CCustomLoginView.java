...
public class CCustomLoginView extends Main implements BeforeEnterObserver {
	...
	private void setupForm() {
		...
		formCard.addClassNames(LumoUtility.Background.BASE,
			LumoUtility.BorderRadius.LARGE, LumoUtility.BoxShadow.LARGE,
			LumoUtility.Padding.LARGE);
		formCard.setSpacing(true);
		formCard.setWidth("400px");
		// Application icon/logo (simple text for now)
		final Image icon =
			new Image("themes/default/images/background1.png", "Derbent Logo");
		icon.setWidth("60px");
		icon.setHeight("60px");
		...
		formCard.add(icon, title, subtitle, usernameField, passwordField, errorMessage,
			loginButton, passwordHint, backLink);
