...
/**
 * Repository interface for CLoginUser entities. Extends CAbstractRepository to
 * inherit basic CRUD operations. This repository is used by the authentication
 * system to: 1. Find users by username for login validation 2. Load user
 * details for Spring Security authentication 3. Manage user account data in the
 * database
 */
public interface CLoginUserRepository extends CAbstractRepository<CLoginUser> {
	...
	/**
	 * Finds a login user by their username (login field). This method is crucial
	 * for the authentication flow. Authentication Flow Usage: 1. User enters
	 * username in login form 2. Spring Security calls
	 * UserDetailsService.loadUserByUsername() 3. CLoginUserService uses this method
	 * to find the user 4. If found, password is verified and user is authenticated
	 ...
	 * @return Optional containing the CLoginUser with projects if found, empty
	 *         otherwise
	 */
