 ...
 * authentication using CUser entities. Security Flow: 1. User accesses protected resource
 * 2. If not authenticated, redirected to CLoginView 3. User enters credentials in login
 * form 4. Form posts to /login endpoint (handled by Spring Security) 5. Spring Security
 * calls CUserService.loadUserByUsername() 6. Password is verified using
 * BCryptPasswordEncoder 7. If successful, user is authenticated and granted access 8.
 * User roles determine what resources they can access
 */
@EnableWebSecurity
@Configuration
class CSecurityConfig extends VaadinWebSecurity {
	...
	/**
	 * Provides BCrypt password encoder bean. BCrypt is a secure hashing function designed
	 * for password storage. Password Encoding Flow: 1. When creating users, plain
	 * passwords are encoded with BCrypt 2. Encoded passwords are stored in database 3.
	 * During authentication, submitted passwords are compared using BCrypt 4. BCrypt
	 * handles salt generation and verification automatically
	...
	/**
	 * Provides UserDetailsService bean for authentication. This exposes the CUserService
	 * as the UserDetailsService for Spring Security.
