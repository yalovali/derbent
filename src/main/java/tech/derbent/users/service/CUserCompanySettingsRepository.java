...
public interface CUserCompanySettingsRepository extends CAbstractRepository<CUserCompanySettings> {
	...
	@Query (
		"SELECT ucs FROM CUserCompanySettings ucs LEFT JOIN FETCH ucs.company LEFT JOIN FETCH ucs.user WHERE ucs.user.id = :userId AND ucs.company.id = :companyId"
	)
	...
	@Query (
		"SELECT ucs FROM CUserCompanySettings ucs LEFT JOIN FETCH ucs.company LEFT JOIN FETCH ucs.user WHERE ucs.ownershipLevel = :ownershipLevel"
	)
	...
	@Query (
		"SELECT ucs FROM CUserCompanySettings ucs LEFT JOIN FETCH ucs.user WHERE ucs.company.id = :companyId AND (ucs.ownershipLevel = 'OWNER' OR ucs.ownershipLevel = 'ADMIN') AND ucs.isActive = true"
	)
	...
	@Query (
		"SELECT ucs FROM CUserCompanySettings ucs LEFT JOIN FETCH ucs.user WHERE ucs.company.id = :companyId AND ucs.role = :role AND ucs.isActive = true"
	)
