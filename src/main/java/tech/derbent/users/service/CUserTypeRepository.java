 ...
 * for project-aware user types with eager loading support.
 */
public interface CUserTypeRepository extends CEntityOfProjectRepository<CUserType> {

    @Query("SELECT ut FROM CUserType ut " + "LEFT JOIN FETCH ut.project " + "LEFT JOIN FETCH ut.assignedTo "
            + "LEFT JOIN FETCH ut.createdBy " + "WHERE ut.id = :id")
