...
public class CUserService extends CAbstractService<CUser> implements UserDetailsService {
    ...
    private Collection<GrantedAuthority> getAuthorities(final String rolesString) {
        ...
        final Collection<GrantedAuthority> authorities = Arrays.stream(rolesString.split(",")).map(String::trim)
                .filter(role -> !role.isEmpty()).map(role -> role.startsWith("ROLE_") ? role : "ROLE_" + role) // Add ROLE_ prefix if not present
    ...
    /**
     * Overrides the base get method to eagerly load CUserType relationship. This prevents LazyInitializationException
     * when the entity is used in UI contexts.
     * 
     * @param id
     *            the user ID
