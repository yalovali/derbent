...
public class CUserService extends CAbstractNamedEntityService<CUser> implements UserDetailsService {
    ...
    /**
     * Creates a new login user with encoded password. This method handles password encoding automatically.
     * 
     * @param username
     *            the username for login
     * @param plainPassword
     *            the plain text password (will be encoded)
     * @param name
    ...
    /**
     * Overrides the base get method to eagerly load CUserType and company relationships. This prevents
     * LazyInitializationException when the entity is used in UI contexts, particularly when company information is used
     * in comboboxes.
     * 
     * @param id
     *            the user ID
    ...
    public Optional<CUser> get(final Long id) {
        LOGGER.debug("Getting CUser with ID {} (overridden to eagerly load userType and company)", id);
        final Optional<CUser> entity = ((CUserRepository) repository).findByIdWithAllRelationships(id);
    ...
    protected void initializeLazyFields(final CUser user) {
        ...
        try {
            ...
            LOGGER.warn("Error initializing lazy fields for user with ID: {}", CSpringAuxillaries.safeGetId(user), e);
        }
    }

    /**
     * Implementation of UserDetailsService.loadUserByUsername(). This is the core method called by Spring Security
     * during authentication. Authentication Flow Step: 1. Spring Security calls this method with username from login
     * form 2. Query database to find CUser by username 3. If not found, throw UsernameNotFoundException 4. If found,
     * convert CUser to Spring Security UserDetails 5. Return UserDetails with username, password, and authorities 6.
     ...
     * 
     * @param userProjectSetting
     *            the project setting to save
     ...
     * @param company
     *            the company to associate with the user
    ...
    public CUser setCompanyAssociation(final CUser user, final tech.derbent.companies.domain.CCompany company) {
        LOGGER.info("setCompanyAssociation called for user: {} with company: {}",
                user != null ? user.getName() : "null", company != null ? company.getName() : "null");
    ...
    /**
     * Auxiliary method to set user profile information. Following coding guidelines to use service layer methods
     ...
     *            the user to configure
     * @param userRole
     *            the user's role enum
     * @param roles
     *            comma-separated role string for Spring Security
     * @return the configured user
     */
    @Transactional
