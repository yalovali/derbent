...
/**
 * Service class for managing user-project relationships. Handles CRUD operations for
 * CUserProjectSettings entities.
 */
@Service
@Transactional (readOnly = true)
public class CUserProjectSettingsService
	extends CAbstractEntityRelationService<CUserProjectSettings> {

	private final CUserProjectSettingsRepository repository;

	@Autowired
	public CUserProjectSettingsService(final CUserProjectSettingsRepository repository,
		final Clock clock) {
	 ...
	 */
	@Transactional
	public CUserProjectSettings addUserToProject(final CUser user, final CProject project,
		final String role, final String permission) {
		LOGGER.debug("Adding user {} to project {} with role {} and permission {}", user,
	...
	@Override
	protected CUserProjectSettings createRelationshipInstance(final Long userId,
		...
		throw new UnsupportedOperationException(
			"Use addUserToProject(CUser, CProject, String, String) method instead");
	}
	...
	@Override
	@Transactional (readOnly = true)
	public Optional<CUserProjectSettings> findRelationship(final Long userId,
		final Long projectId) {
	 ...
	 */
	@Transactional
	public CUserProjectSettings updateUserProjectRole(final CUser user,
		final CProject project, final String role, final String permission) {
		LOGGER.debug("Updating user {} project {} role to {} and permission to {}", user,
			project, role, permission);
		final Optional<CUserProjectSettings> settingsOpt =
			findRelationship(user.getId(), project.getId());
	...
	protected void validateRelationship(final CUserProjectSettings relationship) {
		...
		if (relationship.getUser() == null) {
			throw new IllegalArgumentException(
				"User is required for user project settings");
		}

		if (relationship.getProject() == null) {
			throw new IllegalArgumentException(
				"Project is required for user project settings");
		}

		if ((relationship.getRole() == null) || relationship.getRole().trim().isEmpty()) {
			throw new IllegalArgumentException(
				"Role is required for user project settings");
		}

		if ((relationship.getPermission() == null)
			|| relationship.getPermission().trim().isEmpty()) {
			throw new IllegalArgumentException(
				"Permission is required for user project settings");
		}
	}
}