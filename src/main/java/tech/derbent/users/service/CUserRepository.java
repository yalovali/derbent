...
public interface CUserRepository extends CAbstractNamedRepository<CUser> {
	...
	/**
	 * Finds a user by ID with all lazy relationships eagerly loaded (userType, company,
	 * and projectSettings).
	 * @param id the user ID
	 * @return optional CUser with all relationships loaded
	 */
	@Query (
		"SELECT u FROM CUser u LEFT JOIN FETCH u.userType LEFT JOIN FETCH u.company LEFT JOIN FETCH u.projectSettings WHERE u.id = :id"
	)
	Optional<CUser> findByIdWithAllRelationships(@Param ("id") Long id);
	/**
	 * Finds a user by ID with eagerly loaded company relationship to prevent
	...
	/**
	 * Finds a user by ID with eagerly loaded CUserType to prevent
	 * LazyInitializationException.
	 * @param id the user ID
	 ...
	 */
	@Query (
		"SELECT u FROM CUser u LEFT JOIN FETCH u.userType LEFT JOIN FETCH u.projectSettings WHERE u.id = :id"
	)
	Optional<CUser> findByIdWithUserTypeAndProjects(@Param ("id") Long id);
	/**
	 * Finds a login user by their CUser (login field). This method is crucial for the
	 * authentication flow. Authentication Flow Usage: 1. User enters username in login
	 * form 2. Spring Security calls UserDetailsService.loadUserByUsername() 3. CUser uses
	 * this method to find the user 4. If found, password is verified and user is
	 * authenticated
	...
	/**
	 * Finds a login user by their username and loads associated projects. Useful for
	 * getting complete user data including project assignments.
	 * @param username the username to search for
	 * @return Optional containing the CUser with projects if found, empty otherwise
	 */
	@Query (
		"SELECT u FROM CUser u LEFT JOIN FETCH u.projectSettings WHERE u.login = :username"
	)
