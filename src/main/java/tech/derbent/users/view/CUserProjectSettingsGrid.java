...
public class CUserProjectSettingsGrid extends VerticalLayout {
    ...
    private void deleteSelected() {
        ...
        if (selected == null) {
            new CWarningDialog("No Selection", "Please select a project setting to delete.").open();
            return;
        }
        if (getProjectSettings == null || setProjectSettings == null) {
            new CWarningDialog("System Error", "Project settings handlers are not available. Please refresh the page.")
                    .open();
            return;
        }
    ...
    private void openAddDialog() {
        ...
        if (projectService == null) {
            ...
            new CWarningDialog("System Error", "Project service is not available. Please try again later.").open();
            return;
        }
    ...
    private void openEditDialog() {
        final CUserProjectSettings selected = grid.asSingleSelect().getValue();
        if (selected == null) {
            new CWarningDialog("No Selection", "Please select a project setting to edit.").open();
            return;
        }
        if (currentUser == null) {
            new CWarningDialog("System Error", "Current user information is not available. Please refresh the page.")
                    .open();
            return;
        }
        final CUserProjectSettingsDialog dialog = new CUserProjectSettingsDialog(projectService, selected, currentUser,
    ...
    public void setProjectSettingsAccessors(final Supplier<List<CUserProjectSettings>> getProjectSettings,
            final Consumer<List<CUserProjectSettings>> setProjectSettings, final Runnable saveEntity) {
