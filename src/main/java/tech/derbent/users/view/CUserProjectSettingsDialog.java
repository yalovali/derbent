...
/**
 * Dialog for managing user-project assignments from the user perspective.
 * 
 * This dialog handles both creating new project assignments and editing existing ones.
 * It allows users to be assigned to projects with specific roles and permissions.
 * 
...
public class CUserProjectSettingsDialog extends CDBRelationDialog<CUserProjectSettings, CUser, CProject> {
    ...
    public CUserProjectSettingsDialog(final CProjectService projectService, final CUserProjectSettings settings,
        ...
        super(settings != null ? settings : new CUserProjectSettings(), user, onSave, settings == null);
    ...
    @Override
    protected List<CProject> getAvailableRelatedEntities() {
    ...
    protected void populateEntityField() {
        ...
        if (project != null) {
            projectService.getById(project.getId()).ifPresent(entityComboBox::setValue);
        }
    }
}