...
public class CUserProjectSettingsDialog extends CDBRelationDialog<CUserProjectSettings, CUser, CProject> {
	...
	public CUserProjectSettingsDialog(final CUserService masterService, final CProjectService detailService, final CUserProjectSettings settings,
			final CUser user, final Consumer<CUserProjectSettings> onSave) throws Exception {
		super(settings != null ? settings : new CUserProjectSettings(), user, masterService, detailService, onSave, settings == null);
		setupDialog();
		populateForm();
	}
	...
	protected void populateEntityField() {
		final CProject project = getRelatedEntityFromRelationship(getEntity());
		if (project != null) {
