...
/**
 * User profile dialog for editing user profile information. Extends CDBEditDialog to provide consistent dialog
 * behavior. Allows users to: - Edit display name - Change password - Upload/delete profile picture
 */
public class CUserProfileDialog extends CDBEditDialog<CUser> {
     ...
     *            Password encoder for password validation and encoding
     */
    ...
    private void handleProfilePictureChange() {
        ...
        } else if (!deleteProfilePictureButton.isEnabled() && (data.getProfilePictureData() != null)) {
            // Picture was deleted
    ...
    private void updateProfilePicturePreview() {
        LOGGER.debug("Updating profile picture preview for user: {}", data != null ? data.getLogin() : "null");

        if ((data != null) && (data.getProfilePictureData() != null) && (data.getProfilePictureData().length > 0)) {
