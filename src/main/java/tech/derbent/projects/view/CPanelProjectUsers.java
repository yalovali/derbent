...
/**
 * Simplified panel for managing users within a project. This is the reverse direction of CPanelUserProjectSettings -
 * instead of showing which projects a user has access to, this shows which users have access to a specific project and
 * allows management of their roles/permissions. Key features: - Add new users to the project - Edit existing user
 * roles/permissions - Remove users from the project - Automatic data synchronization with parent entity
 */
public class CPanelProjectUsers extends CPanelUserProjectBase<CProject, CUserProjectSettings> {
    ...
    public CPanelProjectUsers(final CProject currentEntity, final CEnhancedBinder<CProject> beanValidationBinder,
        ...
        super("Project Users", currentEntity, beanValidationBinder, CProject.class, entityService,
    ...
    protected String createDeleteConfirmationMessage(final CUserProjectSettings selected) {
        final String userName = selected.getUser() != null
                ? selected.getUser().getName() + " "
                        + (selected.getUser().getLastname() != null ? selected.getUser().getLastname() : "")
                : "Unknown User";
        return String.format(
                "Are you sure you want to remove user '%s' from this project? This action cannot be undone.", userName);
    }
    ...
    protected void openAddDialog() {
        ...
        final CProjectUserSettingsDialog dialog = new CProjectUserSettingsDialog((CProjectService) entityService,
    ...
    public void setAccessors(final Supplier<List<CUserProjectSettings>> getProjectUsers, final Runnable saveEntity) {
