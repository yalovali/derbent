...
/**
 * CCurrency - Domain entity representing currencies used in orders. Layer: Domain (MVC)
 * Defines the various currencies that can be used in order transactions, including
 * currency code, symbol, and exchange rate information. This entity extends CEntityNamed
 * and adds currency-specific fields such as currency code (USD, EUR, etc.) and symbol ($,
 * €, etc.).
 */
@Entity
...
public class CCurrency extends CEntityNamed {
	...
	@MetaData (
		...
		description = "ISO 4217 currency code (e.g., USD, EUR, GBP)", hidden = false,
		order = 2, maxLength = 3
	)
	...
	@MetaData (
		...
		description = "Currency symbol (e.g., $, €, £)", hidden = false, order = 3,
		maxLength = 5
	)
	...
	public String toString() {
		return currencyCode != null ? currencyCode + " (" + getName() + ")"
			: super.toString();
	}
}