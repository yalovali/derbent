...
/**
 * COrder - Domain entity representing orders in the company system. Layer: Domain (MVC)
 ...
 * responsible parties, and approval workflow. This entity extends CEntityOfProject to
 * inherit project association and standard audit fields, with comprehensive
 * order-specific fields using MetaData annotations for automatic form generation. Key
 * Features: - Project association (via CEntityOfProject) - Provider company and contact
 * management - Financial tracking with currency and cost information - Multi-level
 * approval workflow - Status tracking and responsibility assignment - Full audit trail
...
public class COrder extends CEntityOfProject<COrder> {
	...
	@MetaData (
		...
		hidden = false, order = 2, dataProviderBean = "COrderTypeService"
	)
	...
	@MetaData (
		...
		description = "Name of the company providing goods or services", hidden = false,
		order = 10, maxLength = 200
	)
	...
	@MetaData (
		...
		description = "Contact person at the provider company", hidden = false,
		order = 11, maxLength = 100
	)
	...
	@MetaData (
		...
		description = "Email address of the provider contact", hidden = false, order = 12,
		maxLength = 150
	)
	...
	@MetaData (
		...
		description = "User who requested this order", hidden = false, order = 20,
	...
	@MetaData (
		...
		description = "User responsible for managing this order", hidden = false,
	...
	@MetaData (
		...
		description = "Current status of the order", hidden = false, order = 30,
	...
	@MetaData (
		...
		description = "Currency for the order cost", hidden = false, order = 40,
	...
	@DecimalMax (
		...
		message = "Estimated cost cannot exceed 99,999,999,999.99"
	)
	@MetaData (
		displayName = "Estimated Cost", required = false, readOnly = false,
		defaultValue = "0.00", description = "Estimated cost of the order",
	...
	@DecimalMax (
		value = "99999999999.99", message = "Actual cost cannot exceed 99,999,999,999.99"
	)
	@MetaData (
		displayName = "Actual Cost", required = false, readOnly = false,
		defaultValue = "0.00", description = "Actual cost of the order", hidden = false,
		order = 42
	)
	...
	@MetaData (
		...
		description = "Date when the order was created", hidden = false, order = 50
	)
	...
	@MetaData (
		...
		description = "Date when the order is required to be completed", hidden = false,
		order = 51
	)
	...
	@MetaData (
		...
		description = "Actual or planned delivery date", hidden = false, order = 52
	)
	...
	@MetaData (
		...
		description = "External order reference number", hidden = false, order = 60,
		maxLength = 50
	)
	...
	@MetaData (
		...
		description = "Address where the order should be delivered", hidden = false,
		order = 61, maxLength = 500
	)
	...
	@MetaData (
		...
		description = "Approval records for this order", hidden = false, order = 70
	)
