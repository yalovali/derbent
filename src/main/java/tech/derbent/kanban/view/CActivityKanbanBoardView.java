 ...
 * View (MVC) Displays all activities of the current project grouped by activity status in
 * a Kanban-style layout. Each activity status forms a column containing activity cards
 * grouped by type. Implements project awareness and real-time updates.
 */
@Route ("activities-kanban")
@PageTitle ("Activity Kanban Board")
@Menu (
	order = 1.2, icon = "class:tech.derbent.kanban.view.CActivityKanbanBoardView",
	title = "Project.Kanban"
)
@PermitAll
public class CActivityKanbanBoardView extends VerticalLayout
	implements CProjectChangeListener, CInterfaceIconSet {
	...
	/**
	 * Handles when an activity is dropped into a different column. Updates the activity
	 * status in the database and refreshes the kanban board.
	...
	private void handleActivityDropped(final CActivity droppedActivity) {
		...
		try {
			...
			activityService.updateEntityStatus(droppedActivity,
				droppedActivity.getStatus());
			// Refresh the kanban board to reflect the changes
			loadKanbanData();
			LOGGER.info("Successfully updated activity status for: {}",
				droppedActivity.getName());
		} catch (final Exception e) {
			LOGGER.error("Error updating activity status for: {}",
				droppedActivity.getName(), e);
	...
	private void initializeView() {
		...
		emptyStateContainer
			.add(new Div("Create some activities to see them on the kanban board."));
	...
	private void loadKanbanData() {
		...
		try {
			...
			for (final Map.Entry<CActivityStatus,
				List<CActivity>> entry : activitiesByStatus.entrySet()) {
			...
			LOGGER.error("Error loading kanban data for project: {}", project.getName(),
				e);
	...
	/**
	 * Called when the component is attached to the UI. Registers the project change
	 * listener.
	 */
	@Override
