...
/**
 * Service to manage layout state (horizontal vs vertical) for views. Uses Vaadin session
 * to store layout preference.
 */
@Service
public class LayoutService {
	...
	private final Set<CLayoutChangeListener> layoutChangeListeners =
	...
	public void addLayoutChangeListener(final CLayoutChangeListener listener) {
		...
		if (listener != null) {
			...
			LOGGER.debug("Layout change listener registered: {}",
	...
	public void clearLayoutChangeListeners() {
		LOGGER.debug("Clearing all layout change listeners - current count: {}",
			layoutChangeListeners.size());
		...
		if (ui != null) {
				...
				layoutChangeListeners.forEach(listener -> {
					...
					if (listener != null) {
						...
						try {
							...
							LOGGER.debug("Notified layout listener: {}",
								listener.getClass().getSimpleName());
			...
			layoutChangeListeners.forEach(listener -> {
				...
				if (listener != null) {
					...
					try {
						listener.onLayoutModeChanged(newMode);
						LOGGER.debug("Directly notified layout listener: {}",
							listener.getClass().getSimpleName());
					} catch (final Exception e) {
						LOGGER.error(
							"Error directly notifying layout change listener: {}",
							listener.getClass().getSimpleName(), e);
					}
				}
				else {
	...
	public void removeLayoutChangeListener(final CLayoutChangeListener listener) {
		...
		if (listener != null) {
			...
			if (removed) {
				LOGGER.debug("Layout change listener unregistered: {}",
					listener.getClass().getSimpleName());
			}
			else {
				LOGGER.debug("Layout change listener was not found in the list: {}",
					listener.getClass().getSimpleName());
			}
		}
		else {
