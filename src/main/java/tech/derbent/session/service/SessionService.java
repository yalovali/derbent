...
/**
 * Service to manage user session state including active user and active project. Uses
...
public class SessionService {
	...
	private final Set<CProjectListChangeListener> projectListChangeListeners =
		ConcurrentHashMap.newKeySet();
	...
	public void addProjectChangeListener(final CProjectChangeListener listener) {
		...
		if (listener != null) {
			...
			// LOGGER.debug("Project change listener registered: {}",
	...
	public void addProjectListChangeListener(final CProjectListChangeListener listener) {
		...
		if (listener != null) {
			...
			// LOGGER.debug("Project list change listener registered: {}",
			// listener.getClass().getSimpleName());
		}
	}
	...
	/**
	 * Gets the currently active project from the session. If no project is set, returns
	...
	/**
	 * Gets the currently active user from the session. If no user is set, attempts to
	 * load the user from the authentication context. optinal means that it may return an
	 * empty value if no user is found.
	 */
		...
		if (activeUser == null) {
			...
			final Optional<
				org.springframework.security.core.userdetails.User> authenticatedUser =
					authenticationContext.getAuthenticatedUser(
	...
	/**
	 * Gets all available projects for the current user. For now, returns all projects.
	 * Can be enhanced to filter by user permissions.
	 */
	public List<CProject> getAvailableProjects() { return projectService.findAll(); }

	/**
	 * Event listener for project list changes. This method is called when projects are
	 * created, updated, or deleted to notify all registered listeners.
			...
			ui.access(() -> {
				projectChangeListeners.forEach(listener -> {
					...
					try {
						...
						// LOGGER.debug("Notified listener: {}",
						// listener.getClass().getSimpleName());
					} catch (final Exception e) {
						LOGGER.error("Error notifying project change listener: {}",
	...
	/**
	 * Notifies all registered project list change listeners about changes to the project
	 * list. This method safely handles UI access for components that may be in different
	 * UIs.
	 */
	public void notifyProjectListChanged() {
		LOGGER.debug("Notifying {} project list change listeners of project list change",
			projectListChangeListeners.size());
		// Use UI.access to safely notify listeners that may be in different UI contexts
		final UI ui = UI.getCurrent();
			...
			ui.access(() -> {
				projectListChangeListeners.forEach(listener -> {
					...
					try {
						...
						LOGGER.debug("Notified project list listener: {}",
							listener.getClass().getSimpleName());
					} catch (final Exception e) {
						LOGGER.error("Error notifying project list change listener: {}",
							listener.getClass().getSimpleName(), e);
					}
				});
			});
	...
	/**
	 * Unregisters a component from receiving project change notifications. Components
	 * should call this method when they are detached from the UI.
	 ...
	 */
	public void
		removeProjectListChangeListener(final CProjectListChangeListener listener) {
	...
	public void setActiveProject(final CProject project) {
		...
		if (session != null) {
			...
			LOGGER.info("Active project set to: {}",
	...
	public void setActiveUser(final CUser user) {
		...
		if (session != null) {
			...
			LOGGER.info("Active user set to: {}",
				user != null ? user.getLogin() : "null");
		}
	}

	/**
	 * Sets the layout service. This is called after bean creation to avoid circular
	 * dependency.
	 */
