...
/**
 * CCommentRepository - Repository interface for CComment entities. Layer: Service (MVC) -
 * Repository interface Provides data access methods for comment entities with support
 * for: - Activity-based queries - Project-based queries - Author-based queries -
 * Chronological ordering - Eager loading to prevent lazy initialization exceptions
 */
public interface CCommentRepository extends CAbstractRepository<CComment> {
	 ...
	 */
	@Query (
		"SELECT c FROM CComment c WHERE c.activity = :activity ORDER BY c.eventDate ASC"
	)
	List<CComment>
		findByActivityOrderByEventDateAsc(@Param ("activity") CActivity activity);
	/**
	...
	Page<CComment> findByActivityOrderByEventDateAsc(
		@Param ("activity") CActivity activity, Pageable pageable);
	/**
	 ...
	 */
	@Query (
		"SELECT c FROM CComment c LEFT JOIN FETCH c.author LEFT JOIN FETCH c.priority WHERE c.activity = :activity ORDER BY c.eventDate ASC"
	)
	List<CComment>
		findByActivityWithRelationships(@Param ("activity") CActivity activity);
	/**
	 * Finds all comments by a specific author, ordered by event date with eagerly loaded
	 * relationships.
	 * @param author the comment author
	 * @return list of comments by the author ordered by event date
	 */
	@Query (
		"SELECT c FROM CComment c LEFT JOIN FETCH c.priority WHERE c.author = :author ORDER BY c.eventDate DESC"
	)
	 ...
	 */
	@Query (
		"SELECT c FROM CComment c LEFT JOIN FETCH c.activity LEFT JOIN FETCH c.author LEFT JOIN FETCH c.priority WHERE c.id = :id"
	)
	 ...
	 */
	@Query (
		"SELECT c FROM CComment c LEFT JOIN FETCH c.author LEFT JOIN FETCH c.priority WHERE c.activity = :activity AND c.important = true ORDER BY c.eventDate ASC"
	)
