...
public class CPageServiceInvoice extends CPageServiceDynamicPage<CInvoice> implements IPageServiceHasStatusAndWorkflow<CInvoice> {
	 ...
	 * This is a reusable component that can be added to invoice detail views.
	...
	public Component createFinancialSummaryPanel(final CInvoice invoice) {
		...
		final Component discountRow = createMetricRow("Discount (" + invoice.getDiscountRate() + "%)", formatCurrency(invoice.getDiscountAmount()), VaadinIcon.TAG);
	...
	/** Creates a payment status badge component with color coding.
	 * This is a reusable component for displaying payment status.
	...
	public Component createPaymentStatusBadge(final CPaymentStatus status) {
		...
		badge.addClassNames(LumoUtility.Padding.Horizontal.SMALL, LumoUtility.Padding.Vertical.XSMALL,
			LumoUtility.FontSize.SMALL, LumoUtility.FontWeight.MEDIUM);
		badge.getStyle()
			.set("border-radius", "var(--lumo-border-radius-m)")
	...
	public Component createPaymentPlanInfo(final CInvoice invoice) {
		...
		info.addClassNames(LumoUtility.Padding.Horizontal.SMALL, LumoUtility.Padding.Vertical.XSMALL,
			LumoUtility.Gap.XSMALL, LumoUtility.AlignItems.CENTER);
		info.getStyle()
			.set("background-color", "var(--lumo-contrast-5pct)")
			.set("border-radius", "var(--lumo-border-radius-m)")
		...
		final Span text = new Span(String.format("Installment %d of %d",
			invoice.getInstallmentNumber(), invoice.getPaymentPlanInstallments()));
		text.addClassNames(LumoUtility.FontSize.SMALL, LumoUtility.FontWeight.MEDIUM);
