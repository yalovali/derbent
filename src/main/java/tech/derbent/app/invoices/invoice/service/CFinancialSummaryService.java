...
/** CFinancialSummaryService - Service for generating financial reports and summaries.
 * Provides period-based financial reporting including income, expenses, and profit calculations. */
@Service
...
public class CFinancialSummaryService {
    ...
    public CFinancialSummaryService(
            final CInvoiceService invoiceService,
            final CProjectExpenseService expenseService,
    ...
    public BigDecimal calculateTotalInvoiced(final CProject project, final LocalDate startDate, final LocalDate endDate) {
        ...
        return invoices.stream()
                .filter(inv -> inv.getProject().getId().equals(project.getId()))
                .filter(inv -> isDateInRange(inv.getInvoiceDate(), startDate, endDate))
                .map(CInvoice::getTotalAmount)
    ...
    public BigDecimal calculateTotalExpenses(final CProject project, final LocalDate startDate, final LocalDate endDate) {
        ...
        return expenses.stream()
                .filter(exp -> exp.getProject().getId().equals(project.getId()))
                .filter(exp -> isDateInRange(exp.getExpenseDate(), startDate, endDate))
                .map(CProjectExpense::getAmount)
                .filter(java.util.Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public BigDecimal calculateTotalIncome(final CProject project, final LocalDate startDate, final LocalDate endDate) {
        ...
        return incomes.stream()
                .filter(inc -> inc.getProject().getId().equals(project.getId()))
                .filter(inc -> isDateInRange(inc.getIncomeDate(), startDate, endDate))
                .map(CProjectIncome::getAmount)
                .filter(java.util.Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public BigDecimal calculateProfitMargin(final CProject project, final LocalDate startDate, final LocalDate endDate) {
        ...
        return netProfit.divide(totalIncome, 4, java.math.RoundingMode.HALF_UP)
    ...
    public BigDecimal calculateTotalOverdue(final CProject project) {
        ...
        return invoices.stream()
                .filter(inv -> inv.getProject().getId().equals(project.getId()))
                .filter(CInvoice::isOverdue)
                .map(CInvoice::getRemainingBalance)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public long countInvoicesByStatus(final CProject project, final CPaymentStatus status) {
        ...
        return invoices.stream()
                .filter(inv -> inv.getProject().getId().equals(project.getId()))
                .filter(inv -> inv.getPaymentStatus() == status)
                .count();
    }
    ...
    public List<CInvoice> getInvoicesDueSoon(final CProject project, final int days) {
        ...
        return invoices.stream()
                .filter(inv -> inv.getProject().getId().equals(project.getId()))
                .filter(inv -> inv.getPaymentStatus() != CPaymentStatus.PAID)
                .filter(inv -> inv.getPaymentStatus() != CPaymentStatus.CANCELLED)
