...
public class CWorkflowEntityInitializerService extends CInitializerServiceBase {
	...
	public static void initializeSampleWorkflow(final String name, final CProject project, final List<tech.derbent.api.entityOfCompany.domain.CProjectItemStatus> statuses,
			final List<tech.derbent.app.roles.domain.CUserProjectRole> roles,
			final tech.derbent.app.workflow.service.CWorkflowEntityService workflowEntityService,
			final tech.derbent.app.workflow.service.CWorkflowStatusRelationService workflowStatusRelationService) {
		tech.derbent.api.utils.Check.notNull(name, "Workflow name cannot be null");
		tech.derbent.api.utils.Check.notNull(project, "Project cannot be null");
		tech.derbent.api.utils.Check.notNull(statuses, "Statuses list cannot be null");
		tech.derbent.api.utils.Check.notNull(roles, "Roles list cannot be null");
		tech.derbent.api.utils.Check.notEmpty(statuses, "Statuses list cannot be empty");
		tech.derbent.api.utils.Check.notEmpty(roles, "Roles list cannot be empty");
		
		final List<tech.derbent.api.entityOfCompany.domain.CProjectItemStatus> filteredStatuses =
				statuses.stream().filter(status -> status != null).peek(status -> tech.derbent.api.utils.Check.isSameCompany(project, status))
						.sorted(java.util.Comparator.comparing(tech.derbent.api.entityOfCompany.domain.CProjectItemStatus::getSortOrder, java.util.Comparator.nullsLast(Integer::compareTo))
								.thenComparing(tech.derbent.api.entityOfCompany.domain.CProjectItemStatus::getId, java.util.Comparator.nullsLast(Long::compareTo)))
						.toList();
		tech.derbent.api.utils.Check.notEmpty(filteredStatuses, "No statuses available for workflow " + name + " in project " + project.getName());
		final List<tech.derbent.app.roles.domain.CUserProjectRole> filteredRoles = roles.stream().filter(role -> role != null && role.getProject() != null
				&& role.getProject().getId() != null && role.getProject().getId().equals(project.getId())).toList();
		tech.derbent.api.utils.Check.notEmpty(filteredRoles, "No roles available for workflow " + name + " in project " + project.getName());
	 ...
	 * 
	 * Creates multiple workflow entities with complete status transitions including:
	...
	public static void initializeSampleWorkflowEntities(final CProject project, final boolean minimal,
			...
			final tech.derbent.app.roles.service.CUserProjectRoleService userProjectRoleService,
			final tech.derbent.app.workflow.service.CWorkflowEntityService workflowEntityService,
			final tech.derbent.app.workflow.service.CWorkflowStatusRelationService workflowStatusRelationService) {
		try {
			// Get available statuses for this project
			final List<tech.derbent.api.entityOfCompany.domain.CProjectItemStatus> statuses = projectItemStatusService.listByCompany(project.getCompany()).stream()
					.sorted(java.util.Comparator.comparing(tech.derbent.api.entityOfCompany.domain.CProjectItemStatus::getSortOrder, java.util.Comparator.nullsLast(Integer::compareTo))
							.thenComparing(tech.derbent.api.entityOfCompany.domain.CProjectItemStatus::getId, java.util.Comparator.nullsLast(Long::compareTo)))
					.toList();
			tech.derbent.api.utils.Check.notEmpty(statuses, "No project item statuses found for project: " + project.getName());
			final List<tech.derbent.app.roles.domain.CUserProjectRole> roles = userProjectRoleService.listByProject(project).stream()
					.filter(role -> role.getProject() != null && project.getId().equals(role.getProject().getId())).toList();
			tech.derbent.api.utils.Check.notEmpty(roles, "No user project roles found for project: " + project.getName());
