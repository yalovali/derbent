...
public class CGanttTimelineHeader extends CVerticalLayout {
        ...
        private void configureControlBar() {
                ...
                final CButton reset = createControlButton("vaadin:refresh", "Reset to full range",
                                () -> applyRange(fullRangeStart, fullRangeEnd, true));
        ...
        private void renderWeekLines(final long totalDays, final CHorizontalLayout container) {
                ...
                while (!current.isAfter(endDate)) {
                        ...
                        final LocalDate weekEnd = current.plusWeeks(1).minusDays(1).isAfter(endDate) ? endDate
                                        : current.plusWeeks(1).minusDays(1);
