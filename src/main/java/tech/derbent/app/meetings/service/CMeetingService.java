...
public class CMeetingService extends CProjectItemService<CMeeting> implements IEntityRegistrable, IEntityWithView {
    ...
    @Override
    public Class<CMeeting> getEntityClass() {
    ...
    public void initializeNewEntity(final CMeeting entity) {
        ...
        final CProject currentProject = sessionService.getActiveProject()
                .orElseThrow(
                        () -> new CInitializationException("No active project in session - cannot initialize meeting"));
        final CUser currentUser = sessionService.getActiveUser()
                .orElseThrow(
                        () -> new CInitializationException("No active user in session - cannot initialize meeting"));
        IHasStatusAndWorkflowService.initializeNewEntity(entity, currentProject, meetingTypeService,
    ...
    /**
     * Lists meetings by project ordered by sprintOrder for sprint-aware components.
     * Items with null sprintOrder will appear last.
     * 
