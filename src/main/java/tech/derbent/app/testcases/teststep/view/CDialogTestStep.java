 ...
 * <p>
 * Add mode (isNew = true):
 * - Creates new test step with order number
 * - Fields: action, expected result, test data, notes
 * <p>
 * Edit mode (isNew = false):
 * - Edits existing test step
 * - All fields editable except step order (auto-managed)
 */
public class CDialogTestStep extends CDialogDBEdit<CTestStep> {

	private static final Logger LOGGER = LoggerFactory.getLogger(CDialogTestStep.class);
	private static final long serialVersionUID = 1L;

	private final CTestStepService testStepService;
	private final ISessionService sessionService;
	private final CEnhancedBinder<CTestStep> binder;
	private final CFormBuilder<CTestStep> formBuilder;
	 ...
	 * @param step            the test step entity (new or existing)
	 * @param onSave          callback for save action
	 * @param isNew           true if creating new step, false if editing */
	public CDialogTestStep(final CTestStepService testStepService, final ISessionService sessionService, final CTestStep step,
			final Consumer<CTestStep> onSave, final boolean isNew) throws Exception {
		...
		this.binder = CBinderFactory.createEnhancedBinder(CTestStep.class);
	...
	private void createFormFields() throws Exception {
		...
		textAreaAction.setPlaceholder("Enter the action to perform in this test step...");
		textAreaAction.setHelperText("Maximum 2000 characters");
		binder.forField(textAreaAction).bind(CTestStep::getAction, CTestStep::setAction);
		...
		binder.forField(textAreaExpectedResult).bind(CTestStep::getExpectedResult, CTestStep::setExpectedResult);
		formLayout.add(textAreaExpectedResult);

		// Test data text field
		textFieldTestData = new CTextField("Test Data");
		textFieldTestData.setWidthFull();
		textFieldTestData.setMaxLength(1000);
		textFieldTestData.setPlaceholder("Enter test data to use (optional)...");
		textFieldTestData.setHelperText("Maximum 1000 characters");
		binder.forField(textFieldTestData).bind(CTestStep::getTestData, CTestStep::setTestData);
		...
		binder.forField(textAreaNotes).bind(CTestStep::getNotes, CTestStep::setNotes);
	...
	public String getDialogTitleString() {
		return isNew ? "Add Test Step" : "Edit Test Step";
	}
	...
	protected String getFormTitleString() {
		return isNew ? "New Test Step" : "Edit Test Step";
	}

	@Override
	protected String getSuccessCreateMessage() {
		return "Test step added successfully";
	}

	@Override
	protected String getSuccessUpdateMessage() {
		return "Test step updated successfully";
	}
	...
	protected void populateForm() {
		try {
			...
			LOGGER.debug("Form populated for test step: {}", getEntity().getId() != null ? getEntity().getId() : "new");
		} catch (final Exception e) {
			LOGGER.error("Error populating form", e);
			CNotificationService.showException("Error loading test step data", e);
		}
	}
	...
	protected void validateForm() {
		...
		// Save test step
		testStepService.save(getEntity());

		LOGGER.debug("Test step validated and saved: {}", getEntity().getId());
	}
}
