 ...
 * </ul>
 * <p>
 * ISO 29119 Part 3 compliant test execution interface. ISTQB usability standards for keyboard navigation and result recording. */
public class CComponentTestExecution extends CVerticalLayout
	...
	private void createTestCard() {
		...
		currentTestCard.add(testCaseNameLabel, testCaseDescriptionLabel, stepNavigatorLabel, expectedLabel, expectedResultLabel, actualLabel,
	...
	@Override
	public String getComponentName() { return "testExecution"; }

	@Override
	public CTestRun getValue() { return currentTestRun; }
	...
	private boolean isAllStepsCompleted() { return allSteps.stream().allMatch(s -> s.getResult() != CTestResult.NOT_EXECUTED); }
		...
		try {
			...
			final Runnable onConfirm = () -> {
				try {
					...
					currentTestRun = completed;
					final String summary = String.format(
							"Test Execution Complete!\n\n" + "Total Test Cases: %d\n" + "Passed: %d\n" + "Failed: %d\n\n" + "Total Steps: %d\n"
									+ "Passed: %d\n" + "Failed: %d\n\n" + "Overall Result: %s",
							completed.getTotalTestCases(), completed.getPassedTestCases(), completed.getFailedTestCases(),
							completed.getTotalTestSteps(), completed.getPassedTestSteps(), completed.getFailedTestSteps(), completed.getResult());
	...
	private void populateJumpToComboBox() {
		...
		for (int i = 0; i < allSteps.size(); i++) {
			...
			final String caseName = caseResult != null && caseResult.getTestCase() != null ? caseResult.getTestCase().getName() : "Unknown";
	...
	private void updateHeader() {
		...
		final String sessionInfo = String.format("Session: %s | Suite: %s | Tester: %s", currentTestRun.getName(),
	...
	private void updateTestCard() {
		...
		if (caseResult != null && caseResult.getTestCase() != null) {
			...
			testCaseDescriptionLabel.setText(caseResult.getTestCase().getDescription() != null ? caseResult.getTestCase().getDescription() : "");
		} else {
		...
		final String expectedResult = currentStep.getTestStep() != null && currentStep.getTestStep().getExpectedResult() != null
				? currentStep.getTestStep().getExpectedResult() : "No expected result defined";
