...
/**
 * CComment - Company-scoped domain entity representing user comments.
 * 
 * Stores comment text and metadata about comments.
 ...
 * Parent entities (Activity, Risk, Meeting, Sprint, Project, Issue, etc.) have:
 *   @OneToMany(cascade = CascadeType.ALL, orphanRemoval = true)
 *   @JoinColumn(name = "activity_id")  // or risk_id, issue_id, etc.
 *   private Set<CComment> comments = new HashSet<>();
 * 
 * CComment has NO back-reference to parents (clean unidirectional pattern).
 * 
 * Layer: Domain (MVC)
 */
@Entity
...
public class CComment extends CEntityOfCompany<CComment> {
	...
	@AMetaData(
		...
		readOnly = false,
		defaultValue = "false",
		description = "Mark this comment as important",
		hidden = false
	)
