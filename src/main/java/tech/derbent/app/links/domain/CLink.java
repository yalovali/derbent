...
/**
 * CLink - Company-scoped domain entity representing bidirectional links between entities.
 * 
 ...
 * @JoinColumn(name = "source_entity_activity_id") // or risk_id, issue_id, etc.
 * private Set<CLink> links = new HashSet<>();
 * 
 * CLink has NO back-reference to parents (clean unidirectional pattern).
 * 
 * Layer: Domain (MVC)
 */
@Entity
...
public class CLink extends CEntityOfCompany<CLink> {
     ...
     */
    public CLink(final String sourceType, final Long sourceId, 
                 final String targetType, final Long targetId, 
    ...
    public void setSourceEntityType(final String sourceEntityType) {
    ...
    public void setTargetEntityType(final String targetEntityType) {
    ...
    /**
     * Get the display name for the target entity.
     * Attempts to load the entity and get its name.
     * 
    ...
    public String toString() {
        return String.format("CLink{id=%d, %s#%d -> %s#%d, type=%s}", 
            getId(), sourceEntityType, sourceEntityId, 
            targetEntityType, targetEntityId, linkType);
    }

    /**
     * Creates a clone of this link with the specified options.
     * 
     * Note: Links are typically NOT cloned due to their bidirectional nature
     * and the complexity of maintaining referential integrity.
     ...
     * @return a new instance of the link with cloned data
     * @throws Exception if cloning fails
     */
    @Override
    public CLink createClone(final CCloneOptions options) throws Exception {
        // Get parent's clone (CEntityOfCompany -> CEntityNamed -> CEntityDB)
        final CLink clone = super.createClone(options);

        // Clone basic link fields
        clone.sourceEntityType = this.sourceEntityType;
        clone.sourceEntityId = this.sourceEntityId;
        clone.targetEntityType = this.targetEntityType;
        clone.targetEntityId = this.targetEntityId;
        clone.linkType = this.linkType;
        clone.description = this.description;

        LOGGER.debug("Successfully cloned link with options: {}", options);
        return clone;
    }
