 ...
 * </p>
 * <p>
 * Sprint items are OWNED by their parent entities (CActivity/CMeeting)
 * via @OneToOne with CASCADE.ALL and orphanRemoval=true. This means:
 * </p>
 * <ul>
 * <li>Sprint items are created ONCE when Activity/Meeting is created</li>
 * <li>Sprint items are NEVER deleted independently - only when parent is
 * deleted</li>
 ...
 * </p>
 * <p>
 * The sprint field determines whether an item is in backlog or assigned to a
 * sprint:
 * </p>
 * <ul>
 * <li><strong>sprint = NULL</strong>: Item is in the backlog (not assigned to
 * any sprint)</li>
 * <li><strong>sprint = CSprint</strong>: Item is assigned to that specific
 * sprint</li>
 * </ul>
 * <p>
 ...
 * <li>Set sprint field to NULL for backlog, or target sprint for sprint
 * assignment</li>
 ...
 * </p>
 * <p>
 * Stores progress-related data (story points, dates, responsible person,
 * progress %). Implements IOrderedEntity for ordering within sprints/backlog.
 * </p>
 * 
 * @see CActivity
...
public class CSprintItem extends CEntityDB<CSprintItem> implements IHasIcon, IOrderedEntity {
    ...
    @AMetaData(displayName = "Component Widget", required = false, readOnly = false, description = "Component Widget for item", hidden = false, dataProviderBean = "pageservice", dataProviderMethod = "getComponentWidget")
    ...
    @AMetaData(displayName = "Sprint", required = false, readOnly = false, description = "The sprint this item belongs to (null = backlog)", hidden = false, dataProviderBean = "CSprintService")
    ...
    @AMetaData(displayName = "Progress %", required = false, readOnly = false, defaultValue = "0", description = "Completion percentage (0-100)", hidden = false)
    ...
    @AMetaData(displayName = "Responsible", required = false, readOnly = false, description = "Person responsible for completing this work", hidden = false, dataProviderBean = "CUserService")
    private CUser responsible;
    @Transient
    @AMetaData(displayName = "Status", required = false, readOnly = false, description = "Current status of the item", hidden = false, setBackgroundFromColor = true, useIcon = true)
    ...
    public CComponentWidgetEntity<CSprintItem> getComponentWidget() {
    ...
    /**
     * Set the kanban column ID for transient display purposes. Used to track which
     * kanban column this item is displayed in.
     * 
    ...
    public String toString() {
        return String.format("CSprintItem{id=%d, sprint=%s, storyPoint=%d, progress=%d%%}", getId(),
