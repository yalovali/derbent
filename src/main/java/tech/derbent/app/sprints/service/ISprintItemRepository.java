...
public interface ISprintItemRepository extends IAbstractRepository<CSprintItem> {
	...
	/** Find a specific sprint item by sprint ID and item ID.
	 * @param sprintId the sprint ID
	 * @param itemId the project item ID
	 * @return the sprint item, or null if not found */
	@Query ("SELECT e FROM #{#entityName} e WHERE e.sprint.id = :sprintId AND e.itemId = :itemId")
	CSprintItem findByMasterIdAndItemId(@Param ("sprintId") Long sprintId, @Param ("itemId") Long itemId);
