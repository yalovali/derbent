...
/**
 * CPageServiceSprint - Page service for Sprint management UI. Handles UI events
 * and interactions for sprint views.
 */
public class CPageServiceSprint extends CPageServiceDynamicPage<CSprint>
    ...
    private static CSprint drag_getTargetSprintFromDropTarget(final CDragDropEvent event) {
        ...
        if (event.getSource() instanceof CGrid<?>) {
            ...
            // final CSprint sprint = (CSprint) ((CGrid<?>)
            // event.getSource()).getSelectedEntity();
            if (event.getTargetItem() instanceof CSprint) {
                ...
                LOGGER.warn("Drop event target item is not a CSprint: {}",
                        event.getTargetItem().getClass().getSimpleName());
                return null;
            }
            ...
            LOGGER.error("Cannot determine target sprint from drop event source: {}",
    ...
    /**
     * Creates and configures the backlog items component for displaying items not
     * in the sprint.
     * 
     ...
     * 
     * @param event the drop event containing the dragged item, target position, and
     *              drop location
     */
    ...
    private void drag_handle_backlogItem_toSprint(final CDragDropEvent event) {
        try {
            ...
            if (!(draggedItem instanceof CProjectItem)) {
                LOGGER.error("Dragged item is not a CProjectItem: {}",
                        draggedItem != null ? draggedItem.getClass() : "null");
                return;
            }
            final ISprintableItem itemToAdd = (ISprintableItem) draggedItem;
            final GridDropLocation dropLocation = event.getDropLocation();
            final CSprintItem targetSprintItem = event.getTargetItem() instanceof CSprintItem
                    ? (CSprintItem) event.getTargetItem()
                    : null;
    ...
    private void drag_handle_sprintItem_reorder(final CDragDropEvent event) {
        try {
            ...
            if (draggedId == null || targetId == null) {
                LOGGER.warn("Sprint item reorder skipped - missing item IDs (dragged: {}, target: {})", draggedId,
                        targetId);
                return;
            }
            final GridDropLocation dropLocation = event.getDropLocation();
            ...
            if (draggedSprint == null || targetSprint == null || draggedSprint.getId() == null
                    || targetSprint.getId() == null) {
                LOGGER.warn(
                        "Sprint item reorder skipped - missing sprint references (dragged sprint: {}, target sprint: {})",
                        draggedSprint != null ? draggedSprint.getId() : null,
                        targetSprint != null ? targetSprint.getId() : null);
                return;
            }
            if (!draggedSprint.getId().equals(targetSprint.getId())) {
                LOGGER.info(
                        "Sprint item reorder skipped - items belong to different sprints (dragged sprint: {}, target sprint: {})",
            ...
            items.sort(Comparator
                    .comparingInt(item -> item.getItemOrder() != null ? item.getItemOrder() : Integer.MAX_VALUE));
            ...
            if (targetIndex == -1) {
                LOGGER.warn("Sprint item reorder skipped - target item {} not found in sprint {}", targetItem.getId(),
                        draggedSprint.getId());
                return;
            }
            ...
            LOGGER.info(
                    "[SprintReorder] Reordering sprint items in sprint {}: move item {} to position {} (dropLocation: {}, target: {})",
    ...
    /**
     * Handles moving a sprint item back to the backlog (from sprint items grid to
     * backlog).
     * 
     * @param event the drop event containing the dragged sprint item, target
     *              position, and drop location
     */
    private void drag_handle_sprintItem_toBacklog(final CDragDropEvent event) {
        ...
        LOGGER.info("[BacklogDrop] Moving sprint item {} back to backlog (dropLocation: {}, target: {})",
                sprintItem.getId(), event.getDropLocation(),
                event.getTargetItem() instanceof CProjectItem ? ((CProjectItem<?>) event.getTargetItem()).getId()
                        : null);
        try {
    ...
    /**
     * Handles moving a sprint item to a different sprint (from sprint items grid to
     * sprint widget on master grid). This method is called when a
     * CSprintItem is dragged from a sprint items grid and dropped onto a sprint
     * widget in the master grid.
     * 
     * @param event the drop event containing target and location
     */
    private void drag_handle_sprintItem_toSprint(final CDragDropEvent event) {
        try {
            ...
            if (!(draggedObject instanceof CSprintItem)) {
                LOGGER.error("Dragged item is not a CSprintItem: {}",
                        draggedObject != null ? draggedObject.getClass() : "null");
                return;
            }
            ...
            LOGGER.debug("Moving sprint item {} from sprint {} to sprint {}", draggedItem.getId(), sourceSprintId,
                    targetSprint.getId());
            ...
            if (!sourceSprintId.equals(targetSprint.getId())) {
                ...
                LOGGER.warn("Sprint item {} already in sprint {}, no change needed", draggedItem.getId(),
                        targetSprint.getId());
            }
    ...
    /**
     * Inserts a backlog item into a sprint at the correct position based on drop
     * location and target.
     * 
     ...
     */
    private void drag_insertBacklogItemIntoSprint(final CSprint targetSprint, final ISprintableItem itemToAdd,
            final GridDropLocation dropLocation,
            final CSprintItem targetSprintItem) {
        try {
            final List<CSprintItem> existingItems = new ArrayList<>(
                    sprintItemService.findByMasterId(targetSprint.getId()));
            existingItems.sort(Comparator
                    .comparingInt(item -> item.getItemOrder() != null ? item.getItemOrder() : Integer.MAX_VALUE));
            int insertIndex = existingItems.size();
            if (targetSprintItem != null) {
                for (int i = 0; i < existingItems.size(); i++) {
                    if (existingItems.get(i).getId() != null
                            && existingItems.get(i).getId().equals(targetSprintItem.getId())) {
            ...
            LOGGER.info("[SprintInsert] Calculated insert index {} (existing: {})", insertIndex + 1,
                    existingItems.size());
            for (int i = 0; i < existingItems.size(); i++) {
                ...
                item.setItemOrder(newOrder);
                sprintItemService.save(item);
                // LOGGER.debug("[SprintInsert] Saved sprint item {} with order {}",
                // item.getId(), newOrder);
            }
        } catch (final Exception e) {
     ...
     * drag-drop service instead of deleting the sprint item. Sprint items
     * are owned by Activity/Meeting with CASCADE.ALL orphanRemoval=true, so
     * deleting them would delete the parent entity.
     * </p>
     * 
    ...
    public void on_backlogItems_drop(@SuppressWarnings("unused") final Component component, final Object value) {
        try {
            ...
            LOGGER.error("Unhandled drop on backlog - draggedItem type: {}",
                    draggedItem != null ? draggedItem.getClass().getSimpleName() : "null");
        } catch (final Exception e) {
    ...
    public void on_description_blur(final Component component, final Object value) {
        LOGGER.info("function: on_description_blur for Component type: {}",
                component.getClass().getSimpleName() + " current value: " + value + " on page service:"
    ...
    public void on_masterGrid_drop(final Component component, final Object value) {
        try {
            ...
            Check.instanceOf(value, CDragDropEvent.class, "Drop value must be CDragDropEvent");
            final CDragDropEvent event = (CDragDropEvent) value;
            // Vaadin sets the drop target as the ComponentEvent source; drag origin lives
            // on the stored dragStart event.
            final boolean isInternalDrag = getActiveDragStartEvent().getSourceList().contains(component);
            final Object draggedItem = getActiveDragStartEvent().getDraggedItems().get(0);
            LOGGER.info("=== Drop on Master Grid === (internal: {}, draggedItem type: {}, dropTarget: {} id:{})",
                    isInternalDrag,
                    draggedItem != null ? draggedItem.getClass().getSimpleName() : "null",
            ...
            if (draggedItem instanceof CSprintItem) {
                if (isInternalDrag && event.getTargetItem() instanceof CSprintItem) {
                    ...
                    if (draggedSprintItem.getSprint() != null && targetSprintItem.getSprint() != null
                            && draggedSprintItem.getSprint().getId() != null
    ...
    public void populateForm() {
        LOGGER.debug(
                "populateForm called - CComponentListSprintItems receives entity updates via IContentOwner interface");
    }

    /**
     * Reorders backlog items after inserting a new item at a specific sprint order
     * position. All items with sprintOrder >= newOrder need to be
     * shifted up by 1.
     * 
     * @param newOrder      the sprint order value of the newly inserted item
     * @param excludeItemId the ID of the item being inserted (to exclude it from
     *                      reordering)
     */
    ...
    private void updateBacklogItemOrder(final ISprintableItem item, final CDragDropEvent event) {
        ...
        if (targetItem == null || dropLocation == null) {
            ...
            LOGGER.info("[BacklogReorder] Dropped on empty space - assigning item {} to end of backlog (order {})",
                    item.getId(), newOrder);
        ...
        if (targetOrder == null) {
            LOGGER.warn("Target item {} has null sprint order, assigning dropped item to end of backlog",
                    targetItem.getId());
