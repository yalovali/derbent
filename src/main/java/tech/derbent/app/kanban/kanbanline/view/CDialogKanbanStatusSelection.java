...
/** CDialogKanbanStatusSelection - Simple, colorful dialog for selecting a status during kanban drag-drop.
 * 
 * This dialog displays a list of available statuses with their colors and icons,
 * allowing the user to select which status to apply when dragging an item to a column
 * that has multiple valid statuses.
 * 
 * Features:
 * - Large, colorful status buttons with icon and status name
 * - Easy to click on touch devices
 * - Cancel option to abort the status change
...
public class CDialogKanbanStatusSelection extends CDialog {
	...
	/** Creates a large, colorful button for a status.
	 * 
	 * The button displays:
	 * - Status icon (if available)
	 * - Status name
	 * - Background color matching the status color
	...
	private CButton createStatusButton(final CProjectItemStatus status) {
		...
		final String backgroundColor = status.getColor() != null && !status.getColor().isBlank() 
			? status.getColor() 
			: CProjectItemStatus.DEFAULT_COLOR;
		...
		button.getStyle()
			...
			.set("color", textColor)
			.set("border", "2px solid rgba(0, 0, 0, 0.1)")
	...
	protected Icon getFormIcon() throws Exception {
		return VaadinIcon.CLIPBOARD_CHECK.create();
	}

	@Override
	protected String getFormTitleString() {
		return "Choose Status for Column '" + columnName + "'";
	}

	/** Determines if a hex color is light (closer to white) or dark (closer to black).
	 * 
	 * Uses a simple brightness calculation based on RGB values.
	 * 
	 ...
	 */
	private boolean isLightColor(final String hexColor) {
