...
/**
 * CComponentKanbanColumn - Renders a single kanban column with its header and
 * post-it items.
 */
public class CComponentKanbanColumn extends CComponentBase<CKanbanColumn>
    ...
    // PERFORMANCE OPTIMIZATION: Cache filtered items to avoid repeated filtering
    // operations.
    // Without caching: 6-8 filter calls per column (called from refreshItems,
    // refreshStoryPointTotal, etc.)
    ...
    public CComponentKanbanColumn() {
        ...
        defaultBadge.getStyle().set("background-color", "#E3F2FD").set("color", "#0D47A1").set("padding", "2px 6px")
                .set("border-radius", "6px")
                .set("font-size", "10px").set("font-weight", "600");
        storyPointTotalLabel = new CSpan();
        storyPointTotalLabel.getStyle().set("background-color", "#E8F5E9").set("color", "#2E7D32")
                .set("padding", "4px 8px")
                .set("border-radius", "6px").set("font-size", "12px").set("font-weight", "700")
    ...
    public void drag_checkEventAfterPass(final CEvent event) {
        LOGGER.debug("Drag event check after pass: {} comp id:{} event type:{}", event, getId(),
    ...
    public boolean drag_isDropAllowed(CDragStartEvent event) {
        ...
        // FIXED: Allow drop to any column - workflow validation is handled by
        // CPageServiceKanbanLine.handleKanbanDrop()
        // Previous logic was: column.getIncludedStatuses().stream().anyMatch(status ->
        // status.getId().equals(sprintItem.getStatus().getId()))
        // This incorrectly required the CURRENT status to be in the column, rather than
        // checking for valid transitions.
        // The proper workflow transition validation happens in the drop handler, which
        // will show appropriate warnings
        // if no valid transition exists while still allowing the visual column
        // assignment.
        return true;
    }
    ...
    public void drag_setDragEnabled(final boolean enabled) {
        // itemsLayout.getChildren().filter(CComponentKanbanPostit.class::isInstance).map(component
        // -> (CComponentKanbanPostit) component)
    ...
    public void drag_setDropEnabled(final boolean enabled) {
        ...
        // Note: Vaadin Flow 24.8 does not provide addDragOverListener or
        // addDragLeaveListener in the Java API.
        // These events are managed client-side in HTML5 DnD but not exposed server-side
        // by Vaadin.
        // For visual feedback during drag-over, we can use CSS hover states or
        // implement custom
        // client-side JavaScript integration if needed. The main drag-drop logic uses
        // the drop event.
        //
    ...
    /**
     * Filters items that should appear in this column and caches the result. This
     * is an expensive operation (stream filter) that was called multiple
     * times per refresh cycle. Now called only once, with results cached until
     * items or column value changes.
     */
    private List<CSprintItem> filterItems(final List<CSprintItem> items) {
        LOGGER.debug("[DragDrop] Filtering items for kanban column {}",
        ...
        final List<CSprintItem> filtered = items.stream().filter(Objects::nonNull).filter(item -> {
            ...
            if (!matches && itemColumnId != null) {
                LOGGER.debug("[DragDrop] Item {} has kanbanColumnId {} (not matching column {})", item.getId(),
                        itemColumnId, columnId);
            }
            return matches;
        }).toList();
        LOGGER.info("[DragDrop] Column {} (id {}) filtered to {} items out of {} total", column.getName(), columnId,
     ...
     * ==================== PERFORMANCE OPTIMIZATION: Avoid repeated expensive
     * filtering operations. WITHOUT CACHING (old code): - refreshItems()
     * calls filterItems() → 1st filter - refreshStoryPointTotal() calls
     * filterItems() → 2nd filter - Each refresh methods calls filterItems()
     * independently - Result: 6-8 filter operations per column per refresh WITH
     * CACHING (optimized): - First call to getFilteredItems() calls
     * filterItems() and caches result - Subsequent calls return cached result (no
     * filtering) - Cache invalidated only when items or column value
     * changes - Result: 1 filter operation per column per refresh (85% reduction)
     * Example with 5 columns: - Without caching: 5 columns × 6 filters =
     * 30 filter operations - With caching: 5 columns × 1 filter = 5 filter
     * operations - Improvement: 83% reduction in filtering overhead
     */
    ...
    /**
     * Invalidates the cached filtered items. Called when sprintItems or column
     * value changes. CACHE INVALIDATION: Clear cache to force recomputation
     * on next access. This ensures cached data is always fresh after data or
    ...
    /**
     * Updates the column UI when its value changes. ONE REFRESH ONLY PATTERN: This
     * method is part of the refresh optimization strategy. Without
     * optimization: setValue() would trigger full refresh even when items not yet
     * set, then setItems() would trigger another full refresh = DOUBLE
     * REFRESH. With optimization: Skip refresh if items are empty (component not
     * fully initialized). This ensures refresh happens only ONCE after
     * both value and items are set.
     */
    @Override
    protected void onValueChanged(final CKanbanColumn oldValue, final CKanbanColumn newValue,
    ...
    private void refreshStatuses() {
        LOGGER.debug("Refreshing statuses label for kanban column {}",
                getValue() != null ? getValue().getName() : "null");
        final CKanbanColumn column = getValue();
        ...
        final String statuses = column.getIncludedStatuses().stream().filter(Objects::nonNull)
                .map(status -> status.getName())
                .filter(name -> name != null && !name.isBlank()).sorted(String::compareToIgnoreCase)
    ...
    public void select_checkEventAfterPass(final CEvent event) {
        LOGGER.debug("[KanbanSelect] Selection propagated for column {}",
                getValue() != null ? getValue().getName() : "null");
    }
    ...
    /**
     * Sets the items displayed in this column. ONE REFRESH ONLY PATTERN: This
     * method is part of the refresh optimization strategy. Without
     * optimization: setItems() would trigger full refresh even when value not yet
     * set, then setValue() would trigger another full refresh = DOUBLE
     * REFRESH. With optimization: Skip refresh if value is null (component not
     * fully initialized). This ensures refresh happens only ONCE after both
     * value and items are set. CORRECT initialization order in parent
     * (CComponentKanbanBoard): 1. columnComponent.setValue(column); // Sets
     * configuration, skips refresh (items empty) 2.
     * columnComponent.setItems(sprintItems); // Triggers SINGLE refresh (value is
     * set) Result: ONE
     * refresh per column instead of TWO (50% reduction in refresh calls).
     */
    public void setItems(final List<CSprintItem> items) {
        ...
        invalidateCache();
        // ONE REFRESH ONLY: Only refresh if column value is set - avoid premature
        ...
        if (getValue() != null) {
            // CRITICAL FIX: Call full refresh to initialize header, colors, statuses, and
            // items
            // Previously only called refreshItems() which skipped header/color
            // initialization
            applyBackgroundColor();
            refreshHeader();
            refreshStatuses();
