...
public class CPageServiceKanbanLine extends CPageServiceDynamicPage<CKanbanLine> {
    ...
    /**
     * Applies the selected status to the project item and saves it. This method
     * encapsulates the save logic so it can be called both from automatic
    ...
    private void applyStatusAndSave(final CProjectItem<?> item, final CSprintItem sprintItem,
        ...
        try {
            ...
            final CProjectItemService<?> projectItemService = (CProjectItemService<?>) CSpringContext
                    .getBean(projectItemServiceClass);
    ...
    public void bind() {
        try {
            ...
            if (getView() instanceof CDynamicPageViewWithoutGrid) {
                // after form creation
                final CHorizontalLayout layout = getView().getDetailsBuilder().getFormBuilder()
                        .getHorizontalLayout("kanbanBoard");
    ...
    /**
     * Handles dragging a sprint item between kanban columns (updates status and
     * column). This is the original kanban drag-drop logic, extracted to a
    ...
    private void handleDragBetweenColumns(final CSprintItem sprintItem, final CDragDropEvent event) {
        ...
        final CProjectItemStatusService projectItemStatusService = CSpringContext
                .getBean(CProjectItemStatusService.class);
        final List<CProjectItemStatus> targetStatuses = projectItemStatusService.resolveStatusesForColumn(targetColumn,
        ...
        if (targetStatuses.isEmpty()) {
            ...
            componentKanbanBoard.getUI().ifPresent(ui -> ui.access(() -> {
                ...
                CNotificationService
                        .showWarning("Item moved to '" + targetColumn.getName() + "' column, but status remains '"
    ...
    /**
     * Handles dragging a backlog item (CProjectItem) to a kanban column (adds to
     * sprint). This method creates a new sprint item for the backlog
     * item, adds it to the current sprint, assigns it to the target column, and
     * resolves the appropriate status based on workflow rules.
     * 
    ...
    private void handleDragFromBacklog(final CProjectItem<?> projectItem, final CDragDropEvent event) {
        LOGGER.info("[DragDrop] Handling drag from backlog to kanban column - adding item {} to sprint",
                projectItem.getId());
        try {
            // Get target column
            final CKanbanColumn targetColumn = resolveTargetColumn(event);
            ...
            final tech.derbent.app.sprints.service.CSprintItemService sprintItemService = CSpringContext
                    .getBean(tech.derbent.app.sprints.service.CSprintItemService.class);
            ...
            LOGGER.info("[DragDrop] Updated sprint item - sprint: {}, kanbanColumnId: {}, itemOrder: {}",
                    currentSprint.getId(), targetColumn.getId(),
                    nextOrder);
            // Resolve valid statuses for target column
            final CProjectItemStatusService projectItemStatusService = CSpringContext
                    .getBean(CProjectItemStatusService.class);
            final List<CProjectItemStatus> targetStatuses = projectItemStatusService
                    .resolveStatusesForColumn(targetColumn, (IHasStatusAndWorkflow<?>) projectItem);
            LOGGER.info("[DragDrop] Resolved {} target statuses for column {}", targetStatuses.size(),
                    targetColumn.getName());
            // Handle status assignment based on number of valid statuses
            if (targetStatuses.isEmpty()) {
                // No valid status: add to sprint but warn about status
                LOGGER.warn(
                        "[DragDrop] No valid workflow transitions to target column {}, adding to sprint without status change",
                        targetColumn.getName());
                ...
                final CProjectItemStatus newStatus = targetStatuses.get(0);
                LOGGER.info(
                        "[DragDrop] Single status available for column {}, automatically setting status to {} for backlog item {}",
                ...
                final Class<?> projectItemServiceClass = CEntityRegistry
                        .getServiceClassForEntity(projectItem.getClass());
                final CProjectItemService<?> projectItemService = (CProjectItemService<?>) CSpringContext
                        .getBean(projectItemServiceClass);
                projectItemService.revokeSave(projectItem);
                LOGGER.info("[DragDrop] Project item {} status updated to {}", projectItem.getId(),
                        newStatus.getName());
                // Save the sprint item (cascades from parent save)
                sprintItemService.save(existingSprintItem);
                LOGGER.info("[DragDrop] Sprint item {} saved to database", existingSprintItem.getId());
                // CRITICAL: Defer UI refresh until after Vaadin drop event completes
                componentKanbanBoard.getUI().ifPresent(ui -> ui.access(() -> {
                    LOGGER.info("[DragDrop] Refreshing UI after backlog item added to sprint with status '{}'",
                            newStatus.getName());
                    // Refresh both board and backlog
                    componentKanbanBoard.reloadSprintItems();
                    componentKanbanBoard.refreshComponent();
                ...
                LOGGER.info(
                        "[DragDrop] Multiple statuses ({}) available for column {}, showing selection dialog for backlog item {}",
                        targetStatuses.size(), targetColumn.getName(), projectItem.getId());
                // Save sprint item first (without status change)
                sprintItemService.save(existingSprintItem);
                LOGGER.info("[DragDrop] Sprint item {} saved to database (before status dialog)",
                        existingSprintItem.getId());
     ...
     * <p>
     * <strong>CRITICAL FIX:</strong> This method now correctly uses the unified
     * drag-drop service instead of manually deleting the sprint item.
     * Sprint items are owned by Activity/Meeting with CASCADE.ALL
     * orphanRemoval=true.
     * </p>
     * <p>
     ...
     * </p>
     * <ul>
     * <li>Removed incorrect item.setSprintItem(null) call (violates ownership
     * pattern)</li>
     * <li>Removed incorrect sprintItemService.delete(sprintItem) call (causes
     * cascade delete)</li>
     * <li>Now uses dragDropService.moveSprintItemToBacklog() which sets sprint to
     * NULL</li>
     * </ul>
     * 
    ...
    private void handleDropOnBacklog(final Object draggedItem, final CDragDropEvent event) {
        ...
        try {
            ...
            /*
             * componentKanbanBoard.getUI().ifPresent(ui -> ui.access(() -> { // Refresh
             * both board and backlog
             * componentKanbanBoard.reloadSprintItems();
             * componentKanbanBoard.refreshComponent(); final CComponentKanbanColumnBacklog
             * backlogColumn =
             * componentKanbanBoard.getBacklogColumn(); if (backlogColumn != null) {
             * backlogColumn.refreshComponent(); }
             * CNotificationService.
             * showSuccess("Item removed from sprint and returned to backlog");
             * LOGGER.
             * info("Successfully removed sprint item {} from sprint (status preserved)",
             * sprintItem.getId()); }));
             */
        } catch (final Exception e) {
    ...
    /**
     * Handles kanban board drop events by updating sprint item status and kanban
     * column assignment. This is the core drag-drop workflow for the
     * kanban board: 1. Extract dragged item from active drag start event
     * (CSprintItem or CProjectItem) 2. Determine drop target (backlog column or
     * regular kanban column) 3a. Drop on backlog: Remove item from sprint (returns
     * to backlog) 3b. Drop on kanban column from backlog: Add item to
     * sprint + set status + assign column 3c. Drop on kanban column from another
     * column: Update status and kanban column assignment 4. Save changes
     * to database 5. Refresh kanban board to reflect changes Backlog Integration: -
     * Drops onto CComponentKanbanColumnBacklog remove items from sprint
     * - Drags from backlog (CProjectItem) add items to sprint - Drags between
     * kanban columns (CSprintItem) update status/column Status Resolution
     * Logic: - Gets statuses mapped to target column (column.getIncludedStatuses())
     * - Intersects with workflow-valid transitions
     * (getValidNextStatuses()) - Single status: automatic transition - Multiple
     * statuses: user selection via dialog - No statuses: warning
     * notification, but still saves column assignment CRITICAL BUG FIX: Always save
     * sprint item to persist kanbanColumnId changes, even when no valid
     * status transition exists. Previously, returning early without save caused
     * drag-drop to appear broken (item visually moved but position not
     * persisted to database). This ensures drag-drop respects both kanban column
     * mappings AND workflow transition rules.
     */
    @SuppressWarnings("null")
    private void handleKanbanDrop(final CDragDropEvent event) {
        try {
            ...
            final Object draggedItem = dragStartEvent.getDraggedItems().isEmpty() ? null
                    : dragStartEvent.getDraggedItems().get(0);
    ...
    public void on_kanbanBoard_dragEnd(final Component component, final Object value) {
        LOGGER.debug("Kanban board drag end event received. Active drag item name is {}.",
                getActiveDragStartEvent() != null && !getActiveDragStartEvent().getDraggedItems().isEmpty()
                        ? getActiveDragStartEvent().getDraggedItems().get(0).toString()
                        : "None");
    ...
    /**
     * Saves sprint item to persist kanbanColumnId without changing the project item
     * status. This method is used when drag-drop updates the visual
     * column assignment but there is no valid workflow transition available. The
     * sprint item must still be saved to persist the kanbanColumnId
     * change, otherwise the drag-drop appears broken (item visually moves but
     * position is lost on refresh). CRITICAL: This fixes a bug where
     * returning early without save caused inconsistent behavior - sometimes
     * drag-drop worked (when status transition was valid), sometimes it didn't
    ...
    /**
     * Shows a dialog for the user to select which status to apply when multiple
     * valid statuses exist. This dialog displays all valid statuses with
     * their colors and icons, allowing the user to choose the appropriate status
     * for the transition. If the user cancels, the status is not changed
     ...
     * @param targetStatuses List of valid statuses the user can choose from (must
    ...
    private void showStatusSelectionDialog(final CProjectItem<?> item, final CSprintItem sprintItem,
        ...
        final CDialogKanbanStatusSelection dialog = new CDialogKanbanStatusSelection(targetColumn.getName(),
     ...
     * Similar to showStatusSelectionDialog but for backlog items that are
     * being added to a sprint rather than existing sprint items being moved between
     * columns.
     * 
                    ...
                    if (selectedStatus != null) {
                        ...
                        try {
                            ...
                            componentKanbanBoard.getUI().ifPresent(ui -> ui.access(() -> {
                                ...
                                componentKanbanBoard.reloadSprintItems();
                                componentKanbanBoard.refreshComponent();
                                final CComponentKanbanColumnBacklog backlogColumn = componentKanbanBoard
                                        .getBacklogColumn();
                                if (backlogColumn != null) {
                                    backlogColumn.refreshComponent();
                                }
