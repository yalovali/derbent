...
public class CActivityService extends CProjectItemService<CActivity> implements IEntityRegistrable, IEntityWithView {
    ...
    public void initializeNewEntity(final CActivity entity) {
        ...
        final CProject currentProject = sessionService.getActiveProject()
                .orElseThrow(() -> new CInitializationException(
                        "No active project in session - cannot initialize activity"));
        final CUser currentUser = sessionService.getActiveUser()
                .orElseThrow(
                        () -> new CInitializationException("No active user in session - cannot initialize activity"));

        // Initialize workflow-based status and type
        IHasStatusAndWorkflowService.initializeNewEntity(entity, currentProject, entityTypeService,
        ...
        Check.notEmpty(priorities, "No activity priorities available in project " + currentProject.getName()
                + " - cannot initialize new activity");
    ...
    /**
     * Lists activities by project ordered by sprintOrder for sprint-aware
     * components. Items with null sprintOrder will appear last.
     * 
