...
public class CActivityImportHandler extends CProjectItemImportHandler<CActivity, CActivityType> {
    ...
    @Override
    protected void applyExtraFields(final CActivity entity, final CExcelRow row, final CProject<?> project, final int rowNumber,
            final Map<String, String> rowData) {
        final String priorityName = row.string("priority");
        if (!priorityName.isBlank()) {
            final CActivityPriority priority = priorityService.findByNameAndCompany(priorityName, project.getCompany()).orElse(null);
            if (priority == null) {
