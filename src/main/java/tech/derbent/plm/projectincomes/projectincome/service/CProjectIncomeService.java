...
@PermitAll
public class CProjectIncomeService extends CProjectItemService<CProjectIncome, CProjectIncomeType> implements IEntityRegistrable, IEntityWithView {
	...
	/** Copy CProjectIncome-specific fields from source to target entity. Uses direct setter/getter calls for clarity.
	 * @param source  the source entity to copy from
	 * @param target  the target entity to copy to
	 * @param options clone options controlling what fields to copy */
	@Override
	public void copyEntityFieldsTo(final CProjectIncome source, final CEntityDB<?> target, final CCloneOptions options) {
		super.copyEntityFieldsTo(source, target, options);
		if (!(target instanceof final CProjectIncome targetIncome)) {
			return;
		}
		// Copy basic fields
