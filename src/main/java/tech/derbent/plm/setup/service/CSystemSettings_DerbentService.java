...
/** CSystemSettings_DerbentService - Derbent PLM-specific system settings service. Layer: Service (MVC) Active when: default profile or 'derbent'
 * profile (NOT 'bab' profile) Provides comprehensive configuration management for full-featured PLM environments. Follows the same pattern as
 * CProject_DerbentService. Follows Derbent pattern for system settings services. */
...
public class CSystemSettings_DerbentService extends CSystemSettingsService<CSystemSettings_Derbent> implements IEntityRegistrable, IEntityWithView {
	...
	public CSystemSettings_DerbentService(final ISystemSettings_DerbentRepository repository, final Clock clock,
			final ISessionService sessionService) {
		super(repository, clock, sessionService);
	}

	@Override
	public Class<CSystemSettings_Derbent> getEntityClass() { return CSystemSettings_Derbent.class; }
	...
	protected void validateEntity(final CSystemSettings_Derbent entity) {
		...
		if (entity.getReportGenerationTimeoutMinutes() != null && entity.getReportGenerationTimeoutMinutes() > 60) {
			throw new CValidationException("Report generation timeout cannot exceed 60 minutes");
		}
		if (entity.getAuditLogRetentionDays() != null && entity.getAuditLogRetentionDays() > 2555) {
			throw new CValidationException("Audit log retention cannot exceed 2555 days (7 years)");
		}
		if (entity.getApiRateLimitPerMinute() != null && entity.getApiRateLimitPerMinute() > 10000) {
			throw new CValidationException("API rate limit cannot exceed 10000 requests per minute");
		}
		if (entity.getNotificationBatchSize() != null && entity.getNotificationBatchSize() > 1000) {
			throw new CValidationException("Notification batch size cannot exceed 1000");
		}
