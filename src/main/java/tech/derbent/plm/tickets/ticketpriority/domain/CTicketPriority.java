...
/**
 * CTicketPriority - Domain entity representing ticket priority levels.
 * Provides predefined priority levels for ticket categorization and workflow management.
 * Layer: Domain (MVC)
...
public class CTicketPriority extends CTypeEntity<CTicketPriority> {
	...
	public String toString() {
		return String.format(
				"CTicketPriority{id=%d, name='%s', color='%s', sortOrder=%d, active=%s, company=%s, priorityLevel=%d, isDefault=%s}",
				getId(), getName(), getColor(), getSortOrder(), getActive(), getCompany() != null ? getCompany().getName() : "null",
				priorityLevel, isDefault
		);
	}
}
