...
public class CProductVersionService extends CProjectItemService<CProductVersion> implements IEntityRegistrable, IEntityWithView {
	...
	/** Copy CProductVersion-specific fields from source to target entity. Uses direct setter/getter calls for clarity.
	 * @param source  the source entity to copy from
	 * @param target  the target entity to copy to
	 * @param options clone options controlling what fields to copy */
	@Override
	public void copyEntityFieldsTo(final CProductVersion source, final CEntityDB<?> target, final CCloneOptions options) {
		super.copyEntityFieldsTo(source, target, options);
		if (!(target instanceof final CProductVersion targetVersion)) {
			return;
		}
		// Copy basic fields
