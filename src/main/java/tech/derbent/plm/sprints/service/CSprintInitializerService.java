...
public class CSprintInitializerService extends CProjectItemInitializerService {

	static final Class<?> clazz = CSprint.class;
	private static final Logger LOGGER =
			LoggerFactory.getLogger(CSprintInitializerService.class);
	...
	private static void addSprintItemIfPresent(final CSprint sprint,
			final List<? extends ISprintableItem> items, final int index,
	...
	private static void addSprintItems(final CSprint sprint,
			final List<? extends ISprintableItem> items, final int[] indexes,
			final Long[] storyPoints) {
		if (indexes == null) {
			return;
		}
		for (int i = 0; i < indexes.length; i++) {
			final Long storyPoint =
					storyPoints != null && i < storyPoints.length
							? storyPoints[i] : null;
	...
	public static CDetailSection createBasicView(final CProject<?> project)
		...
		try {
			...
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"description"));
			scr.addScreenLine(
					CDetailLinesService.createLineFromDefaults(clazz, "color"));
			// Scrum Guide 2020 - Sprint Goal & Definition of Done
			scr.addScreenLine(CDetailLinesService
					.createSection("Scrum Guide 2020 - Sprint Artifacts"));
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"sprintGoal"));
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"definitionOfDone"));
			...
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"assignedTo"));
			scr.addScreenLine(
					CDetailLinesService.createSection("Sprint Items"));
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"sprintItems"));
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"itemCount"));
			...
			scr.addScreenLine(CDetailLinesService
					.createSection("Sprint Retrospective (Scrum Guide 2020)"));
			scr.addScreenLine(CDetailLinesService.createLineFromDefaults(clazz,
					"retrospectiveNotes"));
			scr.addScreenLine(CDetailLinesService
	...
	public static void initializeSample(final CProject<?> project,
		...
		try {
			record SprintSeed(String name, String description,
					String sprintGoal, String definitionOfDone,
					String retrospectiveNotes, int startOffsetWeeks,
					int durationDays, Integer velocity, int[] userStoryIndexes,
					Long[] userStoryPoints, int[] activityIndexes,
					Long[] activityPoints, int[] issueIndexes,
					Long[] issuePoints, int[] featureIndexes,
					Long[] featurePoints) {}
			final CSprintService sprintService =
					CSpringContext.getBean(CSprintService.class);
			...
			final List<CSprintType> availableTypes =
					sprintTypeService.listByCompany(project.getCompany());
			...
			final String standardDefinitionOfDone =
			...
			if (minimal) {
				...
				final List<CUserStory> userStories =
						userStoryService.listByProject(project);
				...
				final CSprintType sprintType = availableTypes.isEmpty()
						? sprintTypeService.getRandom(project.getCompany())
						: availableTypes.get(0);
				final CUser assignedUser = availableUsers.isEmpty()
						? userService.getRandom(project.getCompany())
						: availableUsers.get(0);
				final CSprint sprint =
						new CSprint("Sprint 1 - Identity Kickoff", project);
				sprint.setDescription(
						"Initial delivery sprint for the identity modernization stream.");
				...
				sprint.setSprintGoal(
						"Deliver the first working MFA enrollment slice and prove audit-ready session controls.");
				...
				final CUserStoryService userStoryService =
						CSpringContext.getBean(CUserStoryService.class);
				final CFeatureService featureService =
						CSpringContext.getBean(CFeatureService.class);
				final CIssueService issueService =
						CSpringContext.getBean(CIssueService.class);
				final List<CUserStory> userStories =
						userStoryService.listByProject(project);
				final List<CFeature> features =
						featureService.listByProject(project);
				final List<CActivity> activities =
						activityService.listByProject(project);
				final List<CIssue> issues = issueService.listByProject(project);
				final List<SprintSeed> sprintSeeds = List.of(new SprintSeed(
						...
						new SprintSeed("Sprint 3 - Billing Operations",
								...
								standardDefinitionOfDone, "", 4, 13, 18,
								new int[] {
										4, 5
								}, new Long[] {
										8L, 5L
						...
						new SprintSeed("Sprint 4 - Release Hardening",
								...
								standardDefinitionOfDone, "", 6, 13, 13,
								new int[] {
										6
								}, new Long[] {
										3L
				...
				for (final SprintSeed seed : sprintSeeds) {
					final CSprintType sprintType = availableTypes.isEmpty()
							? sprintTypeService.getRandom(project.getCompany())
							: availableTypes
									.get(sprintIndex % availableTypes.size());
					final CUser assignedUser = availableUsers.isEmpty()
							? userService.getRandom(project.getCompany())
							: availableUsers
									.get(sprintIndex % availableUsers.size());
					...
					sprint.setColor(CSprint.DEFAULT_COLOR);
					sprint.setStartDate(
							baseSprintStart.plusWeeks(seed.startOffsetWeeks()));
					sprint.setEndDate(sprint.getStartDate()
							.plusDays(seed.durationDays()));
					...
					final CSprint savedSprint = sprintService.save(sprint);
					addSprintItems(savedSprint, userStories,
							seed.userStoryIndexes(), seed.userStoryPoints());
					addSprintItems(savedSprint, activities,
							seed.activityIndexes(), seed.activityPoints());
					addSprintItems(savedSprint, issues, seed.issueIndexes(),
							seed.issuePoints());
					addSprintItems(savedSprint, features, seed.featureIndexes(),
							seed.featurePoints());
					sprintService.save(savedSprint);
					sprintIndex++;
				}
			...
			LOGGER.error("Error creating sample sprints reason={}",
					e.getMessage());
			throw e;
		}
	}
