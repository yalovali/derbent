...
public class CComponentSprintPlanningBoard extends CComponentBase<CSprintPlanningViewEntity> {
	...
	public CComponentSprintPlanningBoard(final ISessionService sessionService) {
		...
		backlogBrowser = new CSprintPlanningBacklogBrowser(dragContext, this::onItemSelected, this::onBacklogDrop, this::onBacklogParentDrop,
		...
		backlogBrowser.setParentItemDoubleClickHandler(item -> openProjectItemEditDialog(item != null ? item.getEntity() : null));
		backlogBrowser.setLeafItemDoubleClickHandler(item -> openProjectItemEditDialog(item != null ? item.getEntity() : null));
	...
	private List<CContextActionDefinition<CGnntItem>> buildLeafQuickActions() {
		return List.of(
				...
				CContextActionDefinition.of("new-leaf-item", "New", VaadinIcon.PLUS_CIRCLE_O, context -> true, this::canCreateLeafItem,
						this::openCreateLeafItemDialog),
				CContextActionDefinition.of("copy-to", "Copy To...", VaadinIcon.COPY, context -> context != null, this::canCopyTo,
						this::openCopyToDialog),
				CContextActionDefinition.of("assign-to", "Assign To...", VaadinIcon.USER, context -> context != null, this::canAssignTo,
						this::openAssignToDialog),
				CContextActionDefinition.of("assign-to-me", "Assign To Me", VaadinIcon.USER, context -> context != null, this::canAssignTo,
						this::assignToMe),
				CContextActionDefinition.of("set-status", "Set Status...", VaadinIcon.CLIPBOARD_CHECK, context -> context != null, this::canSetStatus,
						this::openStatusDialog),
				CContextActionDefinition.of("delete-item", "Delete", VaadinIcon.TRASH, context -> context != null, this::canDeleteItem,
						this::deleteBacklogItem),
				CContextActionDefinition.of("edit-leaf-item", "Edit", VaadinIcon.EDIT, context -> context != null, context -> context != null,
	...
	private List<CContextActionDefinition<CGnntItem>> buildParentQuickActions() {
		return List.of(
				CContextActionDefinition.of("new-parent-item", "New", VaadinIcon.PLUS_CIRCLE_O, context -> true, this::canCreateParentItem,
						this::openCreateParentItemDialog),
				CContextActionDefinition.of("new-leaf-item", "New child", VaadinIcon.PLUS, context -> true, this::canCreateLeafItem,
						this::openCreateLeafItemDialog),
				...
				CContextActionDefinition.of("delete-item", "Delete", VaadinIcon.TRASH, context -> context != null, this::canDeleteItem,
						this::deleteBacklogItem),
				CContextActionDefinition.of("edit-parent-item", "Edit", VaadinIcon.EDIT, context -> context != null, context -> context != null,
						context -> openProjectItemEditDialog(context != null ? context.getEntity() : null)));
	}
		...
		return List.of(
				...
				CContextActionDefinition.of("edit-sprint", "Edit sprint details", VaadinIcon.CALENDAR_CLOCK,
						...
						context -> openEditSprintDialog(
								context != null && context.getEntity() instanceof CSprint ? (CSprint) context.getEntity() : null)));
	}
	...
	private CGnntHierarchyResult buildSprintHierarchy(final CSprintPlanningViewEntity view, final Map<String, CProjectItem<?>> entitiesByKey) {
		...
		for (final CProjectItem<?> entity : entitiesByKey.values()) {
			...
			final CSprintPlanningSprintMetrics current = metricsBySprintId.getOrDefault(sprint.getId(), new CSprintPlanningSprintMetrics(0, 0, 0, 0));
			metricsBySprintId.put(sprint.getId(), new CSprintPlanningSprintMetrics(current.itemDoneCount() + (done ? 1 : 0),
					current.itemTotalCount() + 1, current.storyPointsDone() + (done ? points : 0), current.storyPointsTotal() + points));
		}
	...
	private CBacklogBuildResult buildVisibleBacklogParentNode(final CProjectItem<?> entity, final int hierarchyLevel,
		...
		final boolean matchesFilters = isBacklogCandidate(entity, scope) && filterToolbar.shouldIncludeBacklogParentItem(entity);
	...
	private Map<String, CSprintPlanningSprintMetrics> computeBacklogParentRollups(final List<CGnntItem> visibleLeafItems,
			final Map<String, CProjectItem<?>> hierarchyItemsByKey) {
		if (visibleLeafItems == null || visibleLeafItems.isEmpty() || hierarchyItemsByKey == null || hierarchyItemsByKey.isEmpty()) {
			return Map.of();
		}
		...
		for (final CGnntItem leafItem : visibleLeafItems) {
			...
			while (parentKey != null) {
				...
				final CSprintPlanningSprintMetrics current = rollups.getOrDefault(parentKey, new CSprintPlanningSprintMetrics(0, 0, 0, 0));
				rollups.put(parentKey, new CSprintPlanningSprintMetrics(current.itemDoneCount() + (done ? 1 : 0), current.itemTotalCount() + 1,
						current.storyPointsDone() + (done ? points : 0), current.storyPointsTotal() + points));
	...
	private void deleteBacklogItem(final CGnntItem context) {
		try {
			...
			final String displayName = entity instanceof final CEntityNamed<?> named ? named.getName() : String.valueOf(entity.getId());
	...
	private void deleteEntity(final CEntityDB<?> entity) {
		...
		if (serviceClass == null) {
			throw new IllegalStateException("No service registered for entity type: %s".formatted(entityClass.getSimpleName()));
		}
		final Object serviceBean = CSpringContext.getBean(serviceClass);
		if (!(serviceBean instanceof CAbstractService service)) {
			throw new IllegalStateException("Registered service is not a CAbstractService: %s".formatted(serviceClass.getSimpleName()));
		}
		service.delete(entity.getId());
	}

	private void extracted(Object copiedEntity) {
		try {
			saveEntity((CEntityDB<?>) copiedEntity);
			refreshComponent();
			final String displayName =
					copiedEntity instanceof final CEntityNamed<?> named ? named.getName() : copiedEntity.getClass().getSimpleName();
	...
	private void onBacklogParentDrop(final CGnntItem draggedItem, final CGnntItem dropTarget) {
		try {
			...
			if (!hierarchyNavigationService.isValidParentCandidate(child, parent)) {
				CNotificationService.showWarning("'%s' cannot be placed under '%s'".formatted(child.getName(), parent.getName()));
				return;
			}
	...
	private void openAssignToDialog(final CGnntItem context) {
		try {
			...
			final CDialogEntitySelection<CUser> dialog =
					new CDialogEntitySelection<>("Assign To", types, config -> userService.listByProject(view.getProject()), selected -> {
	...
	private void openCopyToDialog(final CGnntItem context) {
		try {
			...
			final CDialogClone dialog = new CDialogClone(entity, copiedEntity -> extracted(copiedEntity));
			dialog.open();
		} catch (final Exception e) {
	...
	private void openStatusDialog(final CGnntItem context) {
		...
		try {
			...
			final CDialogKanbanStatusSelection dialog = new CDialogKanbanStatusSelection("Workflow", statuses, selectedStatus -> {
	...
	protected void refreshComponent() {
		try {
			...
			backlogBrowser.setParentRollupSummaries(computeBacklogParentRollups(backlogData.leafHierarchy().getFlatItems(), hierarchyItemsByKey));
	...
	private CEntityDB<?> resolveEntityContext(final CGnntItem context) {
		...
		return effectiveContext != null && effectiveContext.getEntity() instanceof CEntityDB<?> ? (CEntityDB<?>) effectiveContext.getEntity() : null;
	}
	...
	private CProjectItem<?> resolveProjectItemContext(final CGnntItem context) {
		final CGnntItem effectiveContext = resolveEffectiveContext(context);
		return effectiveContext != null && effectiveContext.getEntity() instanceof CProjectItem<?> ? (CProjectItem<?>) effectiveContext.getEntity()
				: null;
	}
