...
/**
 * CKanbanService - Interface for services that provide data for Kanban boards. Layer: Service Interface (Abstraction)
 * 
 * Provides the contract for services that can supply entities grouped by status for kanban board display.
 * 
 * @param <T>
 *            the type of entity this service manages
 * @param <S>
 *            the type of status this service works with
 */
public interface CKanbanService<T extends CKanbanEntity, S extends CKanbanStatus> {
     ...
     * 
     * @param projectId
     *            the ID of the project to get entities for
