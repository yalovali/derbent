...
/**
 * CDataProviderResolver - Service for automatically resolving data providers for ComboBox components based on MetaData
 * annotations. This service integrates with Spring's application context to find appropriate service beans and call
 * their data retrieval methods.
 * <p>
 * The resolver supports multiple ways to specify data providers:
 * </p>
 * <ul>
 * <li><strong>Bean Name:</strong> Uses {@code @MetaData(dataProviderBean = "serviceName")}</li>
 * <li><strong>Bean Type:</strong> Uses {@code @MetaData(dataProviderClass = ServiceClass.class)}</li>
 ...
 * </ol>
 * <p>
 * <strong>Caching:</strong> The resolver caches method lookups for performance but not the actual data, ensuring fresh
 * data is always retrieved from services.
 * </p>
 * <p>
 ...
 *     // Using custom method
 *     @MetaData(displayName = "Active Users", dataProviderBean = "userService", dataProviderMethod = "findAllActive")
...
public final class CDataProviderResolver {
     ...
     *            the Spring application context for bean lookup
     */
    @Autowired
     ...
     * @param methodName
     *            the preferred method name to call
     * @param entityType
     *            the entity class for logging purposes
     * @return list of entities from the method call
     */
    private <T extends CEntityDB<T>> List<T> callDataMethod(final Object serviceBean, final String methodName,
        ...
        LOGGER.debug("Calling data method on bean '{}' with preferred method: '{}' for entity type: {}", beanClassName,
        ...
        for (final String currentMethodName : methodsToTry) {
            ...
            try {
                ...
                if (result != null) {
                    LOGGER.debug("Successfully called method '{}' with Pageable on bean '{}' - returned {} items",
                ...
                if (resultNoParams != null) {
                    LOGGER.debug("Successfully called method '{}' without parameters on bean '{}' - returned {} items",
                            currentMethodName, beanClassName, resultNoParams.size());
                    return resultNoParams;
                }
            } catch (final Exception e) {
                LOGGER.debug("Method '{}' failed on bean '{}': {} - trying next method", currentMethodName,
                        beanClassName, e.getMessage());
            }
        }
        LOGGER.error("No suitable data retrieval method found on bean '{}' for entity type: {}", beanClassName,
     ...
     *            the cache key
     * @param beanSupplier
     *            supplier to compute the bean if not cached
     * @return the bean or null if not found
     */
    private Object getBeanFromCache(final String cacheKey, final java.util.function.Supplier<Object> beanSupplier) {
    ...
    public String getCacheStats() {
        return String.format("CDataProviderResolver - Method cache: %d entries, Bean cache: %d entries",
     ...
     *            supplier to compute the method if not cached
     ...
     * <p>
     * This method analyzes the MetaData annotation to determine the appropriate data source and method to call. It
     * supports various configuration options and fallback mechanisms.
     * </p>
     * 
     * @param <T>
     *            the entity type for the ComboBox items
     * @param entityType
     *            the class type of entities to retrieve
     * @param metaData
     *            the MetaData annotation containing provider configuration
     * @return list of entities for the ComboBox, never null but may be empty
     * @throws IllegalArgumentException
     *             if parameters are null
     */
    public <T extends CEntityDB<T>> List<T> resolveData(final Class<T> entityType, final MetaData metaData) {
        ...
        if (metaData == null) {
            LOGGER.error("MetaData parameter is null for entity type: {} - cannot resolve data",
                    entityType.getSimpleName());
            throw new IllegalArgumentException("MetaData cannot be null");
        }
        LOGGER.debug("Resolving data provider for entity type: {} with MetaData configuration",
                entityType.getSimpleName());
        ...
        try {
            ...
            if ((metaData.dataProviderBean() != null) && !metaData.dataProviderBean().trim().isEmpty()) {
                LOGGER.debug("Using specified bean name: '{}' for entity type: {}", metaData.dataProviderBean(),
                        entityType.getSimpleName());
            ...
            if ((metaData.dataProviderClass() != null) && (metaData.dataProviderClass() != Object.class)) {
                LOGGER.debug("Using specified bean class: '{}' for entity type: {}",
                        metaData.dataProviderClass().getSimpleName(), entityType.getSimpleName());
            ...
            LOGGER.error("Failed to resolve data for entity type: {} - returning empty list. Error: {}",
     ...
     *            the class type of entities to retrieve
     * @param methodName
     *            the method name to call on the bean
     * @return list of entities from automatically resolved service
     */
    private <T extends CEntityDB<T>> List<T> resolveDataAutomatically(final Class<T> entityType,
            final String methodName) {
        LOGGER.debug("Attempting automatic resolution for entity type: {} using method: {}", entityType.getSimpleName(),
                methodName);
        final String entityName = entityType.getSimpleName();
        // Try different naming conventions for service beans
        final String[] possibleBeanNames = { entityName + "Service",
                Character.toLowerCase(entityName.charAt(0)) + entityName.substring(1) + "Service",
        ...
        for (final String beanName : possibleBeanNames) {
            ...
            try {
                ...
                LOGGER.debug("Failed to use bean '{}' for entity type: {} - trying next option", beanName, entityName);
            }
        }
     ...
     *            the class type of entities to retrieve
     * @param beanName
     *            the Spring bean name to use
     * @param methodName
     *            the method name to call on the bean
     * @return list of entities from the specified bean
     */
    private <T extends CEntityDB<T>> List<T> resolveDataFromBean(final Class<T> entityType, final String beanName,
            final String methodName) {
        LOGGER.debug("Resolving data from bean '{}' using method '{}' for entity type: {}", beanName, methodName,
                entityType.getSimpleName());
        ...
        try {
            ...
            if (serviceBean == null) {
                LOGGER.error("Failed to retrieve bean '{}' from Spring context", beanName);
                return Collections.emptyList();
            }
     ...
     *            the class type of entities to retrieve
     * @param serviceClass
     *            the Spring bean class type to use
     * @param methodName
     *            the method name to call on the bean
     * @return list of entities from the specified bean type
     */
    private <T extends CEntityDB<T>> List<T> resolveDataFromClass(final Class<T> entityType,
            final Class<?> serviceClass, final String methodName) {
        LOGGER.debug("Resolving data from bean class '{}' using method '{}' for entity type: {}",
        ...
        try {
            ...
            final Object serviceBean = getBeanFromCache(cacheKey, () -> {
                ...
                try {
                    ...
                    LOGGER.warn("Bean of type '{}' not found in application context: {}", serviceClass.getSimpleName(),
                            e.getMessage());
                    return null;
                }
            });
     ...
     * @param entityType
     *            the entity type for caching
     * @return list of entities or null if method not found/failed
     */
    @SuppressWarnings("unchecked")
    private <T extends CEntityDB<T>> List<T> tryMethodWithoutParams(final Object serviceBean, final String methodName,
            final Class<T> entityType) {
        final String cacheKey = serviceBean.getClass().getName() + ":" + methodName + ":noparams:"
                + entityType.getSimpleName();
     ...
     */
    @SuppressWarnings("unchecked")
    private <T extends CEntityDB<T>> List<T> tryMethodWithPageable(final Object serviceBean, final String methodName,
            final Class<T> entityType) {
        final String cacheKey = serviceBean.getClass().getName() + ":" + methodName + ":pageable:"
                + entityType.getSimpleName();
        ...
        try {
            ...
            if (method != null) {
                ...
                if (result instanceof org.springframework.data.domain.Page) {
                    return ((org.springframework.data.domain.Page<T>) result).getContent();
