...
/**
 * CDataProviderResolver - Service for automatically resolving data providers for ComboBox components
 * based on MetaData annotations. This service integrates with Spring's application context to
 * find appropriate service beans and call their data retrieval methods.
 * 
 * <p>
 ...
 * 
 * <p>
 * <strong>Method Resolution:</strong>
 * The resolver tries to find methods in the following order:
 * </p>
 * <ol>
 ...
 * The resolver caches method lookups for performance but not the actual data,
 * ensuring fresh data is always retrieved from services.
 * </p>
 * 
 * <p>
...
public final class CDataProviderResolver {
    ...
    /**
     * Cache for resolved methods to improve performance.
     * Key format: "beanName:methodName:entityType"
     */
    private final Map<String, Method> methodCache = new ConcurrentHashMap<>();

    /**
     * Cache for resolved bean instances to improve performance.
     * Key format: "beanName" or "className"
     */
     ...
     * 
     * <p>
     * This method analyzes the MetaData annotation to determine the appropriate data source
     * and method to call. It supports various configuration options and fallback mechanisms.
     * </p>
     * 
    ...
    public <T extends CEntityDB> List<T> resolveData(final Class<T> entityType, final MetaData metaData) {
    ...
    private <T extends CEntityDB> List<T> resolveDataFromBean(final Class<T> entityType, 
        ...
        LOGGER.debug("Resolving data from bean '{}' using method '{}' for entity type: {}", 
    ...
    private <T extends CEntityDB> List<T> resolveDataFromClass(final Class<T> entityType, 
        ...
        try {
            ...
            LOGGER.error("Error resolving data from bean class '{}': {}", serviceClass.getSimpleName(), 
                    e.getMessage(), e);
            return Collections.emptyList();
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private <T extends CEntityDB> List<T> resolveDataAutomatically(final Class<T> entityType, 
            final String methodName) {
        
        LOGGER.debug("Attempting automatic resolution for entity type: {} using method: {}", 
        ...
        for (final String beanName : possibleBeanNames) {
            LOGGER.debug("Trying bean name: '{}' for entity type: {}", beanName, entityName);
            
            try {
                if (applicationContext.containsBean(beanName)) {
                    LOGGER.debug("Found bean '{}' for entity type: {}", beanName, entityName);
                    return resolveDataFromBean(entityType, beanName, methodName);
                }
            } catch (final Exception e) {
                LOGGER.debug("Failed to use bean '{}' for entity type: {} - trying next option", 
                        beanName, entityName);
            }
        }
    ...
    private <T extends CEntityDB> List<T> callDataMethod(final Object serviceBean, 
        ...
        LOGGER.debug("Calling data method on bean '{}' with preferred method: '{}' for entity type: {}", 
        ...
        LOGGER.error("No suitable data retrieval method found on bean '{}' for entity type: {}", 
