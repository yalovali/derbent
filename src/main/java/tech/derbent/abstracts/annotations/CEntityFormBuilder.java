...
public final class CEntityFormBuilder implements ApplicationContextAware {
     ...
     *            the entity class
     * @param dataProvider
     *            the data provider for ComboBox components
     * @return a Div containing the form with enhanced binder
     */
    @SuppressWarnings("unchecked")
    @Deprecated
    ...
    public static <EntityClass> Div buildForm(final Class<?> entityClass, final CEnhancedBinder<EntityClass> binder) {
    ...
    public static <EntityClass> Div buildForm(final Class<?> entityClass, final CEnhancedBinder<EntityClass> binder,
        ...
        for (final Field field : sortedFields) {
            ...
            processedComponents = processMetaForField(binder, formLayout, processedComponents, field);
        }
     ...
     */
    @Deprecated
    public static <EntityClass> Div buildFormAll(final Class<?> entityClass, final CEnhancedBinder<EntityClass> binder,
            final ComboBoxDataProvider dataProvider) {
        return buildForm(entityClass, binder, null);
    }
    ...
    private static List<String> callStringDataMethod(final Object serviceBean, final String methodName,
        ...
        try {
            ...
            final java.lang.reflect.Method method = serviceBean.getClass().getMethod(methodName);
    ...
    private static NumberField createIntegerField(final Field field, final MetaData meta,
        ...
        try {
            ...
            LOGGER.error("Failed to bind integer field for field '{}': {} - using fallback binding", field.getName(),
                    e.getMessage());
            ...
            try {
                ...
                LOGGER.error("Fallback binding also failed for integer field '{}': {}", field.getName(),
                        fallbackException.getMessage());
            }
        }
        return numberField;
    ...
    private static <EntityClass> int processMetaForField(final CEnhancedBinder<EntityClass> binder,
            final VerticalLayout formLayout, int processedComponents, final Field field) {
        Check.notNull(field, "field");
    ...
    private static List<String> resolveStringData(final MetaData meta, final String fieldName) {
        ...
        // For basic implementation, return empty list if no applicationContext is
        // available
        ...
        try {
            ...
            if (!applicationContext.containsBean(beanName)) {
                LOGGER.warn("Data provider bean '{}' not found in Spring context for field '{}'", beanName, fieldName);
                return List.of();
            }
            ...
            LOGGER.error("Error resolving String data for field '{}' from bean '{}': {}", fieldName, beanName,
                    e.getMessage());
            return List.of();
        }
    }
