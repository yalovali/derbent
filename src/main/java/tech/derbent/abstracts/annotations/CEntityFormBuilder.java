...
public final class CEntityFormBuilder implements ApplicationContextAware {
	...
	private static List<String> callStringDataMethod(final Object serviceBean,
		...
		try {
			...
			final java.lang.reflect.Method method =
				serviceBean.getClass().getMethod(methodName);
			...
			if (result instanceof List) {
				...
				LOGGER.warn("Method '{}' did not return a List for String field '{}'",
					methodName, fieldName);
				return List.of();
			}
		} catch (final Exception e) {
			LOGGER.debug("Failed to call method '{}' for String field '{}': {}",
	...
	/**
	 * Creates a ComboBox for String fields with data provider configuration. This method
	 * handles String fields that should be rendered as ComboBox instead of TextField when
	 * metadata specifies a data provider.
	 ...
	 */
	private static List<String> resolveStringData(final MetaData meta,
		...
		if (applicationContext == null) {
			LOGGER.warn(
				"No Spring ApplicationContext available for String data resolution for field '{}'",
				fieldName);
			return List.of();
		}
		...
		if ((beanName == null) || beanName.trim().isEmpty()) {
			LOGGER.debug("No dataProviderBean specified for String field '{}'",
				fieldName);
			return List.of();
		}
		...
		try {
			...
			if (!applicationContext.containsBean(beanName)) {
				LOGGER.warn(
					"Data provider bean '{}' not found in Spring context for field '{}'",
					beanName, fieldName);
				return List.of();
			}
			final Object serviceBean = applicationContext.getBean(beanName);
			LOGGER.debug("Retrieved data provider bean '{}' for String field '{}'",
				beanName, fieldName);
