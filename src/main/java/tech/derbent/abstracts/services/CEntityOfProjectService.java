...
public abstract class CEntityOfProjectService<EntityClass extends CEntityOfProject<EntityClass>>
    ...
    public long countByProject(final CProject project) {
        LOGGER.debug("countByProject called with project: {} for {}", project != null ? project.getName() : "null",
    ...
    /**
     * Finds entities by project with all relationships eagerly loaded. This is the standard method for project-based
     * queries that loads all necessary relationships to prevent LazyInitializationException.
     * 
     * @param project
     *            the project to find entities for
    ...
    public List<EntityClass> findEntriesByProject(final CProject project) {
        ...
        try {
            final List<EntityClass> entities = ((CEntityOfProjectRepository<EntityClass>) repository)
    ...
    /**
     * Finds entities by project with pagination support. This method supports grid functionality while maintaining
    ...
    /**
     * Gets an entity by ID with all relationships eagerly loaded. This is the standard method for single entity
     * retrieval that loads all necessary relationships to prevent LazyInitializationException.
     * 
     * @param id
     *            the entity ID
    ...
    /**
     * Minimal lazy field initialization for CEntityOfProject entities. With eager loading of small entities (status,
     * type, user references), this mainly handles any remaining complex relationships.
     * 
     * @param entity
    ...
    public EntityClass save(final EntityClass entity) {
        ...
        if (existing.isPresent()) {
            LOGGER.error("save(entity={}) - Entity with name '{}' already exists in project {}", entity.getId(),
                    trimmedName, entity.getProject().getName());
            throw new IllegalArgumentException("Entity with name '" + trimmedName + "' already exists in project '"
