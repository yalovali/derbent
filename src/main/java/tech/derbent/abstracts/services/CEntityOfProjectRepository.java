...
public interface CEntityOfProjectRepository<EntityClass extends CEntityOfProject<EntityClass>>
    ...
    /**
     * Finds an entity by ID with eagerly loaded project relationship to prevent LazyInitializationException. This base
     * implementation loads common CEntityOfProject relationships.
     * 
     * @param id
     *            the entity ID
     * @return optional entity with loaded relationships
     */
    @Query("SELECT e FROM #{#entityName} e " + "LEFT JOIN FETCH e.project " + "LEFT JOIN FETCH e.assignedTo "
            + "LEFT JOIN FETCH e.createdBy " + "WHERE e.id = :id")
    Optional<EntityClass> findByIdWithProjectRelationships(@Param("id") Long id);

    /**
     * Finds entities by project with eagerly loaded project relationship to prevent LazyInitializationException. This
     * is the base implementation that all CEntityOfProject repositories should use unless they need additional
     ...
     */
    @Query("SELECT e FROM #{#entityName} e " + "LEFT JOIN FETCH e.project " + "LEFT JOIN FETCH e.assignedTo "
            + "LEFT JOIN FETCH e.createdBy " + "WHERE e.project = :project")
