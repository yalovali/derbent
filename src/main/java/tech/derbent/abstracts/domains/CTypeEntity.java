...
/**
 * CTypeEntity - Abstract base class for all type entities in the system.
 * Provides common fields for type management including color, sort order, and active status.
 * Layer: Domain (MVC)
 * 
 * @author Derbent Team
 * @since 1.0
...
public abstract class CTypeEntity extends CEntityOfProject {
	...
	public CTypeEntity() {
		super();
		LOGGER.debug("CTypeEntity default constructor called");
	}
	...
	public CTypeEntity(final String name, final CProject project) {
		super(name, project);
		LOGGER.debug("CTypeEntity constructor called with name: {} and project: {}", name, project);
	}
	...
	public String toString() {
		return String.format("%s{id=%d, name='%s', color='%s', sortOrder=%d, isActive=%s, project=%s}",
