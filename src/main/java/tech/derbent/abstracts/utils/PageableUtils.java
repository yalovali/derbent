...
/**
 * Utility class for safely creating and validating Pageable objects.
 * Prevents the "max-results cannot be negative" Spring Data exception.
 * 
...
public final class PageableUtils {
     ...
     */
    public static Pageable createSafe(int page, int size) {
     ...
     * @throws IllegalArgumentException if page or size is negative
     */
    public static Pageable createSafe(int page, int size, Sort sort) {
        if (page < 0) {
        ...
        if (size == 0) {
            LOGGER.warn("Page size is 0, using default size: {}", DEFAULT_PAGE_SIZE);
            size = DEFAULT_PAGE_SIZE;
        }
        
        if (size > MAX_PAGE_SIZE) {
            LOGGER.warn("Page size {} exceeds maximum {}, using maximum", size, MAX_PAGE_SIZE);
            size = MAX_PAGE_SIZE;
        }
    ...
    public static Pageable createDefault() {
        return createSafe(0, MAX_PAGE_SIZE);
    }
     ...
     */
    public static Pageable validateAndFix(Pageable pageable) {
        if (pageable == null) {
        ...
        if (size < 0) {
            LOGGER.warn("Fixing negative page size {} to default {}", size, DEFAULT_PAGE_SIZE);
            size = DEFAULT_PAGE_SIZE;
            needsFix = true;
        }
        
        if (size == 0) {
            LOGGER.warn("Fixing zero page size to default {}", DEFAULT_PAGE_SIZE);
            size = DEFAULT_PAGE_SIZE;
            needsFix = true;
        }
