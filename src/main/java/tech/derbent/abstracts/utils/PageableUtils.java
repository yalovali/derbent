...
/**
 * Utility class for safely creating and validating Pageable objects. Prevents the
 * "max-results cannot be negative" Spring Data exception.
...
public final class PageableUtils {
	...
	public static Pageable createSafe(final int page, int size, final Sort sort) {
		...
		if (size == 0) {
			LOGGER.warn("Page size is 0, using default size: {}",
				CEntityConstants.DEFAULT_PAGE_SIZE);
			size = CEntityConstants.DEFAULT_PAGE_SIZE;
		}

		if (size > CEntityConstants.MAX_PAGE_SIZE) {
			LOGGER.warn("Page size {} exceeds maximum {}, using maximum", size,
				CEntityConstants.MAX_PAGE_SIZE);
	...
	public static Pageable validateAndFix(final Pageable pageable) {
		...
		if (size < 0) {
			LOGGER.warn("Fixing negative page size {} to default {}", size,
				CEntityConstants.DEFAULT_PAGE_SIZE);
			size = CEntityConstants.DEFAULT_PAGE_SIZE;
			needsFix = true;
		}
		...
		if (size == 0) {
			LOGGER.warn("Fixing zero page size to default {}",
				CEntityConstants.DEFAULT_PAGE_SIZE);
			size = CEntityConstants.DEFAULT_PAGE_SIZE;
			needsFix = true;
		}
