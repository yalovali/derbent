...
/**
 * Utility class for handling enhanced validation errors and providing detailed error
 * reporting for form binding operations.
 */
public class CValidationUtils {
	...
	public static String
		getValidationErrorSummary(final CEnhancedBinder<?> enhancedBinder) {
	...
	/**
	 * Handles validation exceptions with enhanced error reporting. Shows detailed
	 * field-level errors if using an enhanced binder.
	 * @param binder         the binder that threw the exception
	 * @param exception      the validation exception
	 * @param entityTypeName the name of the entity type for logging
	 */
	public static void handleValidationException(final CEnhancedBinder<?> binder,
		final ValidationException exception, final String entityTypeName) {
		LOGGER.error("Validation error for {}: {}", entityTypeName,
	...
	/**
	 * Checks if a binder has validation errors (works with both standard and enhanced
	 * binders).
	 ...
	 */
	public static void logDetailedErrors(final CEnhancedBinder<?> enhancedBinder,
		...
		errors.forEach((field, error) -> LOGGER.error("Field validation error - {}: {}",
	 ...
	 * @param entityTypeName the entity type name for context
	 */
	public static void showEnhancedValidationDialog(
		final CEnhancedBinder<?> enhancedBinder, final String entityTypeName) {
		...
		errors.forEach((field, error) -> {
			message.append("• ").append(formatFieldName(field)).append(": ").append(error)
				.append("\n");
		});
	 ...
	 * @param entityTypeName the entity type name for context
	 */
	public static void showEnhancedValidationNotification(
		final CEnhancedBinder<?> enhancedBinder, final String entityTypeName) {
		...
		final String message =
			String.format("Validation failed for %s in %d field(s): %s", entityTypeName,
				fieldsWithErrors.size(), String.join(", ", fieldsWithErrors));
	 ...
	 */
	public static <T> boolean validateBean(final CEnhancedBinder<T> enhancedBinder,
		final T bean) {
