...
public class CEntityProjectsGrid<T extends CEntityDB<T>> extends VerticalLayout {
    ...
    /**
     * Restores grid selection to the specified project after refresh. This prevents losing the current selection when
    ...
    private void restoreProjectSelection(final CProject project) {
        ...
        try {
            ...
            getProjects.get().stream().filter(p -> project.getId().equals(p.getId())).findFirst()
                    .ifPresentOrElse(matchedProject -> {
