...
public abstract class CAbstractEntityDBPage<EntityClass extends CEntityDB<EntityClass>> extends CAbstractPage
    ...
    protected void createGridLayout() {
        ...
        if (CSearchable.class.isAssignableFrom(entityClass)) {
            searchToolbar = new CSearchToolbar(
                    "Search " + entityClass.getSimpleName().replace("C", "").toLowerCase() + "...");
        ...
        // searching
        grid.setItems(query -> {
            LOGGER.debug("Grid query - offset: {}, limit: {}, sortOrders: {}, searchText: '{}'", query.getOffset(),
                    query.getLimit(), query.getSortOrders(), currentSearchText);
    ...
    public CEnhancedBinder<EntityClass> getBinder() {
        return binder;
    }
