...
public abstract class CAbstractEntityDBPage<EntityClass extends CEntityDB<EntityClass>> extends CAbstractPage
    ...
    public void beforeEnter(final BeforeEnterEvent event) {
        ...
        if (entityID.isPresent()) {
            final Optional<EntityClass> entityByRoute = entityService.getById(entityID.get());
            LOGGER.debug("Entity ID in URL: {}, looking up entity: {}", entityID.get(), entityByRoute);
            ...
            if (entityByRoute.isPresent()) {
                ...
                if (grid != null) {
                    ...
                    // it it necassary
                    sessionService.setActiveId(entityClass.getClass().getSimpleName(), entityByRoute.get().getId());
                    populateForm(entityByRoute.get());
                }
            } else {
                Notification.show(String.format("The requested samplePerson was not found, ID = %s", entityID.get()),
            ...
            LOGGER.debug("No entity ID in URL, trying to select last selected entity ID: {}",
    ...
    /**
     * Creates a button layout for additional buttons if needed by subclasses. Note: Default save/delete/cancel buttons
     * are now in the details tab. This method can be used for additional custom buttons in the main content area.
     * 
     * @param layout
     *            The layout to add buttons to
     */
    ...
    /**
     * Creates the button layout for the details tab. Contains new, save, cancel, and delete buttons with consistent
     * styling.
     * 
    ...
    /**
     * Creates the default details view tab content with save/delete/cancel buttons. Subclasses can override this method
     * to customize the tab content while maintaining consistent button placement and styling.
     */
    ...
    protected void createGridLayout() {
        ...
        // safe validation
        grid.setItems(query -> {
            LOGGER.debug("Grid query - offset: {}, limit: {}, sortOrders: {}", query.getOffset(), query.getLimit(),
                    query.getSortOrders());

            try {
                // Convert Vaadin query to Spring Pageable using VaadinSpringDataHelpers
                final org.springframework.data.domain.Pageable originalPageable = VaadinSpringDataHelpers
                        .toSpringPageRequest(query);
                // Validate and fix the pageable to prevent "max-results cannot be
                // negative" error
                final org.springframework.data.domain.Pageable safePageable = PageableUtils
                        .validateAndFix(originalPageable);
                LOGGER.debug("Safe Pageable - pageNumber: {}, pageSize: {}, sort: {}", safePageable.getPageNumber(),
                        safePageable.getPageSize(), safePageable.getSort());
    ...
    public HasOrderedComponents getBaseDetailsLayout() {
        return baseDetailsLayout;
    }

    public BeanValidationBinder<EntityClass> getBinder() {
        return binder;
    }
    ...
    public Div getDetailsTabLayout() {
    ...
    /**
     * Restores grid selection to the entity with the specified ID after refresh. This prevents losing the current
     * selection when the grid is refreshed.
     * 
     * @param entityId
    ...
    protected void restoreGridSelection(final Long entityId) {
        ...
        try {
            ...
            grid.getDataProvider().fetch(new com.vaadin.flow.data.provider.Query<>())
                    .filter(entity -> entityId.equals(entity.getId())).findFirst().ifPresentOrElse(entity -> {
                        grid.select(entity);
    ...
    /**
     * Safely navigates to the view class without throwing exceptions if UI is not available. This is important for
     * testing and edge cases where UI might not be properly initialized.
     */
    ...
    /**
     * Selects the first item in the grid if available. This is called when no specific entity ID is provided in the
     * URL.
     */
    protected void selectFirstItemIfAvailable() {
        LOGGER.debug("selectFirstItemIfAvailable called for {}", getClass().getSimpleName());

        if (grid == null) {
            LOGGER.warn("Grid is null, cannot select first item");
            return;
        }
        grid.getDataProvider().fetch(new com.vaadin.flow.data.provider.Query<>()).findFirst()
     ...
     * 
     * @param entity
     *            the entity to populate the form with
     */
    ...
    /**
     * Validates an entity before saving. Subclasses can override this method to add custom validation logic beyond the
