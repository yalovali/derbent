...
/**
 * Project-aware accordion description that refreshes content when the active project changes. This base class
 * implements CProjectChangeListener to handle project switching scenarios where panels need to update their content
 * based on the new project context. Layer: View (MVC) Purpose: Extends CAccordionDescription with project change
 * awareness for proper panel refresh
 */
public abstract class CProjectAwareAccordionDescription<EntityClass extends CEntityDB<EntityClass>>
     ...
     * 
     * @param title
     *            custom title for the accordion panel
     * @param currentEntity
     ...
     *            session service for project change notifications
     */
    public CProjectAwareAccordionDescription(final String title, final EntityClass currentEntity,
            final BeanValidationBinder<EntityClass> beanValidationBinder, final Class<EntityClass> entityClass,
    ...
    protected void onAttach(final AttachEvent attachEvent) {
        ...
        if (sessionService != null) {
            ...
            LOGGER.debug("Registered project change listener for panel: {}", getClass().getSimpleName());
        }
    }
    ...
    protected void onDetach(final DetachEvent detachEvent) {
        ...
        if (sessionService != null) {
            ...
            LOGGER.debug("Unregistered project change listener for panel: {}", getClass().getSimpleName());
        }
    }

    /**
     * Called when the active project changes. Implementations should refresh their content to reflect the new project
     * context.
     * 
     * @param newProject
    ...
    /**
     * Refreshes the panel content for a project change. Default implementation clears the base layout and recreates
     * content. Subclasses can override for custom behavior.
     * 
     * @param newProject
    ...
    protected void refreshPanelForProjectChange(final CProject newProject) {
        ...
        try {
            ...
            LOGGER.error("Error refreshing panel content for project change in {}: {}", getClass().getSimpleName(),
                    e.getMessage(), e);
        }
    }

    /**
     * Determines if the panel should refresh based on the entity and new project. Default implementation always returns
     * true. Subclasses can override for specific logic.
     * 
     * @param entity
     *            the current entity
     * @param newProject
