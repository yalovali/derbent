...
public abstract class CDBRelationDialog<RelationshipClass extends CEntityDB<RelationshipClass>, MainEntityClass extends CEntityDB<MainEntityClass>,
	...
	/** Override the save method to use unified relationship save functionality when available, while maintaining the callback pattern. */
	@Override
	protected void save() {
		try {
	...
	/** Adds a unified save functionality that handles both adding new relationships and updating existing ones. This method encapsulates the proper
	 * collection management logic to ensure items are added to lists instead of replacing them.
	...
	protected void saveRelationship(final RelationshipClass relationship, final java.util.function.Supplier<List<RelationshipClass>> getSettings,
	...
	/** Common form validation logic extracted from child classes. Validates all required fields and updates the data object. */
	@Override
