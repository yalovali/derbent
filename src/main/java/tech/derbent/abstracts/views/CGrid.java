...
public class CGrid<EntityClass extends CEntityDB<EntityClass>> extends Grid<EntityClass> {
    ...
    public Column<EntityClass> addCustomColumn(final ValueProvider<EntityClass, ?> valueProvider, final String header,
        ...
        final Column<EntityClass> column = addColumn(valueProvider).setHeader(header).setWidth(width)
                .setFlexGrow(flexGrow).setSortable(true);
    ...
    /**
     * Adds a status column with color-aware rendering. This method creates a column that displays status entities with
     * their associated colors as background using CGridCell.
     * 
     ...
     * after this call.
     * 
     * @param showIconInStatusColumns
     *            true to show icons in status columns
     */
