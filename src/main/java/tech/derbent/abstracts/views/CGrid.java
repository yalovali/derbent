...
public class CGrid<EntityClass extends CEntityDB<EntityClass>> extends Grid<EntityClass> {
	...
	/**
	 * Ensures that the grid has a selected row when data is available. This method is
	 * called automatically when data changes and follows the coding guideline that grids
	 * should always have a selected row.
	 */
	public void ensureSelectionWhenDataAvailable() {
		...
		try {
			...
			if (asSingleSelect().getValue() == null) {
				getDataProvider().fetch(new com.vaadin.flow.data.provider.Query<>())
