...
public class CSampleDataInitializer implements ApplicationRunner {
    ...
    /**
     * Clears all sample data from the database to prepare for fresh initialization. This method ensures that restarting
     * the application multiple times doesn't create duplicate sample data. Note: This is a simplified cleanup that
     * shows the intent. In a production environment, consider using database scripts or admin interfaces for cleanup.
     */
    @Transactional
    ...
    private void createSampleScreens() {
        ...
        try {
            ...
            if (project1 != null) {
                createScreenWithFields(project1, "User Management Screen", "CUser", "company", "name", "description");
            }
        } catch (final Exception e) {
     ...
     */
    private void createScreenWithFields(final CProject project, final String screenName, final String entityType,
            final String fieldClass, final String entityProperty1, final String entityProperty2) {
        // Create the screen
        ...
        // Create first field
        final CScreenLines field1 = screenLinesService.newEntity(savedScreen, fieldClass, entityProperty1);
        field1.setFieldDescription("Description for " + entityProperty1.toLowerCase());
        field1.setIsRequired(true);
        field1.setIsActive(true);
        field1.setLineOrder(1);
        field1.setFieldCaption(fieldClass + " " + entityProperty1);
        screenLinesService.save(field1);
        // Create second field
        final CScreenLines field2 = screenLinesService.newEntity(savedScreen, fieldClass, entityProperty2);
        field2.setFieldDescription("Description for " + entityProperty2.toLowerCase());
        field2.setIsRequired(false);
        field2.setIsActive(true);
        field2.setLineOrder(2);
        field2.setFieldCaption(fieldClass + " " + entityProperty2);
