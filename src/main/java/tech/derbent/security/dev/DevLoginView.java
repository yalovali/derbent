...
@PageTitle("Login")
@AnonymousAllowed
// No @Route annotation - the route is registered dynamically by
// DevSecurityConfig.
class DevLoginView extends Main implements BeforeEnterObserver {
	...
	DevLoginView(final AuthenticationContext authenticationContext) {
		...
		final var exampleUsers = new Div(new Div("Use the following details to login"));
		...
		final var contentDiv = new Div(login, exampleUsers);
		contentDiv.addClassNames("dev-content-div");
		add(contentDiv);
		final var devModeMenuDiv = new Div("You can also use the Dev Mode Menu here to impersonate any user");
	...
	public void beforeEnter(final BeforeEnterEvent event) {
		if (authenticationContext.isAuthenticated()) {
			// Redirect to the main view if the user is already logged in. This makes
			// impersonation easier to work with.
			event.forwardTo("");
			return;
		}
	...
	private Component createSampleUserCard(final DevUser user) {
		...
		final var fullName = new H3(user.getAppUser().getFullName());
		final var credentials = new DescriptionList();
		...
		// Make it easier to log in while still going through the normal authentication
		// process.
		final var loginButton = new Button(VaadinIcon.SIGN_IN.create(), event -> {
