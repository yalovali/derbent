...
public abstract class CEntityNamed<EntityClass> extends CEntityDB<EntityClass> {
    ...
    @AMetaData(displayName = "Created Date", required = false, readOnly = true, description = "Date and time when the activity was created", hidden = false)
    private LocalDateTime createdDate;
    @Column(nullable = true, length = 2000)
    @Size(max = CEntityConstants.MAX_LENGTH_DESCRIPTION, message = ValidationMessages.DESCRIPTION_MAX_LENGTH)
    @AMetaData(displayName = "Description", required = false, readOnly = false, defaultValue = "", description = "Detailed description of the project", hidden = false, maxLength = CEntityConstants.MAX_LENGTH_DESCRIPTION)
    private String description;
    @Column(name = "last_modified_date", nullable = true)
    @AMetaData(displayName = "Last Modified", required = false, readOnly = true, description = "Date and time when the activity was last modified", hidden = false)
    ...
    @AMetaData(displayName = "Name", required = true, readOnly = false, defaultValue = "", description = "Name", hidden = false, maxLength = CEntityConstants.MAX_LENGTH_NAME, setBackgroundFromColor = true)
    private String name;
    ...
    /**
     * Returns the default ordering field for queries. Named entities are ordered by
     * name by default.
     * 
     ...
     * This implementation extends the base class to search in 'name'
     * and 'description' fields in addition to inherited fields from CEntityDB. If
     * no field names are specified, searches only in "name" field.
     * 
     * @param searchValue the value to search for (case-insensitive)
     * @param fieldNames  the list of field names to search in. If null or empty,
     *                    searches only in "name" field. Supported field names:
     *                    "id",
     *                    "active", "name", "description"
     * @return true if the entity matches the search criteria in any of the
     *         specified fields
     */
