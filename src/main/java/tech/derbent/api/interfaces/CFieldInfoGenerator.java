...
public interface CFieldInfoGenerator {
	...
	Logger LOGGER = LoggerFactory.getLogger(CFieldInfoGenerator.class);
	...
	default void initFieldInfos() {
		try {
			...
			LOGGER.error("Error initializing field information for class '{}': {}", getClassName(), e.getMessage(), e);
