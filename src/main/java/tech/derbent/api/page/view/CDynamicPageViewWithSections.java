...
@PermitAll
public class CDynamicPageViewWithSections
		extends CDynamicPageViewForEntityEdit {

	private static final Logger LOGGER =
			LoggerFactory.getLogger(CDynamicPageViewWithSections.class);
	private static final long serialVersionUID = 1L;
	// State tracking for performance optimization
	protected CComponentGridEntity grid;
	private final CGridEntityService gridEntityService;
	private final CVerticalLayout splitBottomLayout =
			new CVerticalLayout(false, false, false);
	// Layout components
	// protected final SplitLayout splitLayout = new SplitLayout();

	public CDynamicPageViewWithSections(final CPageEntity pageEntity,
			final ISessionService sessionService,
			final CDetailSectionService detailSectionService,
			final CGridEntityService gridEntityService) throws Exception {
		...
		try {
			...
			CNotificationService.showException(
					"Failed to initialize dynamic page view with sections for: "
							+ pageEntity.getPageTitle(),
					e);
		}
	}
	...
	private void createMasterSection() {
		try {
			...
			Check.notNull(pageEntity.getGridEntity(),
					"Grid entity cannot be null");
			...
			grid.addSelectionChangeListener(event -> {
				try {
					...
					CNotificationService.showException(
							"Error handling entity selection from grid", e);
				}
			});
			...
			LOGGER.error("Error creating master section with grid entity:{}",
					e.getMessage());
			throw e;
		}
	}
	...
	protected <T extends CEntityDB<T>> T createNewEntity() throws Exception {
		try {
			...
			if (newEntity instanceof CEntityOfProject) {
				((CEntityOfProject<?>) newEntity).setProject(
						getSessionService().getActiveProject().orElse(null));
			}
			// Special handling for CUser entities - create project association through CUserProjectSettings
			else if (newEntity instanceof final CUser user) {
				...
				if (activeProject != null) {
					...
					LOGGER.debug(
							"CUser entity created in context of project: {}",
							activeProject.getName());
				}
			}
			return newEntity;
		} catch (final Exception e) {
			LOGGER.error("Error creating new entity instance for type: {} - {}",
	...
	protected void initializePage() throws Exception {
		try {
			...
			setSizeFull();
			if (pageEntity.getPageTitle() != null
					&& !pageEntity.getPageTitle().trim().isEmpty()) {
				getElement().executeJs("document.title = $0",
						pageEntity.getPageTitle());
			}
			...
			if (!pageEntity.getAttributeHideTopCrudtoolbar()) {
				createCRUDToolbar(splitBottomLayout);
			}
			grid.selectNextItem();
		} catch (final Exception e) {
			LOGGER.error("Error initializing dynamic page view:{}",
					e.getMessage());
			throw e;
		}
	}
	...
	protected void locateItemById(final Long pageItemId) {
		try {
			...
			final CEntityDB<?> entity =
					entityService.getById(pageItemId).orElse(null);
			if (entity != null) {
			...
			LOGGER.error("Error locating item by ID {}: {}", pageItemId,
					e.getMessage());
			throw new IllegalStateException("Error locating item by ID "
					+ pageItemId + ": " + e.getMessage());
		}
	}
	...
	public void onEntityDeleted(final CEntityDB entity) {
		try {
			...
			LOGGER.error("Error handling entity deleted notification:{}",
					e.getMessage());
			throw e;
		}
	}
	...
	public void onEntitySaved(final @Nonnull CEntityDB entity) {
		try {
			...
			if (currentSelection == null
					|| !entity.getId().equals(currentSelection.getId())) {
				grid.selectEntity(entity);
				LOGGER.debug("Selected saved entity in grid: {}",
						entity.getId());
			} else {
				LOGGER.debug(
						"Entity already selected in grid, skipping selection to avoid loop");
			}
		} catch (final Exception e) {
			LOGGER.error("Error handling entity saved notification:{}",
					e.getMessage());
			throw e;
		}
	}
	...
	private void onEntitySelected(
			final CComponentGridEntity.SelectionChangeEvent event)
			throws Exception {
	...
	public void setValue(final CEntityDB<?> entity) {
		LOGGER.debug(
				"Setting current entity in dynamic page view with sections: {}",
				entity);
		try {
			...
			LOGGER.error("Error setting current entity in toolbar: {}",
					e.getMessage());
