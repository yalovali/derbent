...
public class CUserCompanyRoleInitializerService extends CRoleInitializationService {
	...
	public static CGridEntity createGridEntity(final CProject<?> project) {
		...
		grid.setColumnFields(
				List.of("id", "name", "description", "isAdmin", "isUser", "isGuest", "color", "sortOrder"));
		return grid;
	}
	...
	public static CUserCompanyRole initializeSampleBab(final CCompany company, final boolean minimal) throws Exception {
		...
		initializeCompanyEntity(seeds, service, company, minimal, (item, index) -> {
			final CUserCompanyRole role = item;
			role.setColor(CColorUtils.getRandomColor(true));
			role.setSortOrder(1);
		...
		return service.listByCompany(company).stream().filter(role -> BAB_ADMIN_ROLE_NAME.equals(role.getName()))
				.findFirst()
