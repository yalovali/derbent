...
public class CUserCompanyRoleInitializerService extends CInitializerServiceBase {
	...
	public static void initialize(final CProject<?> project, final CGridEntityService gridEntityService,
		...
		if (CSpringContext.isBabProfile()) {
			initBase(clazz, project, gridEntityService, detailSectionService, pageEntityService, detailSection, grid,
					MenuTitle_DEVELOPMENT + menuTitle, pageTitle, pageDescription, showInQuickToolbar, Menu_Order_DEVELOPMENT + menuOrder, null);
		} else {
	...
	public static CUserCompanyRole initializeSampleBab(final CCompany company, final boolean minimal) throws Exception {
		...
		return service.listByCompany(company).stream().filter(role -> BAB_ADMIN_ROLE_NAME.equals(role.getName())).findFirst()
