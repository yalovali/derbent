 ...
 * components.
 * <p>
 * This helper provides transparent, automatic saving and restoring of component
 * values across refreshes and page reloads. Once enabled, the component
 * will automatically:
 * <ul>
 * <li>Save its value whenever the user changes it (not on programmatic
 * changes)</li>
 ...
 * <li><b>Transparent</b> - Works seamlessly with Vaadin's component
 * lifecycle</li>
 * <li><b>User-focused</b> - Only saves user changes, not programmatic
 * updates</li>
 ...
 * <ul>
 * <li><b>ComboBox</b> - Stores selected item (by toString() / custom
 * converter)</li>
 ...
 *     return comboBox.getListDataView().getItems().filter(type -&gt; type.getEntityClass().getName().equals(className))
 ...
 * CValueStorageHelper.valuePersist_enable(comboBox, "filter_project", project -&gt; project.getId().toString(), // Save as
 *                                                                                                            // ID
 ...
 * <h3>How It Works:</h3>
 * <ol>
 * <li><b>Setup:</b> Call valuePersist_enable() with component, storage ID, and
 * converter</li>
 * <li><b>Save:</b> When user changes value, automatically saves toString() to
 * session</li>
 * <li><b>Restore:</b> When component attaches, automatically restores using
 * converter</li>
 * <li><b>Validation:</b> Converter can return null if stored value is no longer
 * valid</li>
 * </ol>
 * <h3>Important Notes:</h3>
 * <ul>
 * <li><b>User Changes Only:</b> Only saves when event.isFromClient() is
 * true</li>
 * <li><b>Null Handling:</b> Converter should return null for invalid stored
 * values</li>
 * <li><b>Storage ID:</b> Must be unique per component within the session</li>
 * <li><b>Initial Value:</b> If no stored value exists, current value is saved
 * as default</li>
 * </ul>
 * 
...
public class CValueStorageHelper {
    ...
    public interface ValueSerializer<T> {
         ...
         * @return The string representation for storage, or null if value cannot be
         *         serialized
         */
     ...
     * <p>
     * This is a convenience method for ComboBox components that use toString() for
     * serialization and require a custom converter for deserialization.
     * </p>
     * 
     ...
     */
    public static <T> void valuePersist_enable(final ComboBox<T> comboBox, final String storageId,
    ...
    /**
     * Enables automatic value persistence for a component with custom
     * serialization.
     * <p>
     ...
     * @param component  The component to enable persistence for (must implement
     *                   HasValue)
     * @param storageId  The unique storage identifier
     ...
     */
    public static <T> void valuePersist_enable(final HasValue<?, T> component, final String storageId,
        ...
        component.addValueChangeListener(event -> {
            if (!event.isFromClient()) {
                ...
                LOGGER.debug(
                        "[ValuePersistence] CValueStorageHelper: Skipping save for storage ID '{}' - change not from client",
                        storageId);
                return;
            }
            try {
                ...
                LOGGER.info(
                        "[ValuePersistence] CValueStorageHelper: Value changed from client for storage ID '{}', value: {}",
                        storageId, value);
                if (value != null) {
                    final String serialized = serializer.toString(value);
                    if (serialized != null) {
                        sessionService.setSessionValue(storageId, serialized);
                        LOGGER.info("[ValuePersistence] CValueStorageHelper: Auto-saved value '{}' for storage ID: {}",
                                serialized, storageId);
                    }
                } else {
                    sessionService.removeSessionValue(storageId);
                    LOGGER.info("[ValuePersistence] CValueStorageHelper: Cleared stored value for storage ID: {}",
                            storageId);
                }
            } catch (final Exception e) {
                LOGGER.error("[ValuePersistence] CValueStorageHelper: Error auto-saving value for storage ID: {}",
                        storageId, e);
            }
        });
        ...
        if (vaadinComponent.isAttached()) {
            LOGGER.info(
                    "[ValuePersistence] CValueStorageHelper: Component already attached for storage ID '{}', restoring value immediately",
                    storageId);
            valuePersist_restoreValue(component, storageId, converter, sessionService);
        }

        LOGGER.info(
                "[ValuePersistence] CValueStorageHelper: Enabled auto-persistence for component with storage ID: {}",
                storageId);
    }
     ...
     * <p>
     * This is a convenience method for TextField components that use simple string
     * values.
     * </p>
     * 
     ...
     * <p>
     * This method sets the value without triggering user-facing side effects by
     * checking if the value change event is from the client. This prevents
     * cascading updates like SQL queries and form population during automatic
     * restoration.
     * </p>
     * <p>
     * If no stored value exists but the component has a current value, that value
     * is saved as the initial default to ensure persistence works on
     ...
     * @param converter      Function to convert storage string back to component
     *                       value
     * @param sessionService The session service for retrieving stored values
     */
    private static <T> void valuePersist_restoreValue(final HasValue<?, T> component, final String storageId,
            final ValueConverter<T> converter,
            final ISessionService sessionService) {
        try {
            final Optional<String> storedValue = sessionService.getSessionValue(storageId);
            LOGGER.info(
                    "[ValuePersistence] CValueStorageHelper: Restoring value for storage ID '{}', storedValue present: {}",
                    storageId,
                    storedValue.isPresent());
            if (storedValue.isPresent()) {
                LOGGER.info("[ValuePersistence] CValueStorageHelper: Found stored value '{}' for storage ID '{}'",
                        storedValue.get(), storageId);
                ...
                if (value != null) {
                    ...
                    // Components should check event.isFromClient() to distinguish user actions from
                    // programmatic updates
                    component.setValue(value);
                    LOGGER.info("[ValuePersistence] CValueStorageHelper: Auto-restored value '{}' for storage ID: {}",
                            value, storageId);
                } else {
                    LOGGER.warn(
                            "[ValuePersistence] CValueStorageHelper: Could not convert stored value '{}' for storage ID: {}",
                            storedValue.get(),
                            storageId);
                }
            } else {
                // No stored value exists - check if component has a current value and save it
                // as initial default
                final T currentValue = component.getValue();
                LOGGER.info(
                        "[ValuePersistence] CValueStorageHelper: No stored value for storage ID '{}', current value: {}",
                        storageId,
                        currentValue);
                if (currentValue != null) {
                    ...
                    if (serialized != null && !serialized.isBlank()) {
                        sessionService.setSessionValue(storageId, serialized);
                        LOGGER.info(
                                "[ValuePersistence] CValueStorageHelper: Saved initial value '{}' as default for storage ID: {}",
                                serialized,
                                storageId);
                    }
                }
            ...
            LOGGER.error("[ValuePersistence] CValueStorageHelper: Error auto-restoring value for storage ID: {}",
                    storageId, e);
        }
    }
}
