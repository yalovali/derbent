...
public final class CImageUtils {
	...
	/** Generates an SVG avatar with initials as a string. This creates a lightweight SVG representation that can be used directly with Vaadin's
	 * SvgIcon component.
	...
	public static String generateAvatarSvg(final String initials, final int size) {
		...
		final String colorHex = String.format("#%02X%02X%02X", backgroundColor.getRed(), backgroundColor.getGreen(), backgroundColor.getBlue());
		...
		svg.append(String.format("<circle cx=\"%d\" cy=\"%d\" r=\"%d\" fill=\"%s\"/>", size / 2, size / 2, size / 2, colorHex));
...
//		LOGGER.debug("Generated SVG avatar with initials '{}' (size: {}x{})", initials, size, size);
	...
	/** Generates an avatar image with initials on a colored background. Creates a PNG image with the specified size, containing centered initials
	 * text on a background color derived from the initials string (for consistency).
	 ...
	 * optimal results. Validates input parameters and handles various image formats, outputting JPEG format. SVG images are returned as-is since they
	 * are vector-based and scale without loss of quality.
