...
/** CViewImport - Single-page Excel import view. Lets users upload a multi-sheet Excel file and import entities project-wide. Layer: View (MVC) */
@Route ("import")
...
public final class CViewImport extends CAbstractPage {
	...
	private Div buildHeader(final Span supportedSpan) {
		...
		final Paragraph subtitle =
				new Paragraph("Upload a multi-sheet .xlsx file. Each sheet name must match a supported entity type. "
	...
	private Grid<CImportRowResult> buildRowResultGrid(final CImportSheetResult sheet) {
		...
		grid.setItems(sheet.getRowResults().stream().filter(r -> !r.isSkipped()).toList());
	...
	private String buildSheetPanelTitle(final CImportSheetResult sheet) {
		...
		return status + " " + sheet.getSheetName() + " — " + sheet.getSuccessCount() + " ok, " + sheet.getErrorCount()
				+ " errors, " + sheet.getSkippedCount() + " skipped";
	}
	...
	private Div buildUploadSection() {
		final Div section = new Div();
		section.addClassNames("import-upload-section", LumoUtility.Padding.MEDIUM, LumoUtility.BorderRadius.MEDIUM);
		...
		upload.addFailedListener(event -> showError("Upload failed: " + event.getReason().getMessage()));
		upload.addFileRejectedListener(event -> showError("File rejected: " + event.getErrorMessage()));
	...
	/** initPage() is called from CAbstractPage() constructor — before this class's fields are assigned. Only build the static UI skeleton here; do NOT
	 * access injected fields (importService, handlerRegistry, sessionService). Data-dependent population happens in postConstruct(). */
	@Override
	...
	private void persistImportHistory(final CImportResult result, final CProject<?> project) {
		...
		try {
			final String username = sessionService.getActiveUser().map(CUser::getUsername).orElse("unknown");
	...
	protected void postConstruct() {
		...
		handlerRegistry.getAllHandlers()
				.forEach(h -> supported.append(h.getEntityClass().getSimpleName().replaceFirst("^C", "")).append(", "));
