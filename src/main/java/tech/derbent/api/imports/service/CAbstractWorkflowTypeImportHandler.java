 ...
 * {@link #applyTypeEntityFields}. Only workflow resolution is added at this level.
 * </p>
 */
public abstract class CAbstractWorkflowTypeImportHandler<T extends CTypeEntity<T>>
	...
	public CImportRowResult importRow(final Map<String, String> rowData, final CProject<?> project, final int rowNumber,
		...
		// WHY: system init Excel is intended to be re-runnable (and also safe on top of
		// code-initialized reference data). Upsert-by-name avoids unique constraint violations.
		...
		if (!workflowName.isBlank()) {
			...
			if (wf == null) {
				throw new CImportException(rowNumber,
						"Workflow '" + workflowName + "' not found. Create it before importing.");
			}
			type.setWorkflow(wf);
		}
