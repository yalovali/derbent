...
/**
 * Central registry for entity metadata using map-based lookups.
 * This replaces the switch-case pattern in CAuxillaries with O(1) lookup performance.
...
public class CEntityRegistry {
	...
	public static void register(final IEntityRegistrable registrable) {
		try {
			...
			if (registrable.getDefaultIcon() != null) {
				defaultIcons.put(entityClass, registrable.getDefaultIcon());
				defaultIconsByName.put(entityClass.getName(), registrable.getDefaultIcon());
				LOGGER.debug("Registered default icon: {} -> {}", simpleName, registrable.getDefaultIcon());
			}
			...
			if (registrable.getDefaultColor() != null) {
				...
				LOGGER.debug("Registered default color: {} -> {}", simpleName, registrable.getDefaultColor());
			}
	 ...
	 * This should be called after all entities have been registered.
	 */
