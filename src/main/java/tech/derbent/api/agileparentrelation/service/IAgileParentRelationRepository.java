...
public interface IAgileParentRelationRepository extends JpaRepository<CAgileParentRelation, Long>, IAbstractRepository<CAgileParentRelation> {
	...
	@Query (
			value = "WITH RECURSIVE descendants AS ("
					+ "  SELECT parent_activity_id FROM cagile_parent_relation WHERE parent_activity_id = :childActivityId " + "  UNION ALL "
	...
	/** Find all children of a parent activity. This query finds all entities that have this activity as their parent.
	 * @param parentActivityId the ID of the parent activity
