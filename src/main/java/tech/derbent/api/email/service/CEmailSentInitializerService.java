...
/** CEmailSentInitializerService - Initializer for sent email archive. Layer: Service (MVC) Profile: derbent (PLM framework) Provides initialization
 * for sent email archive interface with: - Complete audit trail visualization - Read-only archive views - Date range and type filtering - Compliance
 * reporting support
 * @see CEmailSent
 * @see CEmailSentService */
public final class CEmailSentInitializerService extends CEmailInitializerService {
	...
	/** Create comprehensive detail section for sent email archive. Following Derbent standard entity initializer pattern. All fields are read-only as
	 * this is an audit archive.
	...
	/** Create grid entity for sent email archive display. Shows key fields for audit and reporting.
	 * @param project the project context
	...
	private static CGridEntity createGridEntity(final CProject<?> project) throws Exception {
		...
		grid.setColumnFields(List.of("sentAt", "toEmail", "subject", "emailType", "priority", "retryCount"));
		return grid;
	}

	/** Initialize sent email archive entity configuration. Creates page, grid, and navigation entries for email archive.
	 * @param project the project context
	 * @throws Exception if initialization fails */
	...
	/** Initialize sample sent email archive data. Creates historical email records for testing and demonstration.
	 * @param project the project context
