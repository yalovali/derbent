...
/** CEmailSentService - Service for managing sent email archive. Layer: Service (MVC) Profile: derbent (PLM framework) This service manages the
 * archive of successfully sent emails, providing: - Complete audit trail of all sent emails - Date range queries for reporting - Email type and
 * recipient filtering - Compliance and archival support - Statistics and analytics queries Sent emails are moved from CEmailQueued to CEmailSent
 * after successful delivery. This archive provides complete email history for audit and compliance purposes.
 * @see CEmailSent
...
public class CEmailSentService extends CEntityOfCompanyService<CEmailSent> implements IEntityRegistrable, IEntityWithView {
	...
	public CEmailSentService(final IEmailSentRepository repository, final Clock clock, final ISessionService sessionService) {
	...
	/** Find sent emails within a date range for a company. Used for reporting and audit purposes.
	 * @param company   the company to query
	 * @param startDate start of date range (inclusive)
	 * @param endDate   end of date range (inclusive)
	 * @return list of sent emails in date range */
	@Transactional (readOnly = true)
	public List<CEmailSent> findByDateRange(final CCompany company, final LocalDateTime startDate, final LocalDateTime endDate) {
		...
		LOGGER.debug("Finding sent emails for company {} between {} and {}", company.getName(), startDate, endDate);
		return ((IEmailSentRepository) repository).findByDateRange(company, startDate, endDate);
	}

	/** Find sent emails by type for a company. Used for analyzing email distribution by category.
	 * @param company   the company to query
	...
	/** Find sent emails by recipient for a company. Used for tracking communication with specific users.
	 * @param company the company to query
	...
	/** Get recent sent emails for a company. Used for dashboards and recent activity views.
	 * @param company the company to query
	...
	/** Get total count of sent emails for a company. Used for statistics and dashboards.
	 * @param company the company to query
	...
	@Override
	public Class<CEmailSent> getEntityClass() { return CEmailSent.class; }

	@Override
	public Class<?> getInitializerServiceClass() { return CEmailSentInitializerService.class; }

	@Override
	public Class<?> getPageServiceClass() { return CPageServiceEmailSent.class; }
