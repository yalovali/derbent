...
/** CEmailQueuedInitializerService - Initializer for email queue management. Layer: Service (MVC) Profile: derbent (PLM framework) Provides
 * initialization for email queue interface with: - Complete email composition fields (sender, recipient, reply-to) - Priority and type management -
 * Retry tracking and error monitoring - Entity reference linking - Queue status visualization
 * @see CEmailQueued
 * @see CEmailQueuedService */
public final class CEmailQueuedInitializerService extends CEmailInitializerService {
	...
	/** Create comprehensive detail section for email queue entity. Following Derbent standard entity initializer pattern.
	...
	/** Create grid entity for email queue display. Shows key fields for queue monitoring and management.
	 * @param project the project context
	...
	private static CGridEntity createGridEntity(final CProject<?> project) throws Exception {
		...
		grid.setColumnFields(
				List.of("priority", "toEmail", "subject", "emailType", "queuedAt", "retryCount", "lastError"));
		return grid;
	}

	/** Initialize email queue entity configuration. Creates page, grid, and navigation entries for email queue management.
	 * @param project the project context
	 * @throws Exception if initialization fails */
	public static void initialize(final CProject<?> project) throws Exception {
		...
		initBase(clazz, project, gridEntityService, detailSectionService, pageEntityService, detailSection, grid,
				menuTitle, pageTitle, description, toolbar, menuOrder, null);
		LOGGER.info("CEmailQueued entity configuration initialized successfully");
	}

	/** Initialize sample email queue data for testing. Creates test emails with different priorities and states.
	 * @param project the project context
