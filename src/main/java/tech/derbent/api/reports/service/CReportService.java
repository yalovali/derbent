...
/**
 * Service for generating CSV reports from entity data.
 * Handles CSV formatting, field value extraction, and file downloads.
 */
@Service
public class CReportService {
     ...
     */
    public String generateCSV(final List<? extends CEntityDB<?>> entities, final List<EntityFieldInfo> selectedFields,
    ...
    private void writeCSVHeader(final StringWriter writer, final List<EntityFieldInfo> selectedFields) {
        final List<String> headers = selectedFields.stream()
                .map(field -> escapeCsvValue(field.getDisplayName()))
    ...
    /**
     * Extracts the value of a field from an entity.
     * Handles simple fields and complex/relation fields.
     */
    private String extractFieldValue(final CEntityDB<?> entity, final EntityFieldInfo fieldInfo, final Class<?> entityClass)
            throws Exception {
        try {
            ...
            return formatFieldValue(value, field.getType());
        } catch (final Exception e) {
    ...
    private String extractNestedFieldValue(final CEntityDB<?> entity, final String fieldPath) {
        try {
            ...
            return formatFieldValue(currentValue, currentValue != null ? currentValue.getClass() : String.class);
        } catch (final Exception e) {
     ...
     */
    private String formatFieldValue(final Object value, final Class<?> type) {
        if (value == null) {
            return "";
        }
