 ...
 * <p>
 * Provides static methods to generate CSV reports with field selection dialog.
 * Used by page services and grid views for consistent reporting across the application.
 * </p>
 * <p>
 * <b>Usage:</b>
 * <pre>
...
public final class CReportHelper {
     ...
     * <p>
     * This is the main entry point for grid-based reporting.
     * Discovers all available fields, lets user select which to export,
     * then generates and downloads CSV file.
     * </p>
     * 
     ...
     */
    public static <T extends CEntityDB<T>> void generateReport(final List<T> entities, 
        ...
        try {
            ...
            final List<CReportFieldDescriptor> allFields = 
                CReportFieldDescriptor.discoverFields((Class<? extends CEntityDB<?>>) entityClass);
    ...
    private static <T extends CEntityDB<T>> void generateAndDownloadCSV(final List<T> entities,
            final List<CReportFieldDescriptor> selectedFields, final Class<T> entityClass) throws Exception {
        ...
        CNotificationService.showSuccess(
            String.format("Exporting %d records to CSV", entities.size())
        );
        LOGGER.info("CSV report generated successfully: {} records, {} fields", 
            entities.size(), selectedFields.size());
    }

    /**
     * Triggers file download in the browser.
     * Creates a temporary Anchor element and simulates click.
     * 
