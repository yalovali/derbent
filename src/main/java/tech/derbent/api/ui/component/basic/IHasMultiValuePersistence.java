...
/**
 * IHasMultiValuePersistence - Interface for components that need to persist
 * multiple key-value pairs.
 * <p>
 * This interface extends the single-value persistence pattern to support
 * storing multiple values under a namespace. Perfect for components that need
 ...
 * </ul>
 * <p>
 * <b>Storage Pattern:</b> Values are stored with keys in the format:
 * {@code {namespace}.{key}}
 * </p>
 * <p>
 * <b>Example Usage:</b>
...
public interface IHasMultiValuePersistence {
	 ...
	 * <p>
	 * This method removes all key-value pairs that were stored under this
	 * component's namespace.
	 * </p>
	 */
	default void clearAllPersistedValues() {
		if (!isPersistenceEnabled() || getPersistenceNamespace() == null) {
			return;
		}
		try {
			...
			final String prefix = getPersistenceNamespace() + ".";
			...
			getLogger().info("[{}] Cleared all persisted values for namespace: {}", getClass().getSimpleName(),
					getPersistenceNamespace());
		} catch (final Exception e) {
			getLogger().error("[{}] Error clearing persisted values for namespace: {}", getClass().getSimpleName(),
					getPersistenceNamespace(), e);
		}
	}
	 ...
	 */
	default void clearPersistedValue(final String key) {
		if (!isPersistenceEnabled() || getPersistenceNamespace() == null) {
			return;
		}
		try {
			final String fullKey = getPersistenceNamespace() + "." + key;
			final ISessionService sessionService = getSessionService();
			if (sessionService != null) {
				sessionService.removeSessionValue(fullKey);
				getLogger().debug("[{}] Cleared persisted value for key: {}", getClass().getSimpleName(), fullKey);
			}
		} catch (final Exception e) {
	 ...
	 */
	default void disableMultiValuePersistence() {
		setPersistenceEnabled(false);
		getLogger().info("[{}] Multi-value persistence disabled for namespace: {}", getClass().getSimpleName(),
				getPersistenceNamespace());
	}
	 ...
	 * <p>
	 * After enabling, the component can use {@link #persistValue(String, Object)}
	 * and {@link #getPersistedValue(String)} to store and retrieve values.
	 * </p>
	 * 
	 * @param namespace The unique namespace for this component's persisted values
	 *                  (e.g., "kanbanBoard_projectX", "activitiesView_filters")
	 * @throws IllegalArgumentException if namespace is null or blank
	 */
	default void enableMultiValuePersistence(final String namespace) {
		if (namespace == null || namespace.isBlank()) {
			throw new IllegalArgumentException("Persistence namespace cannot be null or blank");
		}
		setPersistenceNamespace(namespace);
		setPersistenceEnabled(true);
		getLogger().info("[{}] Multi-value persistence enabled for namespace: {}", getClass().getSimpleName(),
				namespace);
		// Add attach listener to allow restoration when component is added to UI
		if (this instanceof Component) {
			((Component) this).addAttachListener(event -> {
				if (isPersistenceEnabled()) {
					onPersistenceRestore();
				}
			});
	 ...
	 */
	default Optional<String> getPersistedValue(final String key) {
		if (!isPersistenceEnabled() || getPersistenceNamespace() == null) {
		...
		try {
			...
			if (sessionService != null) {
				...
				if (value.isPresent()) {
					getLogger().debug("[{}] Retrieved persisted value for key: {} = {}", getClass().getSimpleName(),
							fullKey, value.get());
				}
				return value;
			}
	 ...
	 */
	default ISessionService getSessionService() {
	 ...
	 * <p>
	 * Components should override this method to restore their persisted state. This
	 * is called automatically when the component attaches to the UI.
	 * </p>
	 * <p>
	 * <b>Example:</b>
	 * 
	...
	default void onPersistenceRestore() {
		...
		getLogger().debug("[{}] Persistence restore hook called (override to implement)",
	 ...
	 * <p>
	 * The value is stored under the key "{namespace}.{key}". Components are
	 * responsible for serialization of complex objects to strings.
	 * </p>
	 * 
		...
		try {
			...
			if (sessionService != null) {
				if (value != null) {
					...
					getLogger().debug("[{}] Persisted value for key: {} = {}", getClass().getSimpleName(), fullKey,
							serialized);
				} else {
