	...
	default void enablePersistence(final String storageKey) {
		...
		addValueChangeListener(event -> {
			if (!event.isFromClient()) {
				getLogger().debug("[{}] Value change not from client, skipping save for key: {}", getClass().getSimpleName(), getPersistenceKey());
				return;
			}
	...
	default void persistence_restoreValue() {
		...
		try {
			...
			if (storedValue.isPresent()) {
				...
				getLogger().debug("[{}] Restoring value '{}' for key: {}", getClass().getSimpleName(), serialized, getPersistenceKey());
				final T value = deserializeValue(serialized);
				if (value != null) {
					setValue(value);
					getLogger().info("[{}] Restored value for key: {}", getClass().getSimpleName(), getPersistenceKey());
				} else {
			...
			getLogger().error("[{}] Error restoring value for key: {}", getClass().getSimpleName(), getPersistenceKey(), e);
		}
	}
		...
		try {
			...
			if (value != null) {
				...
				if (serialized != null && !serialized.isBlank()) {
					...
					getLogger().debug("[{}] Saved value '{}' for key: {}", getClass().getSimpleName(), serialized, getPersistenceKey());
					return;
				}
			}
			...
			getLogger().error("[{}] Error saving value for key: {}", getClass().getSimpleName(), getPersistenceKey(), e);
		}
	}
