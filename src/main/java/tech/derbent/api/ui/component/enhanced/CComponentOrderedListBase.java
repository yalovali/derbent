...
/**
 * CComponentOrderedListBase - Abstract base component for managing ordered lists.
 * Provides common functionality for components that need to display and manage ordered collections,
 * particularly for fields with @OrderColumn annotations in JPA entities.
 * 
 * <p>Features:
 * <ul>
...
public abstract class CComponentOrderedListBase<T> extends CHorizontalLayout
	...
	/**
	 * Initialize the UI components. Subclasses must implement this to create
	 * their specific UI layout and components.
	 */
	protected abstract void initializeUI();

	/**
	 * Refresh the display to show current state of items.
	 * Subclasses must implement this to update their specific UI components.
	 */
	protected abstract void refreshDisplay();

	/**
	 * Validate an item before adding it to the selected list.
	 * Default implementation always returns true. Override to add custom validation.
	 * 
	...
	protected boolean removeSelectedItem(T item) {
		...
		try {
			boolean removed = selectedItems.remove(item);
			if (removed) {
	...
	protected void updateAvailableItems() {
		...
		if (!allowDuplicates) {
			availableItems.addAll(sourceItems.stream()
	...
	protected void fireValueChangeEvent() {
		...
		ValueChangeEvent<List<T>> event = new ValueChangeEvent<List<T>>() {
			private static final long serialVersionUID = 1L;

			@Override
			public HasValue<?, List<T>> getHasValue() {
				return CComponentOrderedListBase.this;
			}
