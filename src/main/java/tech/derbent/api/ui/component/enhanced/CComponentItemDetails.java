...
/**
 * CComponentItemDetails - A standard component implementing HasValue interface
 * for displaying entity details.
 * <p>
 * This component displays the details of a CProjectItem entity in a
 * CDynamicPageRouter. When a value is set via setValue(), the component
 * automatically locates and displays the entity's default detail page.
 * </p>
 * <p>
 * Features:
 * <ul>
 ...
 *
 * <pre>
 * CComponentItemDetails itemDetails = new CComponentItemDetails(sessionService, pageEntityService,
 ...
 * @since 1.0
 */
public class CComponentItemDetails extends CVerticalLayout
        implements HasValue<HasValue.ValueChangeEvent<CEntityNamed<?>>, CEntityNamed<?>> {
    ...
    private CEntityNamed<?> currentValue = null;
    final CPageEntityService pageEntityService = CSpringContext.getBean(CPageEntityService.class);
    private CEntityNamed<?> previousValue = null;
    private boolean readOnly = false;
    private final ISessionService sessionService;
    // HasValue interface fields
    private final List<ValueChangeListener<? super ValueChangeEvent<CEntityNamed<?>>>> valueChangeListeners = new ArrayList<>();

    /**
     * Creates a new CComponentItemDetails component.
     * 
     * @param sessionService       the session service for accessing user session
     *                             data
     * @param pageEntityService    the page entity service for locating entity
     *                             detail pages
     ...
     */
    public CComponentItemDetails(final ISessionService sessionService) throws Exception {
        ...
        this.currentEntityPageRouter = new CDynamicPageRouter(pageEntityService, sessionService, detailSectionService,
                null);
    ...
    public Registration addValueChangeListener(
            final ValueChangeListener<? super ValueChangeEvent<CEntityNamed<?>>> listener) {
    ...
    protected void fireValueChangeEvent(final CEntityNamed<?> newValue, final boolean fromClient) {
        ...
        if (!valueChangeListeners.isEmpty()) {
            ...
            final ValueChangeEvent<CEntityNamed<?>> event = new ValueChangeEvent<CEntityNamed<?>>() {
                ...
                @Override
                public HasValue<?, CEntityNamed<?>> getHasValue() {
                    return CComponentItemDetails.this;
                }

                @Override
                public CEntityNamed<?> getOldValue() {
                    return oldValue;
                }

                @Override
                public CEntityNamed<?> getValue() {
                    return newValue;
                }
    ...
    private void locateEntityInDynamicPage(final CEntityNamed<?> entity) {
        try {
            ...
            final CPageEntity page = pageEntityService
    ...
    /**
     * Sets the read-only state of this component. Note: Read-only mode prevents
     * value changes but doesn't affect the display.
     * 
     ...
     * 
     * @param requiredIndicatorVisible true to show required indicator, false to
     *                                 hide
     */
    @Override
    ...
    /**
     * Sets the value of this component (the entity to display). This will update
     * the display to show the entity's detail page and fire value change
     * events.
     * 
     * @param value the entity to display (can be null to clear display)
     */
    @Override
    public void setValue(final CEntityNamed<?> value) {
        if (readOnly) {
