...
/**
 * CGrid - Base grid class for consistent field width management based on data
 * types. Layer: View (MVC) Follows the project's coding guidelines by
 * providing a base class for all grids to ensure consistent column widths based
 * on field types: - ID fields: Very small width (80px) - Integer
 * fields: Small width (100px) - BigDecimal fields: Medium width (120px) - Date
 * fields: Medium width (150px) - Boolean/Status fields: Small-Medium
 * width (100px) - Short text fields: Medium width (200px) - Long text fields:
 * Large width (300px+) - Reference fields: Medium width (200px)
 */
// public class CGrid<EntityClass extends CEntityDB<EntityClass>> extends
// Grid<EntityClass> {
public class CGrid<EntityClass> extends Grid<EntityClass> implements IHasDragControl {
    ...
    /**
     * Map to store widget providers for columns that create components implementing
     * IStateOwnerComponent. Key: Column key, Value: Widget provider
     * function
     */
    ...
    public Column<EntityClass> addBooleanColumn(final ValueProvider<EntityClass, Boolean> valueProvider,
            final String header, final String trueText,
    ...
    public Column<EntityClass> addColumn(final ValueProvider<EntityClass, String> valueProvider, final String header,
            final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        Check.notBlank(header, "Header cannot be null or blank");
    ...
    public Column<EntityClass> addColumnEntityCollection(
        ...
        Check.notBlank(header, "Header cannot be null or blank");
        // compute display string from the collection - now supports both CEntityDB and
        // other types (e.g., String)
        final ValueProvider<EntityClass, String> namesProvider = entity -> {
            try {
                ...
                if (items == null || !Hibernate.isInitialized(items)) {
                    LOGGER.debug("Collection for header '{}' is null or not initialized for entity ID: {}", header,
                            entity.toString());
                ...
                return items.stream().map(item -> {
                    try {
                        ...
                        Check.notBlank(value,
                                "Collection item string representation cannot be blank in header: " + header);
                        return value;
                    } catch (final Exception itemEx) {
                        LOGGER.error("Error rendering collection item for header '{}': {}", header, itemEx.getMessage(),
                                itemEx);
        ...
        final Column<EntityClass> column = addColumn(namesProvider).setAutoWidth(true).setSortable(false)
    ...
    public Column<EntityClass> addColumnEntityNamed(
            final ValueProvider<EntityClass, ? extends CEntityDB<?>> valueProvider, final String header) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        Check.notBlank(header, "Header cannot be null or blank");
        ...
        final Column<EntityClass> column = addColumn(nameProvider).setWidth(WIDTH_REFERENCE).setFlexGrow(0)
                .setSortable(true).setResizable(true).setComparator(nameProvider);
        Check.notNull(column, "Column creation failed for header: " + header);
        return styleColumnHeader(column, header);
    }

    public Column<EntityClass> addCustomColumn(final ValueProvider<EntityClass, ?> valueProvider, final String header,
            final String width,
        ...
        final Column<EntityClass> column = addColumn(valueProvider).setWidth(width).setFlexGrow(flexGrow)
    ...
    public Column<EntityClass> addDateColumn(final ValueProvider<EntityClass, LocalDate> valueProvider,
            final String header, final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        return addCustomColumn(valueProvider, header, WIDTH_DATE, key, 0);
    }

    public Column<EntityClass> addDateTimeColumn(final ValueProvider<EntityClass, LocalDateTime> valueProvider,
            final String header,
            final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        return addCustomColumn(valueProvider, header, WIDTH_DATE, key, 0);
    ...
    public Column<EntityClass> addDecimalColumn(final ValueProvider<EntityClass, BigDecimal> valueProvider,
            final String header, final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
    ...
    /**
     * Adds an editable image column using CPictureSelector in icon mode. Clicking
     * on the profile picture opens a dialog for editing.
     * 
     * @param imageDataProvider Provider that returns byte array of image data
     * @param imageDataSetter   Setter function to update the image data when
     *                          changed
    ...
    public Column<EntityClass> addEditableImageColumn(final ValueProvider<EntityClass, byte[]> imageDataProvider,
            final java.util.function.BiConsumer<EntityClass, byte[]> imageDataSetter, final String header,
    ...
    public Column<EntityClass> addEntityColumn(final ValueProvider<EntityClass, ?> valueProvider, final String header,
            final String key,
            @SuppressWarnings("unused") final Class<?> returnType) throws Exception {
        try {
            Check.notNull(valueProvider, "Value provider cannot be null");
            Field field;
            // final Method applyMethod = valueProvider.getClass().getMethod("apply",
            // Object.class);
            ...
            Check.notNull(meta,
                    "AMetaData annotation is missing on field: " + key + " in class: " + clazz.getSimpleName());
            ...
            final Column<EntityClass> column = addComponentColumn(item -> {
                ...
                try {
                    ...
                    LOGGER.error("Error setting entity column value for {}.{}: {}", clazz.getSimpleName(), key,
                            e.getMessage());
            ...
            LOGGER.error("Error adding entity column for header: {}: {}, key: {} clazz:", header, e.getMessage(), key,
                    clazz.getSimpleName());
            throw e;
        }
    }
     ...
     * @return The created column
     */
    public Column<EntityClass> addExpandingLongTextColumn(final ValueProvider<EntityClass, String> valueProvider,
            final String header,
            final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        final Column<EntityClass> column = addColumn(valueProvider).setFlexGrow(1).setSortable(true).setResizable(true);
     ...
     * @return The created column
     */
    public Column<EntityClass> addExpandingShortTextColumn(final ValueProvider<EntityClass, String> valueProvider,
            final String header,
            final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        final Column<EntityClass> column = addColumn(valueProvider).setFlexGrow(1).setSortable(true).setResizable(true);
    ...
    public Column<EntityClass> addIdColumn(final ValueProvider<EntityClass, ?> valueProvider, final String header,
            final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        final Column<EntityClass> column = addComponentColumn(entity -> {
            try {
        ...
        column.setComparator(
                (entity1, entity2) -> compareIds(valueProvider.apply(entity1), valueProvider.apply(entity2)));
        return styleColumnHeader(column, header);
    }
     ...
     * @return The created column
     */
    public Column<EntityClass> addImageColumn(final ValueProvider<EntityClass, byte[]> imageDataProvider,
            final String header) {
        final Column<EntityClass> column = addComponentColumn(entity -> {
            final byte[] imageData = imageDataProvider.apply(entity);
    ...
    public Column<EntityClass> addIntegerColumn(final ValueProvider<EntityClass, Integer> valueProvider,
            final String header, final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        return addCustomColumn(valueProvider, header, WIDTH_INTEGER, key, 0);
    }

    public Column<EntityClass> addLongTextColumn(final ValueProvider<EntityClass, String> valueProvider,
            final String header, final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        return addCustomColumn(valueProvider, header, WIDTH_LONG_TEXT, key, 0);
    }

    public Column<EntityClass> addReferenceColumn(final ValueProvider<EntityClass, String> valueProvider,
            final String header) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        return addCustomColumn(valueProvider, header, WIDTH_REFERENCE, null, 0);
    }

    public Column<EntityClass> addShortTextColumn(final ValueProvider<EntityClass, String> valueProvider,
            final String header, final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
    ...
    public Column<EntityClass> addStoryPointColumn(final ValueProvider<EntityClass, ISprintableItem> itemProvider,
            final Consumer<ISprintableItem> saveHandler, final Consumer<Exception> errorHandler, final String header,
            final String key) {
        ...
        column.setComparator((left, right) -> compareIds(storyPointValue(itemProvider.apply(left)),
                storyPointValue(itemProvider.apply(right))));
        return styleColumnHeader(column, header);
    }

    public Column<EntityClass> addTimeColumn(final ValueProvider<EntityClass, LocalDateTime> valueProvider,
            final String header, final String key) {
        Check.notNull(valueProvider, "Value provider cannot be null");
        Check.notBlank(header, "Header cannot be null or blank");
        return addCustomColumn(valueProvider, header, WIDTH_DATE, key, 0);
     ...
     * <p>
     * Widget columns support sorting when a comparator is provided. The comparator
     * extracts a comparable value from the entity for sorting purposes.
     * </p>
     * 
     * @param <T>            the widget type (must be a Component)
     * @param widgetProvider the function that creates widgets for entities
     * @param comparator     optional comparator for sorting (null to disable
     *                       sorting)
     * @return the created column
    ...
    public void drag_checkEventBeforePass(CEvent event) {
        // LOGGER.debug("Drag event check before pass: {} comp id:{} event type:{}",
    ...
    public void drag_setDropEnabled(final boolean enabled) {
        ...
        // LOGGER.debug("[DragDebug] CGrid: Drop {} for grid id:{}", enabled ? "enabled"
        // : "disabled", getId());
    }

    /**
     * Ensures that the grid has a selected row when data is available. This method
     * is called automatically when data changes and follows the coding
    ...
    public EntityClass getSelectedEntity() {
    ...
    private void initializeGrid() {
        ...
        sm.setDeselectAllowed(false);
        // Note: Do NOT call setupChildDragDropForwarding() here - CGrid already
        // forwards
        // Vaadin Grid events to IHasDragControl listeners via on_grid_dragStart(),
        // on_grid_dragEnd(), on_grid_dragDrop()
    ...
    private ComponentEventListener<GridDropEvent<EntityClass>> on_grid_dragDrop() {
        return event -> {
            try {
                ...
                // dropped on empty grid, cannot check who is this, i have to pass to upper
                // components
                // Note: Vaadin reports the drop target as the event source; we pass the true
                // drag source separately for clarity.
                final CDragDropEvent dropEvent = new CDragDropEvent(getId().orElse("None"), this, targetItem,
                        dropLocation, true);
    ...
    public void setClazz(Class<?> class1) {
        clazz = (Class<EntityClass>) class1;
    }
    ...
    private void setPartVisible(String part, boolean visible) {
        ...
        getElement().executeJs(
                "const element = this.$.table." + part + ";" + "if(element) {" + "    element.style.display = $0;"
                        + "}",
                visible ? "" : "none");
    }
