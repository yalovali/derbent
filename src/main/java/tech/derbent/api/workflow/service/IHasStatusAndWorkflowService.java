...
public interface IHasStatusAndWorkflowService<EntityClass extends CProjectItem<EntityClass>> {
    ...
    static CProjectItemStatus getInitialStatus(final IHasStatusAndWorkflow<?> entity,
        ...
        Check.notEmpty(initialStatuses,
                "No statuses returned from getValidNextStatuses for entity type " + entity.getClass().getSimpleName());
        return initialStatuses.get(0);
    }

    @SuppressWarnings("null")
    static void initializeNewEntity(final IHasStatusAndWorkflow<?> entity, final CProject currentProject,
            final CTypeEntityService<?> typeService,
            final CProjectItemStatusService projectItemStatusService) {
        ...
        Check.notNull(entity.getWorkflow(),
                "Workflow cannot be null for entity type " + entity.getClass().getSimpleName());
        final CProjectItemStatus initialStatus = getInitialStatus(entity, projectItemStatusService);
        Check.notNull(initialStatus,
                "Initial status cannot be null for entity type " + entity.getClass().getSimpleName());
