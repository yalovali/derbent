...
public abstract class CProjectItemService<EntityClass extends CProjectItem<EntityClass>> extends CEntityOfProjectService<EntityClass>
	...
	/** Service-level method to copy CProjectItem-specific fields using direct setters/getters. Override in concrete services to add entity-specific
	 * field copying. Always call super.copyEntityFieldsTo() first!
	...
	protected void validateEntity(final EntityClass entity) {
		...
		if (!entityCompany.getId().equals(statusCompany.getId())) {
			throw new CValidationException("Status must belong to the same company as the entity");
		}
	}
}
