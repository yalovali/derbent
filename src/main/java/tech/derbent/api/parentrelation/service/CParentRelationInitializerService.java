 ...
 * screens. */
public final class CParentRelationInitializerService extends COneToOneRelationBaseInitializationService {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(CParentRelationInitializerService.class);
	...
	public static void addDefaultChildrenSection(final CDetailSection detailSection, final Class<?> entityClass,
		...
		detailSection.addScreenLine(
				CDetailLinesService.createLineFromDefaults(entityClass, FIELD_NAME_PARENT_CHILDREN_PLACEHOLDER));
	}

	/** Add a default parent relation section to an entity detail screen. Checks if entity class implements IHasParentRelation and adds the parent
	 * selector component.
	...
	public static void addDefaultSection(final CDetailSection detailSection, final Class<?> entityClass,
		...
		Check.notNull(project, "project cannot be null");
		if (CSpringContext.isBabProfile()) {
			LOGGER.debug("Skipping Parent Hierarchy section for BAB profile on {}", entityClass.getSimpleName());
			return;
		}
