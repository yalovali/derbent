...
/** Repository for CParentRelation entities. */
public interface IParentRelationRepository extends IAbstractRepository<CParentRelation> {

	@Query("SELECT COUNT(pr) FROM CParentRelation pr WHERE pr.parentItemId = :parentItemId")
	...
	@Query("SELECT pr FROM CParentRelation pr WHERE pr.parentItemId = :parentItemId")
	List<CParentRelation> findChildrenByParentId(@Param("parentItemId") Long parentItemId);

	@Query("SELECT pr FROM CParentRelation pr WHERE pr.parentItemId IS NULL")
	List<CParentRelation> findRootItems();
}
