...
public abstract class CPageService<EntityClass extends CEntityDB<EntityClass>>
        implements IPageServiceHasStatusAndWorkflow<EntityClass> {
     ...
     * <p>
     * Default implementation for entities without workflow support. Simply sets the
     * new status on the current entity. Subclasses should override this
    ...
    public void actionCreate() throws Exception {
        try {
            ...
            LOGGER.error("Error creating new entity instance for type: {} - {}", getEntityClass().getSimpleName(),
                    e.getMessage());
            throw e;
        }
    }
    ...
    private void bindComponent(final Method method, final Component component, final String methodName,
        ...
        Check.isTrue(parameters.length == 2,
                "Method {" + methodName + "} has invalid number of parameters. Expected 2 (Component, Object).");
        Check.instanceOf(parameters[0], Component.class,
                "Method {" + methodName + "} has invalid first parameter. Expected Component");
        Check.instanceOf(parameters[1], Object.class,
                "Method {" + methodName + "} has invalid second parameter. Expected Object");
        ...
        switch (action) {
            ...
            case "dragEnd" -> {
                ...
                if (component instanceof IHasDragControl) {
                    LOGGER.debug("[DragDebug] Component {} implements IHasDragEnd, binding via interface",
                            componentName);
                ...
                if (component instanceof IHasSelectionNotification) {
                    ...
                    throw new IllegalArgumentException(String
                            .format("Selection action requires IHasSelectionNotification. Component '%s' is %s which doesn't support selection events. "
                                    + "Handler method '%s' cannot be bound.", componentName,
                                    component.getClass().getSimpleName(), methodName));
                }
            }
            // add more actions as needed
            default -> Check.warn("Action {" + action + "} not recognized for binding.");
        }
        LOGGER.debug("[BindDebug] Successfully bound method {} to component {} for action {}", methodName,
    ...
    private void bindDragEnd(final IHasDragControl component, final Method method, final String methodName) {
        ...
        final ComponentEventListener<CDragEndEvent> listener = dragEndListenerRegistry.computeIfAbsent(key, k -> {
            ...
            return event -> {
                try {
                    LOGGER.debug("[DragDebug] Invoking {} on component {}", methodName,
        ...
        LOGGER.debug("[BindDebug] Bound IHasDragEnd event to method {} (cached={})", methodName,
                dragEndListenerRegistry.containsKey(key));
    }
    ...
    private void bindDragStart(final IHasDragControl component, final Method method, final String methodName) {
        ...
        LOGGER.debug("[BindDebug] Bound IHasDragStart event to method {} (cached={})", methodName,
                dragStartListenerRegistry.containsKey(key));
    }

    /**
     * Binds a component's drop event to a page service handler method. Supports
     * components implementing IHasDrop interface.
     * 
    ...
    private void bindIHasDropEvent(final IHasDragControl component, final Method method, final String methodName) {
        ...
        LOGGER.debug("[BindDebug] Bound IHasDrop drop event to method {} (cached={})", methodName,
                dropListenerRegistry.containsKey(key));
    }
    ...
    private void bindSelection(final IHasSelectionNotification component, final Method method,
        ...
        LOGGER.debug("[BindDebug] Bound selection event to method {} (cached={})", methodName,
                selectListenerRegistry.containsKey(key));
    }

    protected CDragStartEvent getActiveDragStartEvent() {
        return activeDragStartEvent;
    }
    ...
    protected <T> ComboBox<T> getComboBox(final String fieldName) {
        ...
        Check.notNull(component,
                String.format("Component '%s' not found. Check component registration and field name.", fieldName));
        ...
        throw new IllegalArgumentException(
                String.format("Component '%s' is not a ComboBox. Found: %s. Expected ComboBox or CNavigableComboBox.",
    ...
    protected Object getComponentValue(final String fieldName) {
        final Component component = getComponentByName(fieldName);
        Check.notNull(component,
                String.format("Cannot get value: Component '%s' not found. Check component registration.", fieldName));
    ...
    public CProject getCurrentProject() {
        return getSessionService().getActiveProject().orElse(null);
    }
    ...
    public EntityClass getPreviousEntity() {
    ...
    protected ISessionService getSessionService() {
        return getView().getSessionService();
    }
    ...
    protected EntityClass getValue() {
    ...
    /**
     * Update the save button state based on validation results. This method is
     * called automatically when the name field changes.
     */
    ...
    /**
     * Registers a custom component for method binding. This allows components that
     * are not part of the entity form to be bound to handler methods
    ...
    protected void setActiveDragStartEvent(final CDragStartEvent activeDragStartEvent) {
        this.activeDragStartEvent = activeDragStartEvent;
    }
    ...
    protected void setComponentValue(final String fieldName, final Object value) {
        final Component component = getComponentByName(fieldName);
        Check.notNull(component,
                String.format("Cannot set value: Component '%s' not found. Check component registration.", fieldName));
        Check.instanceOf(component, HasValue.class,
                String.format("Component '%s' does not support setting value (not a HasValue)", fieldName));
        try {
            ((HasValue) component).setValue(value);
        } catch (final Exception e) {
            LOGGER.error("Error setting value for component '{}': {}. Value type: {}, Component type: {}", fieldName,
                    e.getMessage(),
    ...
    public void setPreviousEntity(final EntityClass previousEntity) {
    ...
    /**
     * Validate the current entity. Default implementation checks that the name
     * field is not empty. Subclasses can override this method to add
