...
public class CProjectTypeInitializerService extends CEntityTypeInitializerService {
	...
	public static CProjectType initializeSampleBab(final CCompany company, final boolean minimal) throws Exception {
		...
		return service.listByCompany(company).stream().map(item -> (CProjectType) item)
				.filter(type -> BAB_PROJECT_TYPE_NAME.equals(type.getName())).findFirst()
