...
/**
 * Utility class for image processing operations including resizing, format conversion,
 * and validation. Follows coding guidelines by using proper logging, validation, and
 * exception handling.
 */
public final class CImageUtils {
	...
	private static String detectImageMimeType(final byte[] imageData) {
		...
		final String start =
			new String(imageData, 0, Math.min(20, imageData.length)).toLowerCase();
		...
		if ((imageData.length >= 4) && ((imageData[0] & 0xFF) == 0x89)
			&& ((imageData[1] & 0xFF) == 0x50) && ((imageData[2] & 0xFF) == 0x4E)
			&& ((imageData[3] & 0xFF) == 0x47)) {
			return "image/png";
		}

		// Check for JPEG signature (FF D8)
		if ((imageData.length >= 2) && ((imageData[0] & 0xFF) == 0xFF)
	 ...
	 */
	public static byte[] resizeImage(final byte[] imageData, final int targetWidth,
		final int targetHeight) throws IOException {
		LOGGER.info("CImageUtils.resizeImage called with targetWidth={}, targetHeight={}",
		...
		try {
			...
			LOGGER.debug("Original image dimensions: {}x{}", originalImage.getWidth(),
			...
			final Image scaledImage = originalImage.getScaledInstance(targetWidth,
			...
			LOGGER.info("Image resized successfully from {} bytes to {} bytes",
				imageData.length, resizedImageData.length);
	 ...
	 * @param fileName  Original file name for format validation
	...
	public static void validateImageData(final byte[] imageData, final String fileName) {
		...
		if (imageData.length > MAX_IMAGE_SIZE) {
			throw new IllegalArgumentException(String.format(
				"Image size (%d bytes) exceeds maximum allowed size (%d bytes)",
		...
		if (!isSupported) {
			throw new IllegalArgumentException(
				String.format("Unsupported image format: %s. Supported formats: %s",
		...
		try {
			...
			if (image == null) {
				throw new IllegalArgumentException(
					"Invalid image data - cannot read image");
			}
		} catch (final IOException e) {
			throw new IllegalArgumentException("Invalid image data: " + e.getMessage(),
				e);
		}
