...
/** Test page to demonstrate user avatar functionality using Vaadin's Avatar component. Shows how user avatars are displayed with initials and
 * colors. */
...
public class CUserIconTestPage extends Div {
	...
	private static HorizontalLayout createAvatarDisplay(final CUser user) {
		...
		final Span avatarInfoSpan = new Span(user.getProfilePictureThumbnail() != null ? "Has profile picture" : "Showing initials with color");
	...
	private static VerticalLayout createSection(final String title, final String description) {
	...
	public CUserIconTestPage(@Autowired final CUserService userService) {
		...
		try {
			...
			if (users.isEmpty()) {
				...
				mainLayout.add(createSection("1. Individual Avatar Display", "Each user avatar displayed using the getAvatar() method"));
				...
				mainLayout.add(createSection("2. CLabelEntity Display", "Standard way to display users in the application with icons and names"));
				...
				for (int i = 0; i < displayCount; i++) {
					...
					userLabel.getStyle().set("padding", "8px").set("background-color", "#f5f5f5").set("border-radius", "4px");
					labelSection.add(userLabel);
				}
				mainLayout.add(labelSection);
				// Section 3: Multiple Avatars in Row
				mainLayout.add(createSection("3. Multiple Avatars in Row", "Demonstrates avatar rendering in horizontal layout"));
				...
				for (int i = 0; i < displayCount; i++) {
					final CUser user = users.get(i);
					final Avatar avatar = user.getAvatar();
					avatar.getStyle().set("border", "2px solid #ddd").set("border-radius", "50%");
					avatarRow.add(avatar);
				}
				mainLayout.add(avatarRow);
				// Section 4: Technical Details
				mainLayout.add(createSection("4. Technical Details", "Information about the Avatar component implementation"));
				...
				technicalInfo.getElement().setProperty("innerHTML",
						"<ul style='line-height: 1.8; color: #555;'>"
								...
								+ "<li><strong>Component:</strong> Native Vaadin Avatar - no custom SVG hacks needed</li>" + "</ul>");
