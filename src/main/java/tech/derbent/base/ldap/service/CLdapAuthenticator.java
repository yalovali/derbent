package tech.derbent.base.ldap.service;
...
/**
 * CLdapAuthenticator - LDAP authentication utility using JNDI.
 * 
 * Provides LDAP bind authentication for users marked as LDAP users.
 * Uses javax.naming JNDI (built into Java - no external dependencies).
 * 
 * Authentication Flow:
 * 1. Read LDAP configuration from system settings
 * 2. Connect to LDAP server
 * 3. Build user DN from username
...
public class CLdapAuthenticator {
	...
	public boolean authenticate(final String username, final String password, final CSystemSettings<?> settings) {
		...
		try {
			...
			LOGGER.error("❌ LDAP authentication ERROR for user '{}': Cannot connect to LDAP server '{}'", 
					username, serverUrl);
	...
	/**
	 * Build user DN from username and filter pattern.
	 * 
	 * Examples:
	 * - Filter "(uid={0})" + username "john" + base "ou=users,dc=company,dc=com"
	 *   → "uid=john,ou=users,dc=company,dc=com"
	 * 
	 * - Filter "(sAMAccountName={0})" + username "john" + base "ou=users,dc=company,dc=com"
	 ...
	 */
	private DirContext createContext(final String serverUrl, final String userDn, final String password) 
