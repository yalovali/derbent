 ...
 * navigation menu and a user menu.
 */
...
public final class MainLayout extends AppLayout implements AfterNavigationObserver {
	...
	private Div createAppMarker() {
		...
		slidingHeader.addClassNames(Display.FLEX, AlignItems.CENTER,
			Margin.Horizontal.MEDIUM, Gap.SMALL);
	...
	/**
	 * Creates the side navigation menu. The navigation menu is dynamically populated with
	 * menu entries from `MenuConfiguration`. Each entry is represented as a `SideNavItem`
	 * with optional icons.
	...
	private SideNav createSideNav() {
		...
		MenuConfiguration.getMenuEntries()
			.forEach(entry -> createSideNavItem(nav, entry)); // Add menu entries
		return nav;
	}
	...
	private void createSideNavItem(final SideNav nav, final MenuEntry menuEntry) {
		...
		if (title.contains(".")) {
			...
			final String parent_title = parts[0]; // Use the first part as the parent
													// title find the parent menu entry
	...
	private Component createUserMenu() {
		...
		if (user != null) {
			...
			avatar.setAbbreviation(user.getUsername().length() > 0
		...
		try {
			...
			LOGGER.warn("Error loading user profile picture, using default: {}",
				e.getMessage());
		}
	...
	/**
	 * Sets the avatar image based on the user's profile picture data. This method
	 * properly creates a StreamResource for the Avatar component.
		...
		if (profilePictureData != null && profilePictureData.length > 0) {
			...
			try {
				...
				final StreamResource imageResource =
					new StreamResource("profile-" + user.getId() + ".jpg",
				...
				LOGGER.debug(
					"Set avatar image from user profile picture data for user: {}",
					user.getLogin());
				return;
			} catch (final Exception e) {
				LOGGER.warn("Error creating StreamResource from profile picture: {}",
					e.getMessage());
			}
		}
