...
public abstract class CBaseKanbanColumn<T extends CKanbanEntity, S extends CKanbanStatus> extends Div {
    ...
    protected CBaseKanbanColumn(final S status, final List<T> entities) {
        LOGGER.debug("Creating CBaseKanbanColumn for status: {} with {} entities",
                status != null ? status.getName() : "null", entities != null ? entities.size() : 0);
    ...
    private void createTypeSection(final String typeName, final List<T> typeEntities) {
        ...
        for (final T entity : typeEntities) {
            try {
                ...
                LOGGER.error("Error creating card for entity: {} with type: {}",
                        entity != null ? entity.getName() : "null", typeName, e);
            }
        }
    ...
    private void populateCards() {
        ...
        final Map<String, List<T>> entitiesByType = entities.stream()
                .collect(Collectors.groupingBy(this::getEntityTypeName, Collectors.toList()));
