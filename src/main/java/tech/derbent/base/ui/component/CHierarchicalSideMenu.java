...
/**
 * CHierarchicalSideMenu - A hierarchical side menu component with up to 4 levels of
 * navigation. Layer: View (MVC) Features: - Supports up to 4 levels of menu hierarchy -
 * Sliding animations between levels - Back button navigation - Parses menu entries from
 * route annotations in format: parentItem2.childItem1.childofchileitem1 - Responsive
 * design with proper styling - Current page highlighting
 */
public final class CHierarchicalSideMenu extends Div implements AfterNavigationObserver {
	...
	private final class CMenuItem {
		...
		public CMenuItem(final String name, final String iconName, final String path,
			...
			if (iconName.startsWith("class:")) {
				this.iconName =
					CIconSetLoader.getIconFilename(iconName.replace("class:", ""));
				this.iconColor =
					CIconSetLoader.getIconColorCode(iconName.replace("class:", ""));
			}
			else {
				this.iconName = iconName;
				this.iconColor = getIconColor(iconName);
			}
			this.path = path;
		...
		public Component createComponent() {
			...
			itemText.addClassNames(FontSize.LARGE,
			...
			// Style as clickable
			itemLayout.getElement().getStyle().set("cursor", "pointer")
			...
			if (!isCurrentPage) {
				itemLayout.getElement().addEventListener("mouseenter",
					e -> itemLayout.getElement().getStyle().set("background-color",
		...
		private void handleItemClick(
			final com.vaadin.flow.component.ClickEvent<HorizontalLayout> event) {
	...
	private final class CMenuLevel {
		...
		public CMenuLevel(final String levelKey, final String displayName,
			final CMenuLevel parent) {
		...
		public void addMenuItem(final String name, final String iconName,
			final String path) {
		...
		public void addNavigationItem(final String name, final String iconName,
			final String targetLevelKey) {
			final CMenuItem item =
				new CMenuItem(name, iconName, null, targetLevelKey, true);
			items.add(item);
	...
	public CHierarchicalSideMenu() {
		LOGGER.info("Initializing CHierarchicalSideMenu with up to {} levels",
			MAX_MENU_LEVELS);
		...
		headerLayout.addClassNames(Display.FLEX, AlignItems.CENTER, Padding.MEDIUM,
			HEADER_CLASS);
		...
		showLevel("root");
		LOGGER.info("CHierarchicalSideMenu initialized successfully with {} menu levels",
			menuLevels.size());
	}
	...
	/**
	 * Builds the menu hierarchy from route annotations. Parses menu entries in format:
	 * parentItem2.childItem1.childofchileitem1
	 */
	...
	private String getIconColor(final String iconName) {
		...
		// Activities - Blue
		if ("calendar-clock".equals(cleanIconName) || "tasks".equals(cleanIconName)
			|| "flag".equals(cleanIconName)) {
			return "#007bff";
		}

		// Projects - Orange
		if ("briefcase".equals(cleanIconName) || "folder".equals(cleanIconName)
			|| "dashboard".equals(cleanIconName) || "grid-big".equals(cleanIconName)) {
			return "#fd7e14";
		}
		...
		if ("cogs".equals(cleanIconName) || "building".equals(cleanIconName)
			|| "tags".equals(cleanIconName)) {
			return "#6c757d";
		}
		...
		if ("home".equals(cleanIconName) || "cubes".equals(cleanIconName)
			|| "arrow-left".equals(cleanIconName) || "tree-table".equals(cleanIconName)
			|| "chevron-right".equals(cleanIconName)) {
	...
	private void handleBackButtonClick(
		final ClickEvent<com.vaadin.flow.component.button.Button> event) {
		LOGGER.debug("Back button clicked from level: {}",
			currentLevel != null ? currentLevel.getLevelKey() : "unknown");
	...
	private void initializeStyles() {
		...
		getElement().getStyle().set("overflow", "hidden").set("transition",
	...
	private void processMenuEntry(final MenuEntry menuEntry) {
		...
		// LOGGER.debug("Processing menu entry - title: {}, path: {}, icon: {}", title,
		// path, icon);
		...
		for (int i = 0; i < (levelCount - 1); i++) {
			...
			if (!menuLevels.containsKey(childLevelKey)) {
				...
				final CMenuLevel newLevel =
					new CMenuLevel(childLevelKey, levelName, parentLevel);
	...
	private void updateHeader(final CMenuLevel level) {
		...
		if (level.getParent() != null) {
			...
			final CButton backButton =
				new CButton(levelIcon, this::handleBackButtonClick);
