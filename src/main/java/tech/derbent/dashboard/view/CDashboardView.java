...
/**
 * CDashboardView - Dashboard view showing system summary statistics. Layer: View (MVC)
...
public final class CDashboardView extends CAbstractPage {
	...
	public CDashboardView(final CProjectService projectService,
		...
		LOGGER.info(
			"CDashboardView constructor called with projectService={}, userService={}, activityService={}",
			projectService != null ? projectService.getClass().getSimpleName() : "null",
			userService != null ? userService.getClass().getSimpleName() : "null",
			activityService != null ? activityService.getClass().getSimpleName()
				: "null");
		...
		if (activityService == null) {
			throw new IllegalArgumentException("ActivityService cannot be null");
		}
	...
	public final void beforeEnter(final BeforeEnterEvent event) {
		LOGGER.debug("beforeEnter called for CDashboardView with event: {}",
			event != null ? event.getClass().getSimpleName() : "null");

		if (event == null) {
			LOGGER.warn("BeforeEnterEvent is null");
			return;
		}
		// Additional navigation logic can be added here if needed
		LOGGER.debug("Navigation event handled successfully");
	}
	 ...
	 * @return Map containing project names as keys and activity counts as values, never
	 *         null
	 */
	private final Map<String, Long>
		calculateActivitiesByProject(final List<CProject> projects) {
		LOGGER.debug("calculateActivitiesByProject called with {} projects",
			projects != null ? projects.size() : 0);

		if (projects == null) {
			LOGGER.warn("Projects list is null, returning empty map");
			return Collections.emptyMap();
		}

		try {
			return projects.stream()
				.filter(project -> (project != null) && (project.getName() != null))
				.collect(Collectors.toMap(CProject::getName, project -> {
					...
					try {
						...
						final var activities = activityService.findEntitiesByProject(project);
						final long activityCount =
							activities != null ? activities.size() : 0L;
						LOGGER.debug("Project {} has {} activities", project.getName(),
							activityCount);
						return activityCount;
					} catch (final Exception e) {
						LOGGER.warn("Error calculating activities for project {}: {}",
	 ...
	 */
	private final Map<String, Long>
		calculateUsersByProject(final List<CProject> projects) {
		LOGGER.debug("calculateUsersByProject called with {} projects",
			projects != null ? projects.size() : 0);
			...
			return projects.stream()
				...
				.collect(Collectors.toMap(CProject::getName, project -> {
					...
					try {
						...
						final long userCount = allUsers.stream()
							.filter(user -> (user != null)
								&& (user.getProjectSettings() != null))
						...
						LOGGER.warn("Error calculating users for project {}: {}",
							project.getName(), e.getMessage());
						return 0L;
					}
				}));
	 ...
	 * @param activitiesByProject Map containing activity counts per project, must not be
	 *                            null
	...
	private final Div createActivitiesCard(final Map<String, Long> activitiesByProject) {
		LOGGER.debug("createActivitiesCard called with activitiesByProject size={}",
			activitiesByProject != null ? activitiesByProject.size() : 0);

		if (activitiesByProject == null) {
			LOGGER.warn("activitiesByProject is null");
			return null;
		}
		...
		try {
			...
			final long totalActivities =
				activitiesByProject.values().stream().mapToLong(Long::longValue).sum();
			final Span count = new Span(String.valueOf(totalActivities));

			if (count != null) {
				count.addClassNames(LumoUtility.FontSize.XXXLARGE,
	...
	private final Div createCard() {
		...
		try {
			...
			card.addClassNames(LumoUtility.Background.CONTRAST_5,
				LumoUtility.BorderRadius.LARGE, LumoUtility.Padding.LARGE,
			...
			card.getElement().addEventListener("mouseenter", e -> {
				...
				if (card.getStyle() != null) {
					card.getStyle().set("transform", "translateY(-2px)").set("box-shadow",
			...
			card.getElement().addEventListener("mouseleave", e -> {
				...
				if (card.getStyle() != null) {
					card.getStyle().set("transform", "translateY(0)").set("box-shadow",
	...
	/**
	 * Creates the detailed project breakdown section showing individual project
	 * statistics.
	 ...
	 */
	private final Div createDetailedBreakdown(final List<CProject> projects,
		final Map<String, Long> usersByProject,
		final Map<String, Long> activitiesByProject) {
		LOGGER.debug("createDetailedBreakdown called with {} projects",
			projects != null ? projects.size() : 0);

		if ((projects == null) || (usersByProject == null)
			|| (activitiesByProject == null)) {
			LOGGER.warn(
				"One or more parameters are null - projects: {}, usersByProject: {}, activitiesByProject: {}",
				projects != null, usersByProject != null, activitiesByProject != null);
			return null;
		}
		...
		try {
			...
			if (projectList != null) {
				...
				for (final CProject project : projects) {
					...
					if (project != null) {
						final Div projectCard = createProjectBreakdownCard(project,
							usersByProject, activitiesByProject);
	...
	/**
	 * Creates a card for individual project breakdown showing project name and
	 * statistics.
	 ...
	 */
	private final Div createProjectBreakdownCard(final CProject project,
		final Map<String, Long> usersByProject,
		final Map<String, Long> activitiesByProject) {
		LOGGER.debug("createProjectBreakdownCard called for project: {}",
			project != null ? project.getName() : "null");

		if ((project == null) || (usersByProject == null)
			|| (activitiesByProject == null)) {
		...
		try {
			...
			if (content != null) {
				...
				final String projectName =
					project.getName() != null ? project.getName() : "Unknown Project";
				final Span projectNameSpan = new Span(projectName);

				if (projectNameSpan != null) {
					projectNameSpan.addClassNames(LumoUtility.FontWeight.SEMIBOLD,
						LumoUtility.FontSize.LARGE);
				}
				// Stats
				final HorizontalLayout stats =
					createProjectStats(project, usersByProject, activitiesByProject);
	 ...
	 */
	private final HorizontalLayout createProjectStats(final CProject project,
		final Map<String, Long> usersByProject,
		final Map<String, Long> activitiesByProject) {
		LOGGER.debug("createProjectStats called for project: {}",
			project != null ? project.getName() : "null");
		...
		try {
			...
			if (projectName != null) {
				...
				final long activityCount =
					activitiesByProject.getOrDefault(projectName, 0L);
	...
	private final HorizontalLayout createStatsContainer() {
		...
		try {
			...
			if (statsContainer != null) {
				...
				statsContainer.setDefaultVerticalComponentAlignment(
	...
	private final Div createUsersCard(final Map<String, Long> usersByProject) {
		LOGGER.debug("createUsersCard called with usersByProject size={}",
			usersByProject != null ? usersByProject.size() : 0);
		...
		try {
			...
			final long totalUsers =
				usersByProject.values().stream().mapToLong(Long::longValue).sum();
	...
	private final void handleError(final String message, final Exception exception) {
		LOGGER.error("handleError called with message: {}, exception: {}", message,
	...
	/**
	 * Initializes the page with dashboard components and layout. Sets up the main
	...
	protected final void initPage() {
		...
		try {
			...
			final Paragraph subtitle =
				new Paragraph("Overview of projects, users, and activities");
	...
	/**
	 * Loads and displays the dashboard data including statistics cards and project
	 * breakdown. Handles any errors gracefully and displays them to the user.
	 */
	private final void loadDashboardData() {
		...
		try {
			...
			final List<CProject> projects = projectService != null
				? projectService.findAll() : Collections.emptyList();
			...
			final Map<String, Long> activitiesByProject =
				calculateActivitiesByProject(projects);
			...
			if (!projects.isEmpty() && (usersByProject != null)
				&& (activitiesByProject != null)) {
				final Div breakdown = createDetailedBreakdown(projects, usersByProject,
					activitiesByProject);
	...
	/**
	 * Sets up the toolbar for the dashboard page. Currently no toolbar is needed for the
	 * dashboard.
	 */
	@Override
