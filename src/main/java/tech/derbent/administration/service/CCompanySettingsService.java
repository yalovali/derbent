...
/**
 * CCompanySettingsService - Business logic layer for CCompanySettings entities. Layer: Service (MVC) Provides
 * comprehensive business logic for managing company-wide administration settings, including CRUD operations,
 * validation, and specialized business methods for settings management.
 */
@Service
...
public class CCompanySettingsService extends CAbstractService<CCompanySettings> {
     ...
     * 
     * @param repository
     *            the CCompanySettingsRepository instance
     * @param clock
    ...
    /**
     * Creates default company settings for a company. If settings already exist, returns the existing settings.
     * 
     * @param company
     *            the company to create settings for
     ...
     * @throws IllegalArgumentException
     *             if companyId is null or invalid
     */
    ...
    public Optional<CCompanySettings> findByCompany(final CCompany company) {
        LOGGER.debug("findByCompany called with company: {}", company != null ? company.getName() : "null");
    ...
    /**
     * Finds all companies with email notifications enabled. Useful for system-wide email processing.
     * 
    ...
    @Override
    protected Class<CCompanySettings> getEntityClass() {
        return CCompanySettings.class;
    }

    /**
     * Gets or creates company settings for a company. If settings don't exist, creates default settings.
     * 
     * @param company
     *            the company to get or create settings for
     ...
     * @throws IllegalArgumentException
     *             if settings or company is null
     * @throws EntityNotFoundException
     *             if settings don't exist in database
     */
    @Transactional
    public CCompanySettings updateSettings(final CCompanySettings settings) {
        LOGGER.debug("updateSettings called with settings ID: {}", settings != null ? settings.getId() : "null");
    ...
    private void validateSettingsBusinessRules(final CCompanySettings settings) {
        ...
        if ((settings.getWorkingHoursPerDay() != null) && ((settings.getWorkingHoursPerDay().doubleValue() <= 0)
