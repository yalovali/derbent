...
/**
 * CCompanySettingsService - Business logic layer for CCompanySettings entities. Layer:
 * Service (MVC) Provides comprehensive business logic for managing company-wide
 * administration settings, including CRUD operations, validation, and specialized
 * business methods for settings management.
 */
@Service
...
public class CCompanySettingsService extends CAbstractService<CCompanySettings> {
	 ...
	 */
	public CCompanySettingsService(final CCompanySettingsRepository repository,
		final Clock clock) {
	...
	/**
	 * Creates default company settings for a company. If settings already exist, returns
	...
	public CCompanySettings createDefaultSettingsForCompany(final CCompany company) {
		...
		final Optional<CCompanySettings> existingSettings =
	...
	/**
	 * Finds all companies with email notifications enabled. Useful for system-wide email
	 * processing.
	...
	private void validateSettingsBusinessRules(final CCompanySettings settings) {
		...
		if ((settings.getWorkingHoursPerDay() != null)
			...
			throw new IllegalArgumentException(
				"Working hours per day must be between 0.1 and 24.0");
		}

		// Validate working days
		if ((settings.getWorkingDaysPerWeek() != null)
			&& ((settings.getWorkingDaysPerWeek() < 1)
				|| (settings.getWorkingDaysPerWeek() > 7))) {
			throw new IllegalArgumentException(
				"Working days per week must be between 1 and 7");
		}

		// Validate work hours
		if ((settings.getStartWorkHour() != null)
			&& (settings.getEndWorkHour() != null)) {

			if (settings.getStartWorkHour() >= settings.getEndWorkHour()) {
				throw new IllegalArgumentException(
					"Start work hour must be before end work hour");
			}
		}

		// Validate reminder days
		if ((settings.getDueDateReminderDays() != null)
			&& (settings.getDueDateReminderDays() < 0)) {
			throw new IllegalArgumentException(
				"Due date reminder days cannot be negative");
		}
