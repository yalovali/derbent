...
public class CCompanySettingsService extends CAbstractService<CCompanySettings> {
	...
	public CCompanySettings createDefaultSettingsForCompany(final CCompany company) {
		...
		Check.isTrue(existingSettings.isEmpty(), "Company settings already exist for company: " + company.getName());
		try {
	...
	public CCompanySettings getOrCreateSettingsForCompany(final CCompany company) {
		LOGGER.debug("getOrCreateSettingsForCompany called with company: {}", company != null ? company.getName() : "null");
		if (company == null) {
	...
	private void validateSettingsBusinessRules(final CCompanySettings settings) {
		...
		if ((settings.getWorkingHoursPerDay() != null)
				&& ((settings.getWorkingHoursPerDay().doubleValue() <= 0) || (settings.getWorkingHoursPerDay().doubleValue() > 24))) {
			throw new IllegalArgumentException("Working hours per day must be between 0.1 and 24.0");
		}
		// Validate working days
		if ((settings.getWorkingDaysPerWeek() != null) && ((settings.getWorkingDaysPerWeek() < 1) || (settings.getWorkingDaysPerWeek() > 7))) {
		...
		if ((settings.getCompanyThemeColor() != null) && !settings.getCompanyThemeColor().matches("^#[0-9A-Fa-f]{6}$")) {
