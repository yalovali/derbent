...
/**
 * CCompanySettingsView - View for managing company-wide administration settings.
 * Layer: View (MVC)
 * 
 * Provides a comprehensive interface for administrators to configure company-wide
 * settings including workflow defaults, working hours, notifications, project
 * management preferences, and UI customizations. Each company has one settings record.
 */
...
public final class CCompanySettingsView extends CAbstractMDPage<CCompanySettings> {
    ...
    /**
     * Constructor for CCompanySettingsView.
    ...
    protected void createDetailsLayout() {
        LOGGER.info("Creating details layout for CCompanySettingsView");
        createEntityDetails();
    }

    /**
     * Creates the entity details form using MetaData annotations.
     * The form is automatically generated based on the CCompanySettings entity annotations.
     */
    protected void createEntityDetails() {
        LOGGER.info("Creating entity details for CCompanySettingsView");
        ...
        try {
            ...
            Notification.show("Error creating form: " + e.getMessage(), 3000, 
    ...
    protected void createGridForEntity() {
        ...
        try {
            ...
            grid.addColumn(settings -> settings.getCompany() != null ? settings.getCompany().getName() : "")
                .setHeader("Company")
                .setSortable(true)
                .setAutoWidth(true);
                
            grid.addColumn(CCompanySettings::getDefaultProjectStatus)
                .setHeader("Default Project Status")
                .setSortable(true)
                .setAutoWidth(true);
                
            grid.addColumn(CCompanySettings::getCompanyTimezone)
                .setHeader("Timezone")
                .setSortable(true)
                .setAutoWidth(true);
            ...
            grid.addColumn(settings -> settings.isEmailNotificationsEnabled() ? "Enabled" : "Disabled")
                .setHeader("Email Notifications")
                .setSortable(true)
                .setAutoWidth(true);
            ...
            Notification.show("Error configuring grid: " + e.getMessage(), 3000, 
                            Notification.Position.MIDDLE);
        }
    }
    ...
    protected CButton createSaveButton(final String buttonText) {
        ...
        return CButton.createPrimary(buttonText, event -> {
            ...
            try {
                ...
                refreshGrid();
                Notification.show("Company settings saved successfully", 3000, 
    ...
    protected CButton createDeleteButton(final String buttonText) {
        LOGGER.debug("createDeleteButton called with buttonText: {}", buttonText);
        
        return CButton.createError("Reset to Defaults", event -> {
    ...
    private void performReset(final CCompanySettings entity) {
        LOGGER.debug("performReset called for company: {}", 
                    entity.getCompany() != null ? entity.getCompany().getName() : "unknown");
        ...
        try {
            ...
            // Copy the ID from the existing entity (since CEntityDB doesn't have public setId)
            // We'll update through the service which will handle the ID properly
            ...
            Notification.show("Settings reset to defaults successfully", 3000, 
                            Notification.Position.TOP_CENTER);
