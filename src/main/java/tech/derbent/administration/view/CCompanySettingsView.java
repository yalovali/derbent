 ...
 * View (MVC) Provides a comprehensive interface for administrators to configure
 * company-wide settings including workflow defaults, working hours, notifications,
 * project management preferences, and UI customizations. Each company has one settings
 * record.
 */
@Route ("administration/company-settings/:company_settings_id?/:action?(edit)")
@PageTitle ("Company Administration Settings")
@Menu (
	order = 3.5, icon = "class:tech.derbent.administration.view.CCompanySettingsView",
...
public class CCompanySettingsView extends CAbstractEntityDBPage<CCompanySettings>
	...
	private final String ENTITY_ROUTE_TEMPLATE_EDIT =
		"administration/company-settings/%s/edit";
	...
	/**
	 * Creates the entity details form using MetaData annotations. The form is
	 * automatically generated based on the CCompanySettings entity annotations.
	 */
	@Override
	protected void createDetailsLayout() {
		...
		try {
			...
			final var formLayout =
				CEntityFormBuilder.buildForm(CCompanySettings.class, getBinder());
		...
		return CButton.createPrimary(buttonText, event -> {
			...
			try {
				...
				final var savedEntity =
					companySettingsService.updateSettings(currentEntity);
				...
				LOGGER.info("Company settings saved successfully for company: {}",
					savedEntity.getCompany() != null ? savedEntity.getCompany().getName()
						: "unknown");
	...
	public void onLayoutModeChanged(
		final tech.derbent.session.service.LayoutService.LayoutMode newMode) {
		LOGGER.debug("onLayoutModeChanged called for CCompanySettingsView with mode: {}",
			newMode);
	...
	private void performReset(final CCompanySettings entity) {
		...
		try {
			...
			Notification.show("Settings reset to defaults successfully", 3000,
				Notification.Position.TOP_CENTER);
			LOGGER.info("Company settings reset to defaults for company: {}",
				company.getName());
		} catch (final Exception e) {
