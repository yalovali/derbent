...
public interface CMeetingStatusRepository
	 ...
	 */
	@Query (
		"SELECT s FROM CMeetingStatus s WHERE s.isFinal = false ORDER BY s.sortOrder ASC"
	)
	 ...
	 */
	@Query (
		"SELECT s FROM CMeetingStatus s WHERE s.project = :project ORDER BY s.sortOrder ASC, s.name ASC"
	)
	 ...
	 */
	@Query (
		"SELECT s FROM CMeetingStatus s WHERE s.isFinal = true ORDER BY s.sortOrder ASC"
	)
	 ...
	 * @param project the project to search within - may be null for global statuses
	 * @return Optional containing the status if found, empty otherwise
	 */
	@Query (
		"SELECT s FROM CMeetingStatus s WHERE LOWER(s.name) = LOWER(:name) AND s.project = :project"
	)
	Optional<CMeetingStatus> findByNameAndProject(@Param ("name") String name,
	 ...
	 */
	@Query (
		"SELECT s FROM CMeetingStatus s WHERE LOWER(s.name) IN ('planned', 'scheduled', 'new', 'pending') ORDER BY s.sortOrder ASC"
	)
