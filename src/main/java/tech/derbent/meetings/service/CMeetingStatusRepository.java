 ...
 * access operations for meeting status management. Since CMeetingStatus extends CStatus which extends CTypeEntity which
 * extends CEntityOfProject, this repository must extend CEntityOfProjectRepository to provide project-aware operations.
 */
@Repository
public interface CMeetingStatusRepository extends CEntityOfProjectRepository<CMeetingStatus> {

    /**
     * Find all final status types (completed/cancelled) for a specific project. This replaces the non-project-aware
     * findAllFinalStatuses method.
     * 
     * @param project
     *            the project to filter by
     * @return List of final status types for the project, ordered by sortOrder
     */
    @Query("SELECT s FROM CMeetingStatus s WHERE s.finalStatus = true AND s.project = :project ORDER BY s.sortOrder ASC")
     ...
     */
    @Query("SELECT s FROM CMeetingStatus s WHERE LOWER(s.name) IN ('planned', 'scheduled', 'new', 'pending') AND s.project = :project ORDER BY s.sortOrder ASC")
