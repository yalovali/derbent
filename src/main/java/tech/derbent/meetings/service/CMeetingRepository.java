...
public interface CMeetingRepository extends CEntityOfProjectRepository<CMeeting> {
	@Override
	@Query (
		"SELECT m FROM CMeeting m " + "LEFT JOIN FETCH m.meetingType " + "LEFT JOIN FETCH m.status " + "LEFT JOIN FETCH m.responsible "
				+ "LEFT JOIN FETCH m.relatedActivity " + "LEFT JOIN FETCH m.project " + "WHERE m.id = :id"
	)
	...
	@Query (
		"SELECT m FROM CMeeting m " + "LEFT JOIN FETCH m.meetingType " + "LEFT JOIN FETCH m.status " + "LEFT JOIN FETCH m.responsible "
				+ "LEFT JOIN FETCH m.project " + "WHERE m.project = :project"
	)
