...
public interface CMeetingRepository extends CEntityOfProjectRepository<CMeeting> {
     ...
     */
    @Query("SELECT DISTINCT m FROM CMeeting m " + "LEFT JOIN FETCH m.meetingType " + "LEFT JOIN FETCH m.status "
    ...
    /**
     * Find meeting by ID with eager loading of commonly accessed associations. This prevents N+1 queries when accessing
     * meeting details.
     * 
     * @param id
     *            the meeting ID
     ...
     */
    @Query("SELECT m FROM CMeeting m " + "LEFT JOIN FETCH m.meetingType " + "LEFT JOIN FETCH m.status "
            + "LEFT JOIN FETCH m.responsible " + "LEFT JOIN FETCH m.relatedActivity " + "LEFT JOIN FETCH m.project "
            + "WHERE m.id = :id")
    Optional<CMeeting> findByIdWithEagerLoading(@Param("id") Long id);

    /**
     * Find meetings by project with eager loading for list view. Optimized for displaying meeting lists without loading
     * collections.
     * 
     * @param project
     *            the project
    ...
    @Override
    @Query("SELECT m FROM CMeeting m " + "LEFT JOIN FETCH m.meetingType " + "LEFT JOIN FETCH m.status "
            + "LEFT JOIN FETCH m.responsible " + "LEFT JOIN FETCH m.project " + "WHERE m.project = :project")
