...
public class CMeetingService extends CAbstractNamedEntityService<CMeeting> {
    ...
    /**
     * Gets a meeting by ID with eagerly loaded CMeetingType and participants relationship.
    ...
    public Optional<CMeeting> getWithMeetingTypeAndParticipants(final Long id) {
        LOGGER.debug("Getting CMeeting with ID {} and eagerly loading CMeetingType and participants", id);
        return ((CMeetingRepository) repository).findByIdWithMeetingTypeAndParticipants(id);
    }

    /**
     * Overrides the base get method to eagerly load CMeetingType and participants relationships.
    ...
    public Optional<CMeeting> get(final Long id) {
        LOGGER.debug("Getting CMeeting with ID {} (overridden to eagerly load meetingType and participants)", id);
        final Optional<CMeeting> entity = ((CMeetingRepository) repository).findByIdWithMeetingTypeAndParticipants(id);
    ...
    protected void initializeLazyFields(final CMeeting entity) {
        ...
        try {
            ...
            // Initialize the lazy-loaded CMeetingType relationship
            initializeLazyRelationship(entity.getMeetingType(), "CMeetingType");
            
            // Initialize the lazy-loaded participants relationship
            initializeLazyRelationship(entity.getParticipants(), "CUser participants");
