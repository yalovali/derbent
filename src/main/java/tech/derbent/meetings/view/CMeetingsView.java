...
public class CMeetingsView extends CProjectAwareMDPage<CMeeting> {
	...
	private boolean matchesSearchText(final CMeeting meeting, final String searchText) {
		...
		if ((meeting.getName() != null)
			&& meeting.getName().toLowerCase().contains(searchText)) {
			return true;
		}

		// Search in meeting type name
		if ((meeting.getMeetingType() != null)
			&& (meeting.getMeetingType().getName() != null)) {
		...
		if (meeting.getMeetingDate() != null) {
			final String formattedStartTime = meeting.getMeetingDate().format(
				java.time.format.DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm"));
		...
		if (meeting.getEndDate() != null) {
			final String formattedEndTime = meeting.getEndDate().format(
				java.time.format.DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm"));
		...
		if (!meeting.getParticipants().isEmpty()) {
			final String participants = meeting.getParticipants().stream().map(
				user -> user.getName() != null ? user.getName() : "User #" + user.getId())
				.collect(java.util.stream.Collectors.joining(", "));
		...
		if ((meeting.getDescription() != null)
			&& meeting.getDescription().toLowerCase().contains(searchText)) {
			return true;
		}
		return false;
	}
	...
	protected void setupToolbar() {
		...
		searchField.setPrefixComponent(new com.vaadin.flow.component.icon.Icon(
		...
		searchField.addValueChangeListener(event -> {
			...
			if (entityService instanceof tech.derbent.abstracts.services.CEntityOfProjectService) {
				final tech.derbent.abstracts.services.CEntityOfProjectService<
					CMeeting> projectService =
						(tech.derbent.abstracts.services.CEntityOfProjectService<
							CMeeting>) entityService;
				final Optional<tech.derbent.projects.domain.CProject> activeProject =
					sessionService.getActiveProject();

				if (activeProject.isPresent()) {
					final List<CMeeting> allMeetings = projectService
						.findEntriesByProject(activeProject.get(), Pageable.unpaged());
					final List<CMeeting> filteredMeetings = allMeetings.stream()
						.filter(meeting -> matchesSearchText(meeting,
