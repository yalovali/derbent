...
public class CMeetingsView extends CProjectAwareMDPage<CMeeting> implements CInterfaceIconSet {
    ...
    public static String getIconFilename() {
        return CMeeting.getIconFilename();
    }
    ...
    private boolean matchesSearchText(final CMeeting meeting, final String searchText) {
        ...
        if (meeting.getMeetingDate() != null) {
            final String formattedStartTime = meeting.getMeetingDate()
                    .format(java.time.format.DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm"));
        ...
        if (meeting.getEndDate() != null) {
            final String formattedEndTime = meeting.getEndDate()
                    .format(java.time.format.DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm"));
        ...
        if (!meeting.getParticipants().isEmpty()) {
            final String participants = meeting.getParticipants().stream()
                    .map(user -> user.getName() != null ? user.getName() : "User #" + user.getId())
                    .collect(java.util.stream.Collectors.joining(", "));
    ...
    protected void setupToolbar() {
        ...
        searchField.setPrefixComponent(
                new com.vaadin.flow.component.icon.Icon(com.vaadin.flow.component.icon.VaadinIcon.SEARCH));
        ...
        searchField.addValueChangeListener(event -> {
            ...
            if (entityService instanceof tech.derbent.abstracts.services.CEntityOfProjectService) {
                final tech.derbent.abstracts.services.CEntityOfProjectService<CMeeting> projectService = (tech.derbent.abstracts.services.CEntityOfProjectService<CMeeting>) entityService;
                final Optional<tech.derbent.projects.domain.CProject> activeProject = sessionService.getActiveProject();

                if (activeProject.isPresent()) {
                    final List<CMeeting> allMeetings = projectService.findEntriesByProject(activeProject.get(),
                            Pageable.unpaged());
