...
/**
 * CDecisionsView - Main view for decision management. Layer: View (MVC) Provides a complete decision management
 * interface following the established patterns from CActivitiesView. Includes grid listing, detail editing, and
 * comprehensive panel organization.
 */
...
public class CDecisionsView extends CProjectAwareMDPage<CDecision> {
     ...
     * 
     * @param entityService
     *            decision service for business logic operations
     * @param sessionService
     *            session service for project context
     */
    public CDecisionsView(final CDecisionService entityService, final CSessionService sessionService) {
        super(CDecision.class, entityService, sessionService);
        addClassNames("decisions-view");
    }

    /**
     * Creates the entity details section using decision panels. Follows the same pattern as CActivitiesView for
     * consistency.
     */
    @Override
    ...
    protected void createGridForEntity() {
        ...
        grid.addReferenceColumn(
                decision -> decision.getDecisionType() != null ? decision.getDecisionType().getName() : "No Type",
                "Type");
        grid.addShortTextColumn(
                decision -> decision.getDecisionStatus() != null ? decision.getDecisionStatus().getName() : "No Status",
                "Status", null);
        grid.addShortTextColumn(decision -> decision.getAccountableUser() != null
                ? decision.getAccountableUser().getName()
                : "Unassigned", "Accountable", null);
        grid.addColumn(
                decision -> decision.getEstimatedCost() != null ? decision.getEstimatedCost().toString() : "No Cost",
                "Est. Cost", null);
        ...
        grid.addColumn(
                decision -> decision.getCreatedDate() != null ? decision.getCreatedDate().toLocalDate().toString() : "",
                "Created", null);
     ...
     * 
     * @param decision
     *            the decision to show approval status for
    ...
    /**
     * Override the refreshProjectAwareGrid method to use the eager-loading method for decisions to prevent
    ...
    protected void refreshProjectAwareGrid() {
        ...
        items = getDecisionService().findAllByProject(activeProject.get());
