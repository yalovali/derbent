...
import tech.derbent.abstracts.annotations.AMeasureTime;
...
public class CDecisionsView extends CProjectAwareMDPage<CDecision>
	...
	protected void createDetailsLayout() throws Exception {
		// CAccordionDBEntity<CDecision> panel; Basic Information Panel (opened by
		// default) panel = new CPanelDecisionDescription(getCurrentEntity(), getBinder(),
		// (CDecisionService) entityService); addAccordionPanel(panel); Status & Workflow
		// Panel panel = new CPanelDecisionStatusManagement(getCurrentEntity(),
		// getBinder(), (CDecisionService) entityService); addAccordionPanel(panel); Cost
		// & Financial Impact Panel panel = new
		// CPanelDecisionCostManagement(getCurrentEntity(), getBinder(),
		// (CDecisionService) entityService); addAccordionPanel(panel); Team &
		// Accountability Panel panel = new
		// CPanelDecisionTeamManagement(getCurrentEntity(), getBinder(),
		// (CDecisionService) entityService); addAccordionPanel(panel);
		final CScreen screen = screenService.findByNameAndProject(
			sessionService.getActiveProject().orElse(null),
			CDecisionViewService.BASE_VIEW_NAME);
	...
	/**
	 * Override the refreshProjectAwareGrid method to use the eager-loading method for
	 * decisions to prevent LazyInitializationException.
	 */
	@Override
	protected void refreshProjectAwareGrid() {
		final Optional<CProject> activeProject = sessionService.getActiveProject();
		Assert.isTrue(activeProject.isPresent(),
