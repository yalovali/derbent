 ...
 * Layer: View (MVC)
 * 
 * Provides a complete decision management interface following the established
 * patterns from CActivitiesView. Includes grid listing, detail editing,
 * and comprehensive panel organization.
 */
...
public class CDecisionsView extends CProjectAwareMDPage<CDecision> {
    ...
    /**
     * Creates the entity details section using decision panels.
     * Follows the same pattern as CActivitiesView for consistency.
     */
    @Override
    ...
    protected void createGridForEntity() {
        ...
        grid.addShortTextColumn(decision -> decision.getDecisionStatus() != null ? 
                               decision.getDecisionStatus().getName() : "No Status", 
                               "Status", null);
        ...
        grid.addColumn(decision -> decision.getEstimatedCost() != null ? 
                      decision.getEstimatedCost().toString() : "No Cost", 
                      "Est. Cost", null);
        grid.addColumn(decision -> decision.getApprovalCount() > 0 ? 
                      decision.getApprovedCount() + "/" + decision.getApprovalCount() : "No Approvals", 
                      "Approvals", null);
        grid.addColumn(decision -> decision.getCreatedDate() != null ? 
    ...
    @Override
    protected List<CDecision> getProjectFilteredData(final CProject project,
                                                    final org.springframework.data.domain.Pageable pageable) {
        if (project == null) {
            LOGGER.warn("getProjectFilteredData called with null project");
            return List.of();
        }
        
        LOGGER.info("Loading decisions for project: {}", project.getName());
        return ((CDecisionService) entityService).findByProject(project, pageable).getContent();
    }
