 ...
 * Layer: Data Access (MVC) Provides data access methods for decision approval entities.
 */
public interface CDecisionApprovalRepository
	extends CAbstractNamedRepository<CDecisionApproval> {
	 ...
	 */
	List<CDecisionApproval>
		findByDecisionAndIsRequiredTrueAndIsApprovedIsNull(CDecision decision);
	/**
	 ...
	 */
	@Query (
		"SELECT a FROM CDecisionApproval a LEFT JOIN FETCH a.approver WHERE a.decision = :decision"
	)
	List<CDecisionApproval>
		findByDecisionWithApprover(@Param ("decision") CDecision decision);
	/**
	 ...
	 */
	@Query (
		"SELECT a FROM CDecisionApproval a WHERE a.approver = :approver AND a.dueDate < CURRENT_TIMESTAMP AND a.isApproved IS NULL"
	)
	List<CDecisionApproval>
		findOverdueApprovalsByApprover(@Param ("approver") CUser approver);
}