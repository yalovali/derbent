...
public class CDecisionService extends CEntityOfProjectService<CDecision> {
    ...
    /**
     * Gets a decision by ID with all relationships eagerly loaded to prevent LazyInitializationException. Use this
     * method when loading a single decision for detailed view where lazy-loaded associations will be accessed.
     * 
     * @param id
     *            the decision ID
    ...
    public Optional<CDecision> getById(final Long id) {
        ...
        final Optional<CDecision> decision = decisionRepository.findByIdWithAllRelationships(id);
    ...
    protected void initializeLazyFields(final CDecision entity) {
        ...
        LOGGER.debug("Initializing lazy fields for Decision with ID: {} entity: {}", entity.getId(), entity.getName());
