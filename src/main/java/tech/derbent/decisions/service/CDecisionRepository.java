...
/**
 * CDecisionRepository - Repository interface for CDecision entities. Layer: Data Access (MVC) Provides data access
 * methods for decision entities including project-based queries and eagerly loaded relationships to prevent
...
public interface CDecisionRepository extends CEntityOfProjectRepository<CDecision> {
    ...
    /**
     * Finds a decision by ID with all main relationships eagerly loaded to prevent LazyInitializationException. This
     * extends the base method with decision-specific relationships.
     * 
     * @param id
     *            the decision ID
     * @return optional CDecision with loaded relationships
     */
    @Query("SELECT d FROM CDecision d " + "LEFT JOIN FETCH d.project " + "LEFT JOIN FETCH d.assignedTo "
            + "LEFT JOIN FETCH d.createdBy " + "LEFT JOIN FETCH d.decisionType " + "LEFT JOIN FETCH d.decisionStatus "
            + "LEFT JOIN FETCH d.accountableUser " + "WHERE d.id = :id")
    Optional<CDecision> findByIdWithAllRelationships(@Param("id") Long id);

    /**
     * Finds a decision by ID with eagerly loaded decision type to prevent LazyInitializationException.
     * 
     * @param id
     *            the decision ID
    ...
    /**
     * Finds a decision by ID with eagerly loaded decision type and status to prevent LazyInitializationException.
     * 
     * @param id
     *            the decision ID
     * @return optional CDecision with loaded decisionType and status
     */
    @Query("SELECT d FROM CDecision d LEFT JOIN FETCH d.decisionType LEFT JOIN FETCH d.decisionStatus WHERE d.id = :id")
    Optional<CDecision> findByIdWithDecisionTypeAndStatus(@Param("id") Long id);

    /**
     * Finds all decisions by project with eagerly loaded relationships to prevent LazyInitializationException.
     * 
     * @param project
     *            the project
     ...
     */
    @Query("SELECT DISTINCT d FROM CDecision d JOIN d.approvals a WHERE a.approver = :user AND a.isApproved IS NULL")
