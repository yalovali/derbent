 ...
 * validation, creation, and workflow management. Since CDecisionStatus extends CStatus
...
public class CDecisionStatusService extends CEntityOfProjectService<CDecisionStatus> {
	 ...
	 * This replaces the problematic findAllActiveOrdered() method that didn't require
	 * project.
	...
	public List<CDecisionStatus> findAllActiveOrderedByProject(final CProject project) {
		LOGGER.info(
			"findAllActiveOrderedByProject called for decision statuses in project: {}",
		...
		return ((CDecisionStatusRepository) repository)
			.findByProjectAndIsFinalFalseOrderBySortOrderAsc(project);
	}

	/**
	 * Finds all final decision statuses for a specific project. This replaces the
	 * problematic findAllFinal() method that didn't require project.
	 * @param project the project to find statuses for
	...
	public List<CDecisionStatus> findAllFinalByProject(final CProject project) {
		...
		return ((CDecisionStatusRepository) repository)
			.findByProjectAndIsFinalTrue(project);
	}

	/**
	 * Finds all decision statuses for a specific project ordered by sort order. This
	 * replaces the problematic findAllOrdered() method that didn't require project.
	 * @param project the project to find statuses for
	...
	public List<CDecisionStatus> findAllOrderedByProject(final CProject project) {
		...
		return ((CDecisionStatusRepository) repository)
			.findAllByProjectOrderBySortOrderAsc(project);
	}

	/**
	 * Finds decision statuses that require approval for a specific project. This replaces
	 * the problematic findRequiringApproval() method that didn't require project.
	 * @param project the project to find statuses for
	...
	public List<CDecisionStatus> findRequiringApprovalByProject(final CProject project) {
		LOGGER.info(
			"findRequiringApprovalByProject called for decision statuses in project: {}",
			project != null ? project.getName() : "null");
		...
		return ((CDecisionStatusRepository) repository)
			.findByProjectAndRequiresApprovalTrue(project);
	}
