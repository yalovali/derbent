...
/**
 * CDecisionStatusService - Service class for CDecisionStatus entities.
 * Layer: Service (MVC)
 * 
 * Provides business logic operations for decision status management including
...
public class CDecisionStatusService extends CAbstractNamedEntityService<CDecisionStatus> {
     ...
     */
    @Transactional
    public CDecisionStatus createDecisionStatus(final String name, final String description, 
                                              final boolean isFinal) {
        LOGGER.info("createDecisionStatus called with name: {}, description: {}, isFinal: {}", 
                   name, description, isFinal);
        
        if (name == null || name.trim().isEmpty()) {
            LOGGER.error("createDecisionStatus called with null or empty name");
            throw new IllegalArgumentException("Decision status name cannot be null or empty");
        }
        
        final CDecisionStatus decisionStatus = new CDecisionStatus(name.trim(), description);
    ...
    public CDecisionStatus createDecisionStatus(final String name, final String description, 
                                              ...
                                              final boolean allowsEditing, 
                                              final boolean requiresApproval) {
        LOGGER.info("createDecisionStatus called with name: {}, description: {}, color: {}, isFinal: {}, allowsEditing: {}, requiresApproval: {}", 
        ...
        final CDecisionStatus decisionStatus = new CDecisionStatus(name.trim(), description, color, 
                                                                 isFinal, allowsEditing, requiresApproval);
    ...
    public CDecisionStatus updateStatusProperties(final CDecisionStatus decisionStatus, 
                                                final boolean allowsEditing, 
                                                final boolean requiresApproval) {
        LOGGER.info("updateStatusProperties called with decisionStatus: {}, allowsEditing: {}, requiresApproval: {}", 
                   decisionStatus != null ? decisionStatus.getName() : "null", allowsEditing, requiresApproval);
        ...
        decisionStatus.setAllowsEditing(allowsEditing);
     ...
     */
    @Transactional(readOnly = true)
    public boolean canDecisionsBeModified(final CDecisionStatus decisionStatus) {
        LOGGER.info("canDecisionsBeModified called with decisionStatus: {}", 
                   decisionStatus != null ? decisionStatus.getName() : "null");
        
        if (decisionStatus == null) {
            LOGGER.warn("canDecisionsBeModified called with null decisionStatus");
            return false;
        }
        
        return decisionStatus.canBeModified();
    }
