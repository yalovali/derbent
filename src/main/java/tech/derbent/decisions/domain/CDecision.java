...
public class CDecision extends CEntityOfProject<CDecision> {
    ...
    @MetaData(displayName = "Decision Type", required = false, readOnly = false, description = "Category or type of the decision", hidden = false, order = 2, dataProviderBean = "CDecisionTypeService")
    private CDecisionType decisionType;

    // Cost Estimation
    @Column(name = "estimated_cost", nullable = true, precision = 19, scale = 2)
    @DecimalMin(value = "0.0", inclusive = true)
    @MetaData(displayName = "Estimated Cost", required = false, readOnly = false, description = "Estimated cost impact of the decision", hidden = false, order = 3, min = 0.0)
    ...
    @MetaData(displayName = "Accountable Personnel", required = false, readOnly = false, description = "User accountable for this decision", hidden = false, order = 5, dataProviderBean = "CUserService")
    private CUser accountableUser;

    // Team Members
    @ManyToMany(fetch = FetchType.LAZY)
    @JoinTable(name = "cdecision_team_members", joinColumns = @JoinColumn(name = "decision_id"), inverseJoinColumns = @JoinColumn(name = "user_id"))
    @MetaData(displayName = "Team Members", required = false, readOnly = false, description = "Team members involved in this decision", hidden = false, order = 6, dataProviderBean = "CUserService")
    ...
    @MetaData(displayName = "Implementation Date", required = false, readOnly = false, description = "Date when the decision was or will be implemented", hidden = false, order = 7)
    private LocalDateTime implementationDate;

    // Decision Review Date
    @Column(name = "review_date", nullable = true)
    @MetaData(displayName = "Review Date", required = false, readOnly = false, description = "Date when the decision will be reviewed", hidden = false, order = 8)
     ...
     * 
     * @param name
     *            the decision name - must not be null or empty
     * @param project
     *            the project this decision belongs to - must not be null
     */
     ...
     *            the project this decision belongs to - must not be null
     * @param description
     *            detailed description of the decision - can be null
     */
    public CDecision(final String name, final CProject project, final String description) {
        super(CDecision.class, name, project);
        setDescription(description);
        LOGGER.debug("CDecision constructor called with name: {}, project: {}, description: {}", name,
                project != null ? project.getName() : "null", description);
    }
    // Getters and Setters
     ...
     * 
     * @param approval
     *            the approval to add - must not be null
     */
     ...
     * 
     * @param user
     *            the user to add to the team - must not be null
     */
    public void addTeamMember(final CUser user) {
        LOGGER.debug("addTeamMember called with user: {}", user != null ? user.getName() : "null");
    ...
    public CUser getAccountableUser() {
        return accountableUser;
    }
    ...
    public CDecisionStatus getDecisionStatus() {
        return decisionStatus;
    }

    public CDecisionType getDecisionType() {
        return decisionType;
    }
     ...
     * 
     * @param user
     *            the user to remove from the team - must not be null
     */
    public void removeTeamMember(final CUser user) {
        LOGGER.debug("removeTeamMember called with user: {}", user != null ? user.getName() : "null");
