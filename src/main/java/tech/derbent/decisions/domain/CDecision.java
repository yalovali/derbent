...
/**
 * CDecision - Domain entity representing project decisions with comprehensive
 * management features. Layer: Domain (MVC)
 * 
 * Supports:
 ...
 * - Accountable personnel management
 * - Descriptive documentation
 * 
 * Follows the established patterns from CActivity for consistency.
 */
@Entity
...
public class CDecision extends CEntityOfProject {
    ...
    @JoinColumn(name = "decision_type_id", nullable = true)
    @MetaData(
    ...
    @MetaData(
        ...
        description = "Date when the decision will be reviewed", hidden = false,
        order = 8
    )
    ...
    public CDecision(final String name, final CProject project) {
        super(name, project);
        LOGGER.debug("CDecision constructor called with name: {} and project: {}", 
    ...
    public CDecision(final String name, final CProject project, final String description) {
        ...
        LOGGER.debug("CDecision constructor called with name: {}, project: {}, description: {}", 
