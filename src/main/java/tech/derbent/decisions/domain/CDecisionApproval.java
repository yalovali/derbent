 ...
 * Layer: Domain (MVC) Supports: - Tracking approval status for each approver - Recording
 * approval/rejection dates - Storing approval comments and feedback - Managing approval
 * workflow state Each decision can have multiple approvals from different users or roles.
 */
@Entity
...
public class CDecisionApproval extends CEntityNamed<CDecisionApproval> {
	...
	@MetaData (
		...
		description = "Comments or feedback from the approver", hidden = false, order = 5,
		maxLength = 2000
	)
	...
	public void approve(final String comments) {
		LOGGER.debug("approve called for approval: {} with comments: {}", getName(),
			comments);
	...
	public CDecision getDecision() { return decision; }
	 ...
	 */
	public boolean isApproved() { return Boolean.TRUE.equals(isApproved); }
	 ...
	 */
	public boolean isPending() { return isApproved == null; }
	...
	public void reject(final String comments) {
		LOGGER.debug("reject called for approval: {} with comments: {}", getName(),
			comments);
