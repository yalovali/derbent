 ...
 * field-level error reporting - Integration with existing form builder - Minimal impact on existing code */
public class CEnhancedBinderDemo extends VerticalLayout {
	...
	private void createDemo() {
		...
		// Log output area
		logOutput = new Pre();
		logOutput.getStyle().set("background-color", "#f5f5f5").set("padding", "10px").set("border", "1px solid #ddd").set("max-height", "300px")
				.set("overflow-y", "auto");
	...
	private void logToOutput(final String message) {
		...
		getUI().ifPresent(ui -> ui.getPage().executeJs("arguments[0].scrollTop = arguments[0].scrollHeight", logOutput.getElement()));
	}
}
