...
@Menu (
	order = 998, icon = "class:tech.derbent.examples.view.CExampleSettingsView",
	title = "Examples.Settings.Advanced"
)
@PermitAll
public class CExampleSettingsView extends Div implements CInterfaceIconSet {
	...
	public CExampleSettingsView() {
		...
		final Paragraph description =
			new Paragraph("This example demonstrates responsive flex layout behavior. "
		...
		for (int i = 1; i <= 6; i++) {
			final Card card =
				createCard("Component " + i, "This is the description for component " + i,
					VaadinIcon.values()[i * 10]); // Just picking some icons
