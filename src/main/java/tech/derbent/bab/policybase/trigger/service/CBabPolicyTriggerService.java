...
/**
 * CBabPolicyTriggerService - Service for managing BAB policy triggers.
 * 
 * Provides business logic for trigger entities including:
 * - CRUD operations with validation
 * - Trigger type management
 * - Node compatibility checking
 * - Execution scheduling support
...
public class CBabPolicyTriggerService extends CEntityOfProjectService<CBabPolicyTrigger>
        implements IEntityRegistrable, IEntityWithView {

    private static final Logger LOGGER = LoggerFactory.getLogger(CBabPolicyTriggerService.class);

    public CBabPolicyTriggerService(
            final IBabPolicyTriggerRepository repository,
            final Clock clock,
    ...
    @Override
    public Class<CBabPolicyTrigger> getEntityClass() {
        return CBabPolicyTrigger.class;
    }
    ...
    public Class<?> getInitializerServiceClass() {
        return CBabPolicyTriggerInitializerService.class;
    }
    ...
    protected void validateEntity(final CBabPolicyTrigger entity) {
        ...
        validateUniqueNameInProject(
            (IBabPolicyTriggerRepository) repository,
            entity,
            entity.getName(),
            entity.getProject());
    ...
    private void validateNodeTypeConfiguration(final CBabPolicyTrigger entity) {
        ...
        if (!anyNodeTypeEnabled) {
            throw new CValidationException(
                "At least one node type must be enabled for the trigger");
        }
    }
    ...
    public String checkDeleteAllowed(final CBabPolicyTrigger entity) {
        ...
        // For example, check if trigger is being used by any policy rules
    ...
    public void copyEntityFieldsTo(final CBabPolicyTrigger source, final CEntityDB<?> target, final CCloneOptions options) {
        ...
        if (!(target instanceof CBabPolicyTrigger)) {
            return;
        }
        final CBabPolicyTrigger targetTrigger = (CBabPolicyTrigger) target;
