...
/**
 * CBabPolicyFilterService - Service for managing BAB policy filters.
 * 
 * Provides business logic for filter entities including:
 * - CRUD operations with validation
 * - Filter type management
 * - Node compatibility checking
 * - Condition and transformation validation
...
public class CBabPolicyFilterService extends CEntityOfProjectService<CBabPolicyFilter>
        implements IEntityRegistrable, IEntityWithView {

    private static final Logger LOGGER = LoggerFactory.getLogger(CBabPolicyFilterService.class);

    public CBabPolicyFilterService(
            final IBabPolicyFilterRepository repository,
            final Clock clock,
    ...
    @Override
    public Class<CBabPolicyFilter> getEntityClass() {
        return CBabPolicyFilter.class;
    }
    ...
    public Class<?> getInitializerServiceClass() {
        return CBabPolicyFilterInitializerService.class;
    }
    ...
    protected void validateEntity(final CBabPolicyFilter entity) {
        ...
        validateUniqueNameInProject(
            (IBabPolicyFilterRepository) repository,
            entity,
            entity.getName(),
            entity.getProject());
    ...
    private void validateFilterTypeSpecificFields(final CBabPolicyFilter entity) {
        if (CBabPolicyFilter.FILTER_TYPE_CSV.equals(entity.getFilterType())) {
            // CSV filters should have delimiter configuration
            if (entity.getConfigurationJson() != null && 
                !entity.getConfigurationJson().contains("delimiter")) {
                LOGGER.warn("CSV filter '{}' missing delimiter configuration", entity.getName());
            }
        }

        if (CBabPolicyFilter.FILTER_TYPE_REGEX.equals(entity.getFilterType())) {
            // Regex filters should have patterns
            if (entity.getConditionsJson() == null || 
                entity.getConditionsJson().trim().isEmpty() ||
                !entity.getConditionsJson().contains("patterns")) {
                throw new CValidationException("Regex filters require pattern configuration");
            }
        }

        if (CBabPolicyFilter.FILTER_TYPE_TRANSFORM.equals(entity.getFilterType())) {
            // Transform filters should have transformation template
            if (entity.getTransformationJson() == null || entity.getTransformationJson().trim().isEmpty()) {
                throw new CValidationException("Transform filters require transformation template");
            }
        }
    }
    ...
    private void validateNodeTypeConfiguration(final CBabPolicyFilter entity) {
        ...
        if (!anyNodeTypeEnabled) {
            throw new CValidationException(
                "At least one node type must be enabled for the filter");
        }
    }
    ...
    private void validateNullHandling(final CBabPolicyFilter entity) {
        if (entity.getNullHandling() != null) {
            ...
            if (!strategy.equals("ignore") && !strategy.equals("reject") && 
                !strategy.equals("pass") && !strategy.equals("default")) {
                throw new CValidationException(
