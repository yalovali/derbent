 ...
 * Following Derbent pattern: Abstract service with common validation logic.
 * 
 * Provides shared validation and business logic for all node types:
 * - Name validation and uniqueness checking
 * - Physical interface validation
 * - Common node field validation
 * 
 * Concrete node services extend this and add type-specific validation.
 */
@Profile("bab")
@PreAuthorize("isAuthenticated()")
public abstract class CBabNodeService<NodeType extends CBabNodeEntity<NodeType>> 
	...
	protected CBabNodeService(
			final INodeEntityRepository<NodeType> repository,
			final Clock clock,
	...
	/**
	 * Validate common node fields (name, physical interface).
	 * Subclasses MUST call super.validateEntity() first, then add type-specific validation.
	 */
	@Override
