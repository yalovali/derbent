...
/**
 * CBabROSNodeInitializerService - Initializer for ROS nodes.
 * 
 * Layer: Service (MVC)
 * Active when: 'bab' profile is active
 * Following Derbent pattern: Entity initializer with UI definition.
 * 
 * Creates dynamic pages and grids for ROS node management.
 * Defines form layout with node configuration and ROS-specific fields.
 */
@Service
@Profile("bab")
public final class CBabROSNodeInitializerService extends CInitializerServiceBase {
	...
	/**
	 * Initialize ROS node pages for project.
	...
	public static void initialize(final CProject<?> project, final CGridEntityService gridEntityService,
		...
		initBase(clazz, project, gridEntityService, detailSectionService, pageEntityService, detailSection, grid,
			"Network.ROS Nodes",              // Menu title (hierarchical)
	...
	public static CGridEntity createGridEntity(final CProject<?> project) {
		...
		grid.setColumnFields(List.of("id", "name", "physicalInterface", "isActive", 
			"connectionStatus", "rosMasterPort", "rosVersion", "nodeNamespace", "createdBy", "createdDate"));
		return grid;
	}
	
	/**
	 * Initialize sample ROS nodes for project.
	 * Creates sample nodes for robotics communication.
	 */
	public static void initializeSample(final CProject<?> project, final boolean minimal) throws Exception {
		LOGGER.info("Initializing ROS Node sample data for project: {}", project.getName());
		
		final CBabROSNodeService service = 
			(CBabROSNodeService) CSpringContext.getBean(CEntityRegistry.getServiceClassForEntity(clazz));
