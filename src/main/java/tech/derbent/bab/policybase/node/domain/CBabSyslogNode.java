...
/**
 * CBabSyslogNode - Syslog server virtual network node entity.
 * 
 * Layer: Domain (MVC)
 ...
 * - Stores Syslog-specific fields in cnode_syslog table
 * - node_type discriminator = "SYSLOG"
 * 
 * Represents Syslog receiver nodes for system logging and monitoring.
 * Example: UDP/TCP syslog servers for centralized log collection.
 * Used in BAB Actions Dashboard policy rule engine for log traffic management.
 */
@Entity
...
public class CBabSyslogNode extends CBabNodeEntity<CBabSyslogNode> {
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "File attachments for this Syslog node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Comments for this Syslog node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Related links for this Syslog node",
		hidden = false,
	...
	@AMetaData(
		...
		required = true,
		readOnly = false,
		description = "Syslog server listening port (default: 514)",
		hidden = false
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Default syslog facility (LOCAL0-LOCAL7, USER, KERN)",
		hidden = false,
		maxLength = 20
	)
	...
	@AMetaData(
		displayName = "Min Severity Level",
		required = false,
		readOnly = false,
		description = "Minimum severity to log (DEBUG, INFO, NOTICE, WARNING, ERROR, CRIT, ALERT, EMERG)",
		hidden = false,
		maxLength = 20
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Path to store syslog messages",
		hidden = false,
		maxLength = 255
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Enable TLS encryption for TCP connections",
		hidden = false
	)
