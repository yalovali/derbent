...
/** CBabModbusNodeService - Service for Modbus RTU/ASCII virtual network nodes. Layer: Service (MVC) Active when: 'bab' profile is active Following
 * Derbent pattern: Entity service extending common node base service. Provides Modbus-specific business logic: - Slave ID validation - Serial port
 * configuration validation - Modbus mode validation (RTU/ASCII) */
@Service
@Profile ({
		"bab", "default", "test"
})
@PreAuthorize ("isAuthenticated()")
public class CBabModbusNodeService extends CBabNodeService<CBabModbusNode> implements IEntityRegistrable, IEntityWithView {

	private static final Logger LOGGER = LoggerFactory.getLogger(CBabModbusNodeService.class);

	public CBabModbusNodeService(final IModbusNodeRepository repository, final Clock clock, final ISessionService sessionService) {
	...
	@Override
	public void copyEntityFieldsTo(final CBabModbusNode source, final CEntityDB<?> target, final CCloneOptions options) {
	...
	@Override
	public Class<CBabModbusNode> getEntityClass() { return CBabModbusNode.class; }

	@Override
	public Class<?> getInitializerServiceClass() { return CBabModbusNodeInitializerService.class; }
	// IEntityRegistrable implementation

	@Override
	public Class<?> getPageServiceClass() { return CPageServiceModbusNode.class; }

	@Override
	public Class<?> getServiceClass() { return CBabModbusNodeService.class; }
	...
	protected void validateEntity(final CBabModbusNode entity) {
		...
		if (entity.getSlaveId() == null) {
			throw new CValidationException("Slave ID is required");
		}
		validateNumericField(entity.getSlaveId(), "Slave ID", 247);
		if (entity.getSlaveId() < 1) {
			throw new CValidationException("Slave ID must be between 1 and 247");
		}
		// Baudrate validation
		if (entity.getBaudrate() == null) {
			throw new CValidationException("Baudrate is required");
		}
		if (entity.getBaudrate() < 300 || entity.getBaudrate() > 115200) {
			throw new CValidationException("Baudrate must be between 300 and 115200");
		}
		// Data bits validation
		if (entity.getDataBits() != null && entity.getDataBits() != 7 && entity.getDataBits() != 8) {
			throw new CValidationException("Data bits must be 7 or 8");
		}
		// Stop bits validation
		if (entity.getStopBits() != null && entity.getStopBits() != 1 && entity.getStopBits() != 2) {
			throw new CValidationException("Stop bits must be 1 or 2");
		}
		...
		if (!"NONE".equals(entity.getParity()) && !"EVEN".equals(entity.getParity()) && !"ODD".equals(entity.getParity())) {
			throw new CValidationException("Parity must be NONE, EVEN, or ODD");
		}
		...
		if (!"RTU".equals(entity.getModbusMode()) && !"ASCII".equals(entity.getModbusMode())) {
			throw new CValidationException("Modbus Mode must be RTU or ASCII");
		}
		// Unique slave ID per interface per project check
		final IModbusNodeRepository repo = (IModbusNodeRepository) repository;
		final var existingSlaveId = repo.findBySlaveIdAndInterfaceAndProject(entity.getSlaveId(), entity.getPhysicalInterface(), entity.getProject());
		if (existingSlaveId.isPresent() && !existingSlaveId.get().getId().equals(entity.getId())) {
			throw new CValidationException("Slave ID %d is already used by another Modbus node on interface %s in this project"
