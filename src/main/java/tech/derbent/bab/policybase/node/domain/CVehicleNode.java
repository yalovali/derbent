...
/** CVehicleNode - Vehicle virtual network node entity for CAN bus communication. Layer: Domain (MVC) Active when: 'bab' profile is active Following
 * Derbent pattern: Concrete entity with @Entity annotation. JPA Inheritance: JOINED strategy with @DiscriminatorValue - Inherits common fields from
 * cbab_node table - Stores vehicle-specific fields in cnode_vehicle table - node_type discriminator = "VEHICLE" Represents vehicle virtual nodes
...
public class CVehicleNode extends CBabNodeEntity<CVehicleNode> {
	...
	@AMetaData (
			displayName = "Attachments", required = false, readOnly = false, description = "File attachments for this vehicle node", hidden = false,
			dataProviderBean = "CAttachmentService", createComponentMethod = "createComponent"
	)
	private Set<CAttachment> attachments = new HashSet<>();
	@Column (name = "baud_rate", nullable = false)
	@AMetaData (
			displayName = "Baud Rate", required = false, readOnly = false, description = "CAN bus communication baud rate (e.g., 500000, 250000)",
			hidden = false
	)
	private Integer baudRate = 500000;
	@Column (name = "can_address", nullable = false)
	@AMetaData (
			displayName = "CAN Address", required = true, readOnly = false, description = "CAN bus address for this vehicle node (hexadecimal)",
			hidden = false
	)
	private Integer canAddress;
	@Column (name = "can_protocol", length = 20, nullable = false)
	@AMetaData (
			displayName = "CAN Protocol", required = false, readOnly = false, description = "CAN protocol version (CAN 2.0A, CAN 2.0B, CAN-FD)",
	...
	@JoinColumn (name = "vehicle_node_id")
	@AMetaData (
			displayName = "Comments", required = false, readOnly = false, description = "Comments for this vehicle node", hidden = false,
	...
	@JoinColumn (name = "vehicle_node_id")
	@AMetaData (
			displayName = "Links", required = false, readOnly = false, description = "Related links for this vehicle node", hidden = false,
			dataProviderBean = "CLinkService", createComponentMethod = "createComponent"
	)
	private Set<CLink> links = new HashSet<>();
	@Column (name = "manufacturer", length = 50)
	@AMetaData (
			displayName = "Manufacturer", required = false, readOnly = false, description = "Vehicle manufacturer (Ford, Toyota, etc.)",
			hidden = false, maxLength = 50
	)
	private String manufacturer;
	@Column (name = "model_year")
	@AMetaData (displayName = "Model Year", required = false, readOnly = false, description = "Vehicle model year", hidden = false)
	private Integer modelYear;
	// Vehicle specific fields
	@Column (name = "vehicle_id", length = 50, nullable = false)
	@AMetaData (
			displayName = "Vehicle ID", required = true, readOnly = false,
			description = "Unique identifier for the vehicle (e.g., VIN, fleet number)", hidden = false, maxLength = 50
	)
	private String vehicleId;
	@Column (name = "vehicle_type", length = 30, nullable = false)
	@AMetaData (
			displayName = "Vehicle Type", required = false, readOnly = false, description = "Type of vehicle (CAR, TRUCK, MOTORCYCLE, BUS, etc.)",
			hidden = false, maxLength = 30
	)
	...
	public CVehicleNode(final String name, final CProject<?> project) {
		super(CVehicleNode.class, name, project);
		initializeDefaults(); // Business constructors MUST call this (RULE 2)
	}

	public CVehicleNode(final String name, final CProject<?> project, final String vehicleId, final Integer canAddress) {
		super(CVehicleNode.class, name, project);
