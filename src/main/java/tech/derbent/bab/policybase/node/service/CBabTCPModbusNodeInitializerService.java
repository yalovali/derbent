...
/**
 * CBabTCPModbusNodeInitializerService - Initializer for TCP Modbus nodes.
 * 
 * Layer: Service (MVC)
 * Active when: 'bab' profile is active
 * Following Derbent pattern: Entity initializer with UI definition.
 * 
 * Creates dynamic pages and grids for Modbus TCP node management.
 * Defines form layout with node configuration and Modbus TCP-specific fields.
 */
@Service
@Profile("bab")
public final class CBabTCPModbusNodeInitializerService extends CInitializerServiceBase {
	...
	/**
	 * Initialize TCP Modbus node pages for project.
	...
	public static void initialize(final CProject<?> project, final CGridEntityService gridEntityService,
		...
		initBase(clazz, project, gridEntityService, detailSectionService, pageEntityService, detailSection, grid,
			"Network.TCP Modbus Nodes",       // Menu title (hierarchical)
	...
	public static CGridEntity createGridEntity(final CProject<?> project) {
		...
		grid.setColumnFields(List.of("id", "name", "physicalInterface", "isActive", 
			"connectionStatus", "serverAddress", "serverPort", "unitId", "createdBy", "createdDate"));
		return grid;
	}
	
	/**
	 * Initialize sample TCP Modbus nodes for project.
	 * Creates sample nodes for industrial Ethernet communication.
	 */
	public static void initializeSample(final CProject<?> project, final boolean minimal) throws Exception {
		LOGGER.info("Initializing TCP Modbus Node sample data for project: {}", project.getName());
		
		final CBabTCPModbusNodeService service = 
			(CBabTCPModbusNodeService) CSpringContext.getBean(CEntityRegistry.getServiceClassForEntity(clazz));
