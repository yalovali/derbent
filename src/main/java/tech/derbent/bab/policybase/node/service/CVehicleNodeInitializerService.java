...
/**
 * CVehicleNodeInitializerService - Initializer for Vehicle nodes.
 * 
 * Layer: Service (MVC)
 * Active when: 'bab' profile is active
 * Following Derbent pattern: Entity initializer with UI definition.
 * 
 * Creates dynamic pages and grids for Vehicle node management.
 * Defines form layout with node configuration and vehicle-specific fields.
 */
@Service
@Profile("bab")
public final class CVehicleNodeInitializerService extends CInitializerServiceBase {
	...
	/**
	 * Initialize Vehicle node pages for project.
	...
	public static void initialize(final CProject<?> project, final CGridEntityService gridEntityService,
		...
		initBase(clazz, project, gridEntityService, detailSectionService, pageEntityService, detailSection, grid,
			"Network.Vehicle Nodes",          // Menu title (hierarchical)
	...
	public static CGridEntity createGridEntity(final CProject<?> project) {
		...
		grid.setColumnFields(List.of("id", "name", "physicalInterface", "isActive", 
			"connectionStatus", "vehicleId", "vehicleType", "canAddress", "baudRate", "createdBy", "createdDate"));
		return grid;
	}
	
	/**
	 * Initialize sample vehicle nodes for project.
	 * Creates sample nodes for CAN bus vehicle communication.
	 */
	public static void initializeSample(final CProject<?> project, final boolean minimal) throws Exception {
		LOGGER.info("Initializing Vehicle Node sample data for project: {}", project.getName());
		
		final CVehicleNodeService service = 
			(CVehicleNodeService) CSpringContext.getBean(CEntityRegistry.getServiceClassForEntity(clazz));
