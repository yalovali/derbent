...
/** CBabTCPModbusNodeService - Service for Modbus TCP virtual network nodes. Layer: Service (MVC) Active when: 'bab' profile is active Following
 * Derbent pattern: Entity service extending common node base service. Provides Modbus TCP-specific business logic: - Server port validation - Unit ID
 * validation - TCP connection parameters validation */
@Service
@Profile ({
		"bab", "default", "test"
})
@PreAuthorize ("isAuthenticated()")
public class CBabTCPModbusNodeService extends CBabNodeService<CBabTCPModbusNode> implements IEntityRegistrable, IEntityWithView {

	private static final Logger LOGGER = LoggerFactory.getLogger(CBabTCPModbusNodeService.class);

	public CBabTCPModbusNodeService(final ITCPModbusNodeRepository repository, final Clock clock, final ISessionService sessionService) {
	...
	@Override
	public void copyEntityFieldsTo(final CBabTCPModbusNode source, final CEntityDB<?> target, final CCloneOptions options) {
		// STEP 1: ALWAYS call parent first
		super.copyEntityFieldsTo(source, target, options);
		// STEP 2: Type-check target
		if (!(target instanceof final CBabTCPModbusNode targetNode)) {
			return;
		}
	...
	@Override
	public Class<CBabTCPModbusNode> getEntityClass() { return CBabTCPModbusNode.class; }

	@Override
	public Class<?> getInitializerServiceClass() { return CBabTCPModbusNodeInitializerService.class; }
	// IEntityRegistrable implementation

	@Override
	public Class<?> getPageServiceClass() { return CPageServiceTCPModbusNode.class; }

	@Override
	public Class<?> getServiceClass() { return CBabTCPModbusNodeService.class; }
	...
	protected void validateEntity(final CBabTCPModbusNode entity) {
		...
		if (entity.getUnitId() == null) {
			throw new CValidationException("Unit ID is required");
		}
		...
		if (entity.getConnectionTimeoutMs() != null) {
			...
			if (entity.getConnectionTimeoutMs() < 100) {
				throw new CValidationException("Connection Timeout must be at least 100ms");
			}
		}
		if (entity.getResponseTimeoutMs() != null) {
			...
			if (entity.getResponseTimeoutMs() < 100) {
				throw new CValidationException("Response Timeout must be at least 100ms");
			}
		}
		...
		if (entity.getMaxConnections() != null) {
			...
			if (entity.getMaxConnections() < 1) {
				throw new CValidationException("Max Connections must be at least 1");
			}
		}
		...
		final var existingPort = repo.findByPortAndUnitIdAndInterfaceAndProject(entity.getServerPort(), entity.getUnitId(),
				entity.getPhysicalInterface(), entity.getProject());
