...
/**
 * CBabROSNode - ROS (Robot Operating System) virtual network node entity.
 * 
 * Layer: Domain (MVC)
 ...
 * - Stores ROS-specific fields in cnode_ros table
 * - node_type discriminator = "ROS"
 * 
 * Represents ROS communication nodes for robotics applications.
 * Example: ROS master, ROS nodes for topic/service communication.
 * Used in BAB Actions Dashboard policy rule engine for ROS traffic management.
 */
@Entity
...
public class CBabROSNode extends CBabNodeEntity<CBabROSNode> {
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "File attachments for this ROS node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Comments for this ROS node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Related links for this ROS node",
		hidden = false,
	...
	@AMetaData(
		...
		required = true,
		readOnly = false,
		description = "ROS master URI (e.g., http://localhost:11311)",
		hidden = false,
		maxLength = 255
	)
	...
	@AMetaData(
		displayName = "ROS Master Port",
		required = true,
		readOnly = false,
		description = "ROS master port (default: 11311)",
		hidden = false
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "ROS node namespace (e.g., /robot1)",
		hidden = false,
		maxLength = 100
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Comma-separated list of ROS topics",
		hidden = false,
		maxLength = 500
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Comma-separated list of ROS services",
		hidden = false,
		maxLength = 500
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Message queue size for topics",
		hidden = false
	)
	...
	public void setRosMasterPort(Integer rosMasterPort) {
	 ...
	 */
	public boolean isRos2() {
		return "ROS2".equalsIgnoreCase(rosVersion);
	}
}
