...
/** CBabSyslogNodeService - Service for Syslog server virtual network nodes. Layer: Service (MVC) Active when: 'bab' profile is active Following
 * Derbent pattern: Entity service extending common node base service. Provides Syslog-specific business logic: - Port validation - Protocol
 * validation (UDP/TCP) - Facility and severity validation */
@Service
@Profile ({
		"bab", "default", "test"
})
@PreAuthorize ("isAuthenticated()")
public class CBabSyslogNodeService extends CBabNodeService<CBabSyslogNode> implements IEntityRegistrable, IEntityWithView {

	private static final Logger LOGGER = LoggerFactory.getLogger(CBabSyslogNodeService.class);

	public CBabSyslogNodeService(final ISyslogNodeRepository repository, final Clock clock, final ISessionService sessionService) {
	...
	@Override
	public void copyEntityFieldsTo(final CBabSyslogNode source, final CEntityDB<?> target, final CCloneOptions options) {
		// STEP 1: ALWAYS call parent first
		super.copyEntityFieldsTo(source, target, options);
		// STEP 2: Type-check target
		if (!(target instanceof CBabSyslogNode targetNode)) {
			return;
		}
	...
	@Override
	public Class<CBabSyslogNode> getEntityClass() { return CBabSyslogNode.class; }

	@Override
	public Class<?> getInitializerServiceClass() { return CBabSyslogNodeInitializerService.class; }
	// IEntityRegistrable implementation

	@Override
	public Class<?> getPageServiceClass() { return CPageServiceSyslogNode.class; }

	@Override
	public Class<?> getServiceClass() { return CBabSyslogNodeService.class; }
	...
	protected void validateEntity(final CBabSyslogNode entity) {
		...
		if (entity.getListenPort() == null) {
			throw new CValidationException("Listen Port is required");
		}
		validateNumericField(entity.getListenPort(), "Listen Port", 65535);
		if (entity.getListenPort() < 1) {
			throw new CValidationException("Listen Port must be between 1 and 65535");
		}
		...
		if (!"UDP".equals(entity.getProtocol()) && !"TCP".equals(entity.getProtocol())) {
			throw new CValidationException("Protocol must be UDP or TCP");
		}
		// Facility validation
		...
		if (!entity.getLogFilePath().startsWith("/")) {
			throw new CValidationException("Log File Path must be absolute (start with /)");
		}
		...
		if (entity.getMaxMessageSize() != null) {
			...
			if (entity.getMaxMessageSize() < 480) {
				throw new CValidationException("Max Message Size must be at least 480 bytes");
			}
		}
		...
		final var existingPort =
				repo.findByListenPortAndInterfaceAndProject(entity.getListenPort(), entity.getPhysicalInterface(), entity.getProject());
		if (existingPort.isPresent() && !existingPort.get().getId().equals(entity.getId())) {
			throw new IllegalArgumentException("Listen port %d is already used by another Syslog node on interface %s in this project"
