...
public interface INodeEntityRepository<NodeType extends CBabNodeEntity<NodeType>> extends IEntityOfProjectRepository<NodeType> {
	...
	/** Count active nodes by project. Abstract method - concrete repositories implement with specific queries. */
	long countActiveByProject(CProject<?> project);
	/** Count nodes by connection status. Abstract method - concrete repositories implement for dashboard statistics. */
	...
	/** Check if interface is already used by another node. Concrete repositories implement for interface validation. */
	boolean existsByPhysicalInterfaceAndProject(String physicalInterface, CProject<?> project);
	/** Find active nodes by project. Concrete repositories implement with eager loading. */
	List<NodeType> findActiveByProject(CProject<?> project);
	/** Find nodes by connection status. Concrete repositories implement with specific status filtering. */
	List<NodeType> findByConnectionStatus(String connectionStatus);
	/** Find nodes by type and project. Concrete repositories implement with node type filtering. */
	List<NodeType> findByNodeTypeAndProject(String nodeType, CProject<?> project);
	/** Find nodes by physical interface. Concrete repositories implement with specific entity type. */
	List<NodeType> findByPhysicalInterface(String physicalInterface);
	/** Find node by interface and project (for uniqueness validation). Concrete repositories implement for constraint checking. */
