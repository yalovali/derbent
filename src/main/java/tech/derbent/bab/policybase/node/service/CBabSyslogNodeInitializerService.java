...
/**
 * CBabSyslogNodeInitializerService - Initializer for Syslog nodes.
 * 
 * Layer: Service (MVC)
 * Active when: 'bab' profile is active
 * Following Derbent pattern: Entity initializer with UI definition.
 * 
 * Creates dynamic pages and grids for Syslog node management.
 * Defines form layout with node configuration and Syslog-specific fields.
 */
@Service
@Profile("bab")
public final class CBabSyslogNodeInitializerService extends CInitializerServiceBase {
	...
	/**
	 * Initialize Syslog node pages for project.
	...
	public static void initialize(final CProject<?> project, final CGridEntityService gridEntityService,
		...
		initBase(clazz, project, gridEntityService, detailSectionService, pageEntityService, detailSection, grid,
			"Network.Syslog Nodes",           // Menu title (hierarchical)
	...
	public static CGridEntity createGridEntity(final CProject<?> project) {
		...
		grid.setColumnFields(List.of("id", "name", "physicalInterface", "isActive", 
			"connectionStatus", "listenPort", "protocol", "facility", "enableTls", "createdBy", "createdDate"));
		return grid;
	}
	
	/**
	 * Initialize sample Syslog nodes for project.
	 * Creates sample nodes for log collection and monitoring.
	 */
	public static void initializeSample(final CProject<?> project, final boolean minimal) throws Exception {
		LOGGER.info("Initializing Syslog Node sample data for project: {}", project.getName());
		
		final CBabSyslogNodeService service = 
			(CBabSyslogNodeService) CSpringContext.getBean(CEntityRegistry.getServiceClassForEntity(clazz));
