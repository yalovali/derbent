...
/** CBabROSNodeService - Service for ROS virtual network nodes. Layer: Service (MVC) Active when: 'bab' profile is active Following Derbent pattern:
 * Entity service extending common node base service. Provides ROS-specific business logic: - ROS master URI validation - ROS version validation
 * (ROS1/ROS2) - Topic and service validation */
@Service
@Profile ({
		"bab", "default", "test"
})
@PreAuthorize ("isAuthenticated()")
public class CBabROSNodeService extends CBabNodeService<CBabROSNode> implements IEntityRegistrable, IEntityWithView {

	private static final Logger LOGGER = LoggerFactory.getLogger(CBabROSNodeService.class);

	public CBabROSNodeService(final IROSNodeRepository repository, final Clock clock, final ISessionService sessionService) {
	...
	@Override
	public void copyEntityFieldsTo(final CBabROSNode source, final CEntityDB<?> target, final CCloneOptions options) {
	...
	@Override
	public Class<CBabROSNode> getEntityClass() { return CBabROSNode.class; }

	@Override
	public Class<?> getInitializerServiceClass() { return CBabROSNodeInitializerService.class; }
	// IEntityRegistrable implementation

	@Override
	public Class<?> getPageServiceClass() { return CPageServiceROSNode.class; }

	@Override
	public Class<?> getServiceClass() { return CBabROSNodeService.class; }
	...
	protected void validateEntity(final CBabROSNode entity) {
		...
		if (!entity.getRosMasterUri().startsWith("http://") && !entity.getRosMasterUri().startsWith("https://")) {
			throw new CValidationException("ROS Master URI must start with http:// or https://");
		}
		// Port validation
		if (entity.getRosMasterPort() == null) {
			throw new CValidationException("ROS Master Port is required");
		}
		validateNumericField(entity.getRosMasterPort(), "ROS Master Port", 65535);
		if (entity.getRosMasterPort() < 1) {
			throw new CValidationException("ROS Master Port must be between 1 and 65535");
		}
		...
		if (!"ROS1".equals(entity.getRosVersion()) && !"ROS2".equals(entity.getRosVersion())) {
			throw new CValidationException("ROS Version must be ROS1 or ROS2");
		}
		// Namespace validation
		Check.notBlank(entity.getNodeNamespace(), "Node Namespace is required");
		if (!entity.getNodeNamespace().startsWith("/")) {
			throw new CValidationException("Node Namespace must start with /");
		}
		// Topics validation
		...
		final var existingPort =
				repo.findByMasterPortAndInterfaceAndProject(entity.getRosMasterPort(), entity.getPhysicalInterface(), entity.getProject());
		if (existingPort.isPresent() && !existingPort.get().getId().equals(entity.getId())) {
			throw new CValidationException("ROS Master port %d is already used by another ROS node on interface %s in this project"
