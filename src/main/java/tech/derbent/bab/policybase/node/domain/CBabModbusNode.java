...
/**
 * CBabModbusNode - Modbus RTU/ASCII virtual network node entity.
 * 
 * Layer: Domain (MVC)
 ...
 * - Stores Modbus-specific fields in cnode_modbus table
 * - node_type discriminator = "MODBUS"
 * 
 * Represents Modbus serial communication nodes mapped to physical serial interfaces.
 * Example: /dev/ttyS0, /dev/ttyUSB0 for industrial device communication.
 * Used in BAB Actions Dashboard policy rule engine for Modbus traffic management.
 */
@Entity
...
public class CBabModbusNode extends CBabNodeEntity<CBabModbusNode> {
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "File attachments for this Modbus node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Comments for this Modbus node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Related links for this Modbus node",
		hidden = false,
	...
	@AMetaData(
		...
		required = true,
		readOnly = false,
		description = "Modbus slave ID (1-247)",
		hidden = false
	)
	...
	@AMetaData(
		...
		required = true,
		readOnly = false,
		description = "Serial port baudrate (e.g., 9600, 19200, 115200)",
		hidden = false
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Parity checking (NONE, EVEN, ODD)",
		hidden = false,
		maxLength = 10
	)
	...
	@AMetaData(
		displayName = "Modbus Mode",
		required = true,
		readOnly = false,
		description = "Modbus protocol mode (RTU or ASCII)",
		hidden = false,
		maxLength = 10
	)
	 ...
	 */
	public boolean isRtuMode() {
		return "RTU".equalsIgnoreCase(modbusMode);
	}
}
