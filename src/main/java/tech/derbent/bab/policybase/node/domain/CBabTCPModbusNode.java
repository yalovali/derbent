...
/**
 * CBabTCPModbusNode - Modbus TCP virtual network node entity.
 * 
 * Layer: Domain (MVC)
 ...
 * - Stores Modbus TCP-specific fields in cnode_tcp_modbus table
 * - node_type discriminator = "TCP_MODBUS"
 * 
 * Represents Modbus TCP/IP communication nodes mapped to ethernet interfaces.
 * Example: TCP-based Modbus for industrial Ethernet devices.
 * Used in BAB Actions Dashboard policy rule engine for Modbus TCP traffic management.
 */
@Entity
...
public class CBabTCPModbusNode extends CBabNodeEntity<CBabTCPModbusNode> {
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "File attachments for this TCP Modbus node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Comments for this TCP Modbus node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Related links for this TCP Modbus node",
		hidden = false,
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Modbus TCP server IP address or hostname",
		hidden = false,
		maxLength = 100
	)
	...
	@AMetaData(
		...
		required = false,
		readOnly = false,
		description = "Modbus response timeout in milliseconds",
		hidden = false
	)
	...
	public String getConnectionUrl() {
		return "modbus+tcp://%s:%d".formatted(serverAddress, serverPort);
	}
}
