...
public final class CBabPolicyRuleInitializerService extends CInitializerServiceBase {
	...
	public static CDetailSection createBasicView(final CProject<?> project) throws Exception {
		try {
			...
			LOGGER.error("Failed to create policy rule view for projectId={}. reason={}",
	...
	public static CGridEntity createGridEntity(final CProject<?> project) {
		...
		grid.setColumnFields(List.of("id", "name", "active", "rulePriority", "executionOrder", "sourceNode", "actions",
				...
				CBabNodeEntity<
						?>> compatibleNodes =
								availableNodes.stream()
										.filter(node -> node instanceof CBabCanNode
												|| node instanceof CBabFileOutputNode || node instanceof CBabROSNode)
										.toList();
	...
	public static void initializeSample(final CProject<?> project, final boolean minimal) throws Exception {
		...
		initializeProjectEntity(nameAndDescriptions,
				...
				project, minimal, (item, index) -> {
					// Type-safe cast
					if (item instanceof final CBabPolicyRule rule) {
						...
						final CBabNodeEntity<?> sourceNode =
								availableNodes.get(ThreadLocalRandom.current().nextInt(availableNodes.size()));
						rule.setSourceNode(sourceNode);
						rule.setFilter(getAvailableFiltersForNode(sourceNode).stream().findFirst().orElse(null));
						final CBabNodeEntity<?> destinationNode =
								getCompatibleActionDestinationNode(availableNodes, index);
