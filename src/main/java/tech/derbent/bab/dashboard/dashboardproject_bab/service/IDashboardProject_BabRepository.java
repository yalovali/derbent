...
/** IDashboardProject_BabRepository - Repository interface for BAB dashboard projects. Layer: Service (MVC) Following Derbent pattern: Concrete
 * repository with HQL queries. */
@Profile ("bab")
public interface IDashboardProject_BabRepository extends IEntityOfProjectRepository<CDashboardProject_Bab> {
	...
	@Query ("SELECT d FROM CDashboardProject_Bab d WHERE d.active = true ORDER BY d.name ASC")
	List<CDashboardProject_Bab> findByActiveTrue();
	@Query ("SELECT d FROM CDashboardProject_Bab d WHERE d.active = true AND d.project = :project ORDER BY d.name ASC")
	List<CDashboardProject_Bab> findByActiveTrueAndProject(@Param ("project") CProject<?> project);
	@Override
	@Query ("""
			SELECT DISTINCT d FROM CDashboardProject_Bab d
			LEFT JOIN FETCH d.project
