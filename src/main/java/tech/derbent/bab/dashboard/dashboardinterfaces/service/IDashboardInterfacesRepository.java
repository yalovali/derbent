 ...
 * <p>
 * Layer: Repository (MVC) Profile: bab
 * <p>
 * Following Derbent pattern: Interface extending base repository with custom queries. Provides data access methods for BAB interface dashboard
 * entities. */
@Profile ("bab")
public interface IDashboardInterfacesRepository extends IEntityOfProjectRepository<CDashboardInterfaces> {
	...
	@Query ("SELECT COUNT(d) FROM #{#entityName} d WHERE d.project.id = :projectId AND d.active = true")
	...
	/** Find interface dashboards by configuration mode. Eager loads project and related entities.
	...
	/** Find dashboard by ID with complete eager loading. Override base method to prevent N+1 queries in forms.
	...
	/** Find interface dashboards by project and active status. Eager loads project, createdBy, and composition fields.
	 * @param projectId the project identifier
	...
	@Query ("""
			...
			LEFT JOIN FETCH d.comments
			LEFT JOIN FETCH d.links
			WHERE d.project.id = :projectId AND d.active = :active
			ORDER BY d.name ASC
			""")
	List<CDashboardInterfaces> findByProjectIdAndActive(@Param ("projectId") Long projectId, @Param ("active") Boolean active);
	/** Find active interface dashboards for a project with eager loading. Override base method with eager loading for UI performance.
	 * @param projectId the project identifier
	...
	@Query ("""
			...
			LEFT JOIN FETCH d.comments
			LEFT JOIN FETCH d.links
			WHERE d.project.id = :projectId AND d.active = true
			ORDER BY d.id DESC
			""")
