...
/** CNodeEntityService - Abstract service for virtual network nodes in BAB Actions Dashboard. Layer: Service (MVC) Active when: 'bab' profile is
 * active Following Derbent pattern: Abstract service with NO @Service annotation. Provides common business logic for all virtual network node types:
 * - HTTP Server nodes, Vehicle nodes, File Input nodes - Node configuration validation and generation - Calimero integration support - Physical
 * interface conflict detection Abstract services do NOT implement IEntityRegistrable or IEntityWithView. Only concrete services implement these
 * interfaces. */
@Profile ("bab")
...
public abstract class CBabNodeEntityService<NodeType extends CBabNodeEntity<NodeType>> extends CEntityOfProjectService<NodeType> {

	private static final Logger LOGGER = LoggerFactory.getLogger(CBabNodeEntityService.class);

	protected CBabNodeEntityService(final INodeEntityRepository<NodeType> repository, final Clock clock, final ISessionService sessionService) {
		super(repository, clock, sessionService);
	}

	/** Copy entity fields from source to target. Implements service-based copy pattern for node entities. */
	@Override
	public void copyEntityFieldsTo(final NodeType source, final CEntityDB<?> target, final CCloneOptions options) {
		...
		if (!(target instanceof CBabNodeEntity)) {
			return;
		}
		...
		LOGGER.debug("Copied {} node '{}' with options: {}", source.getNodeType(), source.getName(), options);
	}

	/** Copy node-specific fields from source to target. Abstract method for type-specific field copying. */
	...
	/** Generate default node configuration JSON. Abstract method implemented by concrete services for type-specific configuration. */
	protected abstract String generateDefaultNodeConfiguration(final NodeType entity);

	@Override
	public void initializeNewEntity(final Object entity) {
		super.initializeNewEntity(entity);
		if (!(entity instanceof CBabNodeEntity)) {
			return;
		}
		@SuppressWarnings ("unchecked")
	...
	/** Initialize node-specific default values. Abstract method for type-specific initialization. */
	protected abstract void initializeNodeSpecificDefaults(final NodeType entity);

	/** Check if physical interface format is valid. Base implementation - concrete services should override for specific validation. */
	...
	/** Generate updated node configuration JSON. Regenerates configuration based on current node settings. */
	@Transactional
	...
	public void updateConnectionStatus(final NodeType node, final String connectionStatus) {
		...
		save(node);
		LOGGER.debug("Updated connection status for node '{}' to: {}", node.getName(), connectionStatus);
	}
	...
	/** Validate JSON configuration format and content. Abstract method for type-specific JSON validation. */
	protected abstract void validateJsonConfiguration(final NodeType entity, final String configJson);

	/** Validate node configuration JSON. Ensures JSON is valid and contains required fields. */
	protected void validateNodeConfiguration(final NodeType entity) {
		...
		if (configJson != null && !configJson.trim().isEmpty()) {
			try {
				...
				throw new CValidationException("Invalid node configuration JSON: %s".formatted(e.getMessage()));
			}
		}
	}

	/** Validate node-specific fields. Abstract method implemented by concrete services for type-specific validation. */
	protected abstract void validateNodeSpecificFields(final NodeType entity);

	/** Validate physical interface configuration. Checks for interface conflicts and availability. */
	protected void validatePhysicalInterface(final NodeType entity) {
		...
		final boolean interfaceInUse = nodeRepo.existsByPhysicalInterfaceAndProject(physicalInterface, entity.getProject());
		if (interfaceInUse) {
			// If interface is in use, check if it's the same entity (update scenario)
			final var existingNode = nodeRepo.findByPhysicalInterfaceAndProject(physicalInterface, entity.getProject());
			if (existingNode.isPresent() && (entity.getId() == null || !existingNode.get().getId().equals(entity.getId()))) {
				throw new CValidationException(
						"Physical interface '%s' is already in use by node '%s'".formatted(physicalInterface, existingNode.get().getName()));
			}
		}
	...
	/** Validate physical interface format. Can be overridden by concrete services for specific interface validation. */
	...
	protected void validatePhysicalInterfaceFormat(final NodeType entity, final String physicalInterface) {
		...
		if (!isValidPhysicalInterface(physicalInterface)) {
			throw new CValidationException("Invalid physical interface format: '%s'".formatted(physicalInterface));
		}
	}
}
