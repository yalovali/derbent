...
/**
 * CBabNode - Base class for all communication nodes.
 * Following Derbent pattern: Abstract entity class with proper inheritance.
...
public abstract class CBabNode extends CEntityOfCompany<CBabNode> {
	...
	@AMetaData(
		displayName = "Device", required = true, readOnly = true, 
		description = "Device this node belongs to", hidden = false
	)
	...
	@Size(max = 50)
	@AMetaData(
		displayName = "Node Type", required = true, readOnly = true, 
		description = "Type of communication node (CAN, Modbus, Ethernet, ROS)",
	...
	@AMetaData(
		displayName = "Enabled", required = true, readOnly = false, 
		description = "Whether this node is enabled", hidden = false
	)
	...
	@AMetaData(
		...
		description = "Current node status (Active, Inactive, Error)", hidden = false, maxLength = 50
	)
	private String nodeStatus;

	@Column(name = "port_number", nullable = true)
	@AMetaData(
		displayName = "Port", required = false, readOnly = false, 
		description = "Port number or identifier", hidden = false
	)
	...
	@AMetaData(
		...
		description = "User who created this node", hidden = false,
