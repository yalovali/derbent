...
public class CBabNodeCAN extends CBabNode {
	...
	@AMetaData(
		displayName = "Bitrate", required = false, readOnly = false, 
		description = "CAN bus bitrate (e.g., 500000)", hidden = false
	)
	private Integer bitrate;

	@Column(name = "sample_point", nullable = true)
	@AMetaData(
		displayName = "Sample Point", required = false, readOnly = false, 
		description = "CAN bus sample point (0.0-1.0)", hidden = false
	)
	...
	@AMetaData(
		...
		description = "Linux CAN interface name (e.g., can0)", hidden = false, maxLength = 50
	)
